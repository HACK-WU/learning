...
### 解法一览（效果按题定制：一致性强度 / 业务侵入）

| 解法 | 一致性强度 / 业务侵入 | 代价 | 适用边界 |
|------|----------------------|------|---------|
| **A · 双写 + 消费端幂等 + 定时对账** | 一致性强度：**最终一致**（分钟级不一致窗口）；业务侵入：中（改造消费端 + 加对账任务） | 需要一个对账/补偿任务；存在分钟级不一致窗口 | 流量不大、能接受短暂不一致的绝大多数业务 |
| **B · 事务性发件箱（Transactional Outbox）** | 一致性强度：**不丢**（写库成功即保证会发，重复靠幂等收敛）；业务侵入：中高（多一张 outbox 表 + 一个投递器） | 多一张 outbox 表（会膨胀，需定期清理）；需要一个投递器（轮询或 CDC） | **通用正解**：要求"不丢"且不允许凭空多出消息 |
| **C · CDC 直连 binlog（如 Debezium）** | 一致性强度：**不丢**（跟随 binlog，顺序与事务边界一致）；业务侵入：低（业务代码零改动） | 引入 CDC 组件与运维复杂度；表结构变更需要同步维护；捕获的是"数据变更"而非"业务意图" | 多表/遗留系统，或希望业务代码零侵入 |
| **D · Kafka 事务** | 一致性强度：**仅限 Kafka 内部**原子（管不到数据库）；业务侵入：低 | **管不到数据库**——写库与发消息分属两个系统，它无法把它们绑成一个事务 | 消费-处理-再生产（Kafka → Kafka）的流式链路 |
| **E · 先发消息、后写库**（或反之） | 一致性强度：**无改善**（只是换个崩溃方向）；业务侵入：低 | 两个动作之间的崩溃必然产生不一致，只是方向不同 | 只能作为 A 的变体，不是独立解法 |
...
```mermaid
flowchart LR
  W["写库 + 发消息（两次独立动作）"] --> DUP["重复：重试 / 至少一次投递"]
  W --> MISS["遗漏：第二个动作前崩溃"]
  DUP --> I["消费端幂等 upsert<br/>按业务主键收敛"]
  MISS --> R["定时对账任务<br/>比对库与事件"]
  R --> FIX["补偿补发"]
  I --> OK["最终一致"]
  FIX --> OK
  OK -.-> X["不一致没有被消除，只是被压缩到对账周期内<br/>窗口内读到的数据仍然是旧的"]
  style X fill:#c62828,color:#ffffff
```

> 读图指引：两条路分别兜住"重复"和"遗漏"——幂等吃掉重复、对账补上遗漏，最后收敛到最终一致。**红框是它的本质**：不一致并没有被消除，只是被压缩到对账周期内，窗口内读到的仍是旧数据。
...
```mermaid
flowchart LR
  T["本地事务：写 orders + 写 outbox"] --> R["投递器取未投递记录"]
  R --> S["send 到 Kafka"]
  S --> M{"标记 published_at 之前崩溃？"}
  M -->|"否"| D["标记已投递"]
  M -->|"是"| X["下轮再取一次 → 同一事件被发两遍"]
  X --> I["消费端幂等 upsert 收敛"]
  D --> I
  D -.-> Y["outbox 表持续膨胀<br/>必须配清理任务"]
  style X fill:#c62828,color:#ffffff
  style Y fill:#c62828,color:#ffffff
```

> 读图指引：outbox 把"写库"和"待发消息"绑进同一个本地事务，解决的是**丢**；投递器把消息送进 Kafka，但它是**至少一次**——发完还没标记就崩溃会重发。**红框是两个代价**：重复必须靠消费端幂等收敛，outbox 表必须配清理任务。
...
```mermaid
flowchart LR
  DB[("业务库 orders 表")] --> BIN["binlog"]
  BIN --> C["CDC 连接器（Debezium）"]
  C --> T["Kafka topic"]
  T --> CON["下游消费者"]
  C -.-> X["捕获到的是「数据变更」<br/>不是「业务意图」"]
  DB -.-> Y["表结构变更要同步维护连接器配置<br/>否则事件缺字段或解析失败"]
  style X fill:#c62828,color:#ffffff
  style Y fill:#c62828,color:#ffffff
```

> 读图指引：业务只管写库，CDC 从 binlog 侧把变更变成事件，业务代码一行都不用改。**红框是两条代价**：捕获到的是"某行某列从 A 变成 B"而不是"用户下单了"，语义要靠 outbox 之类的结构化表补；表结构一变，连接器配置就得跟着维护。
...
![Kafka 事务的作用域边界](assets/scene-03-d-transaction-scope.svg)

> 读图指引：虚线框内是事务真能管住的三样东西（多个 topic 的输出与消费位移），红色实线框里的数据库提交在作用域之外。**红框是失效点**：拿 Kafka 事务去包数据库写入，是把工具边界记错了，崩溃时依然不一致——它只剩"Kafka → Kafka 流式链路"这一个正确用法。
...
```mermaid
flowchart LR
  S1["顺序一：先写库"] --> C1["发消息前崩溃"] --> R1["有订单 · 无事件<br/>下游永远不知道"]
  S2["顺序二：先发消息"] --> C2["写库前崩溃"] --> R2["有事件 · 无订单<br/>下游处理了不存在的订单"]
  R1 --> X["两种顺序只是换了个崩溃方向<br/>不一致本身没有消失"]
  R2 --> X
  style X fill:#c62828,color:#ffffff
```

> 读图指引：把两个动作排成任意顺序，中间那道缝就是崩溃窗口——先写库则"有订单无事件"，先发消息则"有事件无订单"。**红框是结论**：换顺序只是换了不一致的方向，它解决不了任何问题，只能挑一个方向再靠 A（幂等 + 对账）兜住。
...
|---|---|---|
| **Transactional Outbox + RabbitMQ** | 数据库事务内写 outbox，投递器发布到 RabbitMQ，再用 publisher confirms 和消费幂等收敛 | 仍需 outbox，但任务确认与路由更直接；失去 Kafka 日志式多下游回放模型 |
| **同步 API + 对账任务** | 直接调用下游，失败记录补偿表，定时重试和对账 | 适合下游少、低延迟业务；跨团队扩展和峰值削峰能力较弱 |
...
- **解法映射**：A/D/E → [课 8](../stages/3-可靠性与高可用/lessons/lesson-08-交付语义与幂等.md)；B → [课 10](../stages/4-实战与架构落地/lessons/lesson-10-项目架构设计落地.md)；C → [课 9](../stages/4-实战与架构落地/lessons/lesson-09-代码开发实战.md) + [课 10](../stages/4-实战与架构落地/lessons/lesson-10-项目架构设计落地.md)
...
![Transactional Outbox 与 CDC](assets/scene-03-outbox-and-cdc.svg)

> 读图指引：业务事务只在数据库边界内保证同成同败，投递器/CDC 再把 outbox 事件送进 Kafka；Kafka 侧仍要靠幂等收敛重复。

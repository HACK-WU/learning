...
> 本手册汇总 `kafka/` 全部教学内容：7 个阶段 19 课 + 综合实战项目 + 实战经验与排障速查手册。
> **定位**：复习与速查用的"一本书"。想系统学请回到各课原文（本手册每课都给了链接），想查问题直接翻 [实战与排障](#四实战与排障)。
> 汇总完成时间：2026-08-31；**2026-09-14 更新**：补齐阶段 6–7（课 15–19），并为全 19 课统一新版讲义规范（见 [讲义读法说明](#讲义读法说明2026-09-14-全-19-课统一规范)）；**2026-09-17 更新**：场景解法库由 8 题扩展为 11 题，并补齐场景级 SVG 与 4.2+ share groups 的当前口径。
...
## 讲义读法说明（2026-09-14 全 19 课统一规范）

每一课原文都按同一套结构组织，先弄清这 7 个固定入口，读起来会快很多：

| 入口 | 在哪 | 回答什么 | 长什么样 |
|------|------|---------|---------|
| **一句话本质** | 第一幕末尾 | 本课到底把什么改成了什么 | 加粗一句 + 「处境对照」左右对比 |
| **处境对照** | 第一幕末尾 | 不这么做会怎样 vs 这么做会怎样 | 两行对比，配 ⏳ 数字来源说明 |
| **一眼全局图** | 第三幕开头 | 本课要解决什么问题、靠什么思路 | **固定 SVG**（非 mermaid），**图上零术语** |
| **本课地图** | 第三幕开头 | 分几步走、现在在哪 | **表格**（不是图），不标学习状态 |
| **🧭 衔接句** | 每个知识点前 | 上一步到本步的关系 | `> 🧭 第 N/M 步｜承接：… → 本步：…` |
| **行话对照** | 知识点讲透后 | 本课说法 ↔ 行业标准叫法 | 四列对照表 + 译名标注说明 |
| **一图总结** | 课末 | 本课讲了什么（复习用） | mermaid，知识视角 |

> ⚠️ **三方视角，别混为一谈**：「一眼全局图」在**课首**、**问题视角**、**零术语**，是给没学过的人看的；「本课地图」在**课首**、**路线视角**，是表格不是图；「一图总结」在**课末**、**知识视角**，是给刚学完的人复习用的。三者不可替代。

**关于图中的数字**：凡非实测的量化锚点都标了 ⏳，并在「数字来源说明」里明确区分**本课实测值**与**官方数据**。实测值来自本课 3 节点实验环境（`apache/kafka:4.0.0`），**换环境就会变**——尤其注意：协调者编号取决于 `hash(组名) % 50`（课 18）、broker 能力表条目数随 Kafka 版本变化（课 19）。**请把它们当理解规则的样例，不要当通用阈值照搬。**

**关于译名**（本课程统一口径）：允许自拟中文译名，但**必须标注性质**，标注**只在首次登场给一次**：

- **公认译名**（如「同步副本」「副本」）：不标注，直接用
- **社区译名**：括注「社区常用译法、官方未定中文名」
- **自拟译名**：括注「本文自拟译…非通用译名」
- **官方文档不译的**：沿用英文原名（`RecordBatch`、`JMX`、`ApiVersions`、`__consumer_offsets` 等）

读其他资料时请以**英文原名**或该资料的译名为准，本课程的中文名只是为便于理解而标注。
...
> **七个阶段一条主线**：**动机（为什么）→ 机制（怎么工作）→ 保障（怎么可靠）→ 落地（怎么用对）→ 上生产（怎么敢用）→ 上线后（怎么活得下去）→ 原理（为什么是这样）**。阶段 5、6 属延伸补充，阶段 7 属原理深挖，主线 10 课不依赖它们。
...
> 合计 16 个知识点落点（2 + 6 + 4 + 4），与 [项目 README 知识点地图](projects/电商订单事件中心/README.md) 逐行一致；不等同于全课程 42 个知识点（部分知识点属认知铺垫，未落地为本项目代码）。
...
### [10 场景解法库](10-场景解法库/INDEX.md)（设计态 · 先想后看）

11 道开放设计题，每道给 ≥3 个解法（含代价与适用边界）+ 推荐路径 + 知识点挂钩，**每场景独立成篇**：

| # | 场景 | 核心矛盾 |
|---|------|---------|
| 1 | [流量涨 10 倍，消费者追不上](10-场景解法库/场景-01-消费者追不上.md) | 并行度被分区数锁死，分区数只能加不能减 |
| 2 | [既要保序又要并发](10-场景解法库/场景-02-保序与并发.md) | 保序要求分区映射稳定，扩容必然破坏它 |
| 3 | [数据库写 + 发消息的一致性](10-场景解法库/场景-03-双写一致性.md) | Kafka 事务管不到数据库 |
| 4 | [新业务要消费全部历史数据](10-场景解法库/场景-04-历史数据重放.md) | 默认只留 7 天，重放要靠提前设计 |
| 5 | [消息体太大](10-场景解法库/场景-05-大消息.md) | Kafka 为小消息高吞吐优化 |
| 6 | [跨机房容灾与多活](10-场景解法库/场景-06-跨机房容灾.md) | 副本抗不了整个地域故障 |
| 7 | [事件结构要升级](10-场景解法库/场景-07-事件结构演进.md) | 事件是跨服务契约，下游无法同时上线 |
| 8 | [我要"任务队列"语义](10-场景解法库/场景-08-任务队列语义.md) | 分区绑定的流式消费 ≠ 单条确认队列 |
| 9 | [多租户安全与隔离](10-场景解法库/场景-09-多租户安全与隔离.md) | 共享集群的成本与身份、资源、故障隔离互相拉扯 |
| 10 | [新增 broker 后如何安全扩容](10-场景解法库/场景-10-新增broker与分区重分配.md) | 加机器不会自动搬数据，迁移还会争抢线上带宽 |
| 11 | [滚动升级与协议兼容](10-场景解法库/场景-11-滚动升级与协议兼容.md) | 二进制升级、协议兼容、特性最终化不是同一个状态 |

> 解法全部折叠，**先自己想 30 秒再展开**。索引页有「八问思考框架」，用于应对没见过的场景；通览全部：[场景解法库索引](10-场景解法库/INDEX.md)。
...
| 课 11 | 5 | 认证 · 授权 · 加密 · listeners 与协议映射 |
| 课 12 | 5 | 多租户隔离与配额 |
| 课 13 | 5 | wire protocol 与消息格式 |
| 课 14 | 5 | 分层存储与配置进阶 |
| 课 15 | 6 | 监控与可观测 |
| 课 16 | 6 | 集群运维操作 |
| 课 17 | 7 | 网络层与请求处理模型 |
...
> 全课程合计 **19 课 · 42 个知识点 · 7 个阶段**，另有 1 个综合实战项目 + 实战经验 + 排障速查手册 + 场景解法库。

---

## 🚀 接下来可以做什么

- **练设计**：打开 [10 场景解法库](10-场景解法库/INDEX.md)，挑一个场景先自己设计，再对照解法。

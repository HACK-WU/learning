...
> ⚠️ **关于 `RequestHandlerAvgIdlePercent`（本环境实测证伪，必读）**：它在大量教程中被当作 IO 线程空闲比率使用（典型告警阈值 `< 0.3`），但**本实验环境实测它是累积计数**——5 次采样 `2.36e11 → 2.43e11 → 2.49e11 → 2.55e11 → 2.62e11` 严格单调递增，HELP 行 `attribute=Count`（非 Value）。照抄 `< 0.3` 告警将**永不触发**，且你会误判 IO 线程健康。
>
> 因此上表中它只作为"看板信号名"出现，**没有给阈值**。IO 线程压力请改用 `RequestQueueSize`（队列积压）+ `TotalTimeMs`（请求总耗时）。完整实测见 [课 17：网络层与请求处理模型](../../stages/7-实现原理/lessons/lesson-17-网络层与请求处理模型.md)。
>
> 通用纪律：**任何指标写进告警规则前，先 curl 看实际值域、读 HELP 行的 `attribute=`、连续采样 3~5 次确认单调性。**

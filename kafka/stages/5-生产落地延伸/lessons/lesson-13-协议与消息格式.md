...
> 📌 **一句话本质**：本课做的事，是把「一件一件单独记、每份都带一份完整信息」改成「一箱共用一张单据、箱里的每件只记差多少」。
>
> ⚖️ **处境对照**（沿用第一幕那四个问题：短消息压缩后反而变大、老客户端连新集群、事务怎么标记、一条消息占多少字节）：
> - **不这么做（逐条记全量）**：每条都重复写一遍地址和编号，单据重量接近甚至超过货物本身；**逐条压缩还可能越压越大**；没有版本锚点，客户端一升级就对不上话。
> - **这么做（成批记差值）**：地址只写一次、箱里只写「+0/+1/+2」这种小数字；**整箱一起压**才有收益；单据上留一个版本标记，新旧客户端就能共存。
>
> ⏳ **数字来源说明**：以上为**定性对照**（按"重复写了多少、能不能压缩"比较），不是实测字节数。第四幕会用 `kafka-dump-log.sh` 和一段 varint 编码脚本**当场量出真实数字**（varint：0/1/127 占 1 字节，128/1000 占 2 字节，定长 int32 一律 4 字节）；**具体批次大小取决于你的消息内容与压缩配置**，请以自己的实测输出为准。
...
![逐件发 vs 整箱发](../assets/lesson13-global-single-vs-batch.svg)

> 看图：**左右两边是同一批货、同样的三件东西**，差别只在装法。左边每一件都挂一张完整的单据——同样的地址重复写了三遍，而且**一件一件单独压，短东西可能越压越大**；右边一整箱只共用一张单据，箱里的每件**只写「差多少」（+0 / +1 / +2）**，数字小、占地少，还能**整箱一起压**。**注意最下面那句**："一箱"不是图省事的技巧，而是这套格式**打底的规矩**——后面讲的所有字段，都建立在这个前提上。

**四条硬约束**说明：① 本图只回答"本课要解决什么问题、靠什么思路"；② 图上不出现术语（批次、记录、偏移、压缩、校验码这些词都在下面才出场）；③ 已带读图指引；④ 图旁文字能独立说清同一件事。

### 本课地图（分几步走）

| 步 | 这一步要解决什么（人话） | 对应知识点 |
|:--:|------------------------|-----------|
| 1 | 把一条消息拆到字节，看清"一箱共用一张单据 + 箱里只记差值"到底是怎么落成的 | wire protocol 与消息格式 |

> 只回答"分几步走、现在在哪"；不写机制、不写结论；**不标学习状态**（进度以 `00-学习档案.md` 为准）。本课是**单知识点课**，一步走完：先建立整体轮廓（协议-批次-记录三层），再逐层下探。

### 知识点：wire protocol 与消息格式

> 🧭 第 1/1 步｜承接：第一幕那四个在"使用者视角"下解释不了的问题 → 本步：把消息拆到字节层面，看清格式本身怎么回答它们
...
> 看图：**从上往下看一层套一层**——黄色那一格是这一整箱**共用的一张单据**（从哪开始、多长、什么版本、封条、谁发的、共几件），下面挨着的三件货**共享它**。关键点：**单据只有一份**，不是每件货配一张；想知道某一件的绝对编号，要拿单据上的起始编号**加上**它自己的差值。
...
本课为了讲清思路，用的是"单据/箱子/封条"这类说法。真到读文档、配参数、看日志时，你会遇到下面这些行话——**它们是同一件事的另一套名字**：

| 本课说法（人话） | 行业标准叫法 | 典型配置 / 在哪遇到 | 代价 / 注意 |
|---|---|---|---|
| 一整箱共用的一张单据 | **RecordBatch**（本文自拟译"记录批次"，非通用译名） | `kafka-dump-log.sh` 输出的 `baseOffset / lastOffset / count` 一行 | 批量是**格式的基本假设**，不是优化；极端情况一批只有一条 |
| 箱里的一件货 | **Record**（本文自拟译"记录"，非通用译名） | 客户端 `ConsumerRecord`；dump 输出的 `payload:` 行 | 单条 record **没有绝对 offset 字段** |
| 只写"差多少"（+0/+1/+2） | `offsetDelta` / `timestampDelta` + **varint / varlong**（本文自拟译"变长整数编码"，非通用译名；与 Protobuf 编码相同） | Record 的字节定义；第 4 步的 `varint_size` 脚本 | 小数字 1 字节、定长 int32 一律 4 字节——**省的是元数据开销** |
| 单据上的版本标记 | `magic` 字节（本文自拟译"格式版本号"，非通用译名；当前为 2） | 批次头部第 4 个字段；必须先解析它才能解释后续字节 | 它是**版本演进的锚点**，也是新旧客户端能共存的基础 |
| 封条 | **CRC-32C**（本文自拟译"循环冗余校验"，非通用译名；Castagnoli 多项式） | 批次头部 `crc: uint32` | 覆盖范围是**attributes 到批次末尾**，**不含** `partitionLeaderEpoch`（该字段 broker 收到后才赋值，纳入就得每批复算） |
| 事务的那张"标记单" | **Control Batch**（本文自拟译"控制批次"，非通用译名）＋ **abort marker / commit marker**（本文自拟译"中止标记 / 提交标记"，非通用译名） | `isControl: true`；`attributes` 的 bit 5 | **不传给应用程序**；value 对客户端**不透明** |
| 谁发的、第几次发 | `producerId` / `producerEpoch` / `baseSequence`（即课 8 幂等与事务的三个字段） | dump 输出；未开幂等时为 `-1` | 压实时**空批次也要保留**，否则丢最后序列号 → leader 切换后生产者报 `OutOfSequence` |
| 整箱大小 | **batchLength + 12 字节** | 容量估算、`message.max.bytes` | `batchLength` 统计的是**该字段之后**到末尾，别忘了加前 12 字节 |
| 0.11 之前的老装法 | **message sets**（本文自拟译"消息集"，非通用译名） | 老文档、老工具 | 与现行格式**不是一回事**，看老资料要先分辨 |

> 📖 **关于译名**：本表带"本文自拟译"字样的中文名是**为便于阅读自拟的、非通用译名**——读其他资料时请以英文原名或该资料的译名为准。**标注只在首次登场给一次**，后文不再重复括注。
...
> 📘 **[→ 打开应用实战：协议与消息格式](../../../应用实战/13-协议与消息格式.md)**
>
> 课里讲的是「一箱共用一张单据 + 箱里只记差值」这个格式结论，实战篇带你把它从磁盘上挖出来看：① 基础版——逐条发送并 dump，亲眼看到 `count: 1` 出现五次、头部开销被抄了五遍；② 综合版——攒批 + 整批压缩 + 开幂等，再 dump 一次对比 `count: 5` 与 `producerId` 的变化。

...
> 📌 **一句话本质**：本课做的事，是把**「大家共用一套东西、谁抢到算谁的」**改成**「先划清谁的地盘，再给每人定一个'每次能用多少'的额度」**。
>
> ⚖️ **处境对照**（沿用第一幕的事故：`orders` 消费延迟从毫秒级飙到几十秒）：
> - **不这么做（无限额）**：一个团队的**合理业务**（全量回溯）就能把共用资源吃满，**其他所有团队一起变慢**；而且出事后连"这个 topic 是谁的"都要在群里问一轮。
> - **这么做（划地盘 + 配额）**：命名规范 + 前缀 ACL 让归属一目了然；额度把"能干多少"框住——**正经事照干，只是慢一点**，别人照样用得上。
>
> ⏳ **数字来源说明**：以上为**定性对照**（按"是否相互影响"比较），不是实测延迟数据。"毫秒级 → 几十秒"是**本课场景设定的假设数字**，用于讲清量级变化；真实延迟取决于集群规模、数据量与配额设置，**具体阈值需按自己的业务评估**。
...
![共用电梯：无人限量 vs 定好额度](../assets/lesson12-global-shared-elevator.svg)

> 看图：**左右两边是同一部电梯**（没有分开、也没有多加），差别只在**右边给"每次能搬多少"定了额度**。左边搬家队一拥而上，电梯被挤爆，其他人干等甚至卡住；右边它**照样能搬，只是搬得慢一点**，别人也用得上。**注意最下面那句**：被限的人**不会收到"不许搬"**，只会觉得"今天怎么有点慢"——所以不盯着看，就根本不知道发生了什么。

**四条硬约束**说明：① 本图只回答"本课要解决什么问题、靠什么思路"；② 图上不出现术语（多租户、配额、限流、principal、ACL 这些词都在下面才出场）；③ 已带读图指引；④ 图旁文字能独立说清同一件事。

### 本课地图（分几步走）

| 步 | 这一步要解决什么（人话） | 对应知识点 |
|:--:|------------------------|-----------|
| 1 | 先划清"谁的地盘"，让出事时能一眼找到人 | 多租户隔离（命名 + 前缀 ACL） |
| 2 | 再定"每人能用多少"，别让一个人拖垮所有人 | 配额（客户端配额 / 服务端配额） |

> 只回答"分几步走、现在在哪"；不写机制、不写结论；**不标学习状态**（进度以 `00-学习档案.md` 为准）。

### 知识点：多租户隔离与配额

> 🧭 第 1/2 步｜承接：第一幕里"五个团队几十个 topic 混在一起、一个回溯任务拖垮全集群" → 本步：先划清谁的地盘
...
> 看图：**从上往下看**——额度是绑在"人"身上的（左边那个框），**不管他读写哪个东西都算在他头上**；然后分三种尺子量：**请求快慢**（最该管，它护的是机器的算力）、**搬了多少字节**、**折腾目录的频繁程度**（这条最容易被忘，但它压垮的是"管事的人"）。**最下面黄色那格是重点**：超了之后是**限流**，不是拒绝。
...
> 本课讲的是"限量"这件事的三种尺子，选错尺子等于没限：

| 本课说法（人话） | 行业标准叫法 | 典型配置 / 在哪遇到 | 代价 |
|---|---|---|---|
| 限"折腾得多快"（最该配） | **请求速率配额 request rate** | `--add-config request_percentage=<n>` | **官方认为隔离效果最显著**——护的是 broker CPU |
| 限"搬了多少字节" | **网络带宽配额** `producer_byte_rate` / `consumer_byte_rate` | `kafka-configs.sh --entity-type users` | 有用，但**常常不是真正的瓶颈** |
| 限"建删改多频繁" | **Topic 操作配额** `controller_mutation_rate`（KIP-599） | 高频元数据操作场景 | 最易被忽略；**它护的是 controller（集群大脑）** |
| 限"连得多猛" | **服务端配额**（连接速率 / 最大连接数 / 单 IP 连接数） | broker 侧配置 | 防"连接风暴"（如客户端 bug 疯狂重连） |

> 🔴 **为什么优先限请求速率**：官方明确指出——**过度的 CPU 占用会降低 broker 能服务的有效带宽**，所以"带宽是表象，CPU 才是瓶颈"。很多情况下请求速率配额的隔离效果**比带宽配额更显著**。
> 💡 **配额绑的是"人"不是"数据"**：客户端配额按 **principal（用户主体）** 生效，**与读写哪个 topic 无关**——这正是它适合多租户的原因。
...
> 📘 **[→ 打开应用实战：多租户与配额](../../../应用实战/12-多租户与配额.md)**
>
> 课里讲的是「划地盘 + 限量」两条治理线，实战篇带你把事故复现再堵上：① 基础版——三个团队共用集群，用压测工具猛灌一个 topic，体会「所有人变慢，但没有任何一条报错」；② 综合版——按 principal 配请求速率与带宽配额、关掉自动建 topic、用前缀 ACL 强制命名规范，并解释为什么「不监控就等于没配」。

...
> 📖 **先认三个缩写**（本课全程用它们，中文名是**为便于理解标注的**，读文档时请以英文原名为准）：
> - **SASL**（Simple Authentication and Security Layer，简单认证与安全层；本文标注为"认证框架"）：一套"怎么证明你是谁"的**框架**，Kafka 在它下面挂了四种具体机制（下面会讲）。
> - **ACL**（Access Control List，访问控制列表；本文标注为"权限清单"）：一张"谁能对哪个东西做什么"的**清单**，是 Kafka 落地授权的方式。
> - **SSL**（Secure Sockets Layer，安全套接层；实际用的是它的继任者 **TLS**，但 Kafka 配置项与文档**沿用 SSL 这个名字**）：既可用于**加密传输**，也可用于**认证**（双向证书）。

> 📌 **一句话本质**：本课做的事，是把**「大门敞着、谁来都能进、进来哪儿都能逛」**改成**「先查身份、再按权限开门、路上说的话还不让别人听见」**。
>
> ⚖️ **处境对照**（沿用第一幕的安全团队三问，以"一台被挖矿的机器连上来"为假设场景）：
> - **不这么做（裸奔）**：只要网络通，**任何人都能读走全部订单、也能伪造订单写进去**；而且消息在网上跑的是**明文**，被抓包就是可读文本。出了事连"是谁干的"都查不出来。
> - **这么做（三件套）**：没凭据的人连不上（认证）；有凭据的人也只能碰自己被授权的那一摊（授权）；内容即使被抓包也是密文（加密）。
>
> ⏳ **数字来源说明**：以上为**定性对照**（按"谁能访问、能看到什么"比较），不是实测数据。性能方面官方只说明 SSL **存在损耗、量级取决于 CPU 类型与 JVM 实现**，未给统一倍数——**具体损耗需在自己环境压测**，不要照抄他人的百分比。
...
![只查身份 vs 查身份再按权限开门](../assets/lesson11-global-door-vs-rooms.svg)

> 看图：**左边**只在大门查一次身份——进来之后**所有房间随便逛**，连"重地"也进得去，等于查了也白查；**右边**是查完身份**还要按权限开门**，不是自己的房间直接挡住。**最下面那条黄色是第三件事**：就算人管住了，路上说的话仍可能被听见（敞开说 vs 关进小房间说，后者要费点劲）。三件事缺一件都不算安全——**少了中间那道，"查了身份"也只是全楼通行**。

**四条硬约束**说明：① 本图只回答"本课要解决什么问题、靠什么思路"；② 图上不出现术语（认证、授权、加密、SASL、ACL、listeners 这些词都在下面才出场）；③ 已带读图指引；④ 图旁文字能独立说清同一件事。

### 本课地图（分几步走）

| 步 | 这一步要解决什么（人话） | 对应知识点 |
|:--:|------------------------|-----------|
| 1 | 先把"查身份、管权限、防偷看"这三件事分清楚 | 知识点 1：认证 · 授权 · 加密 |
| 2 | 再把这三套规则绑到不同的门（端口）上 | 知识点 2：listeners 与协议映射 |

> 只回答"分几步走、现在在哪"；不写机制、不写结论；**不标学习状态**（进度以 `00-学习档案.md` 为准）。

### 知识点 1：认证·授权·加密——安全三件套

> 🧭 第 1/2 步｜承接：第一幕里"谁能进、进了能干什么、路上安全吗"三问 → 本步：拆开看，这其实是三件互相独立的事
...
> 本课讲的是"三件独立的事"，认证那一组四种机制是选型时的固定考点：

| 本课说法（人话） | 行业标准叫法 | 典型配置 / 在哪遇到 | 代价 |
|---|---|---|---|
| 查身份（工牌） | **认证 Authentication**（SSL 或 SASL） | `sasl.mechanism`；JAAS 配置 | 三件套之一；**单独开 ≠ 安全** |
| 管能进哪间 | **授权 Authorization**（内置实现为 ACL） | `authorizer.class.name=…StandardAuthorizer`；`kafka-acls.sh` | **不配 = 全放行**；开前务必设 `super.users` |
| 路上防偷看 | **加密 Encryption**（SSL/TLS） | `listener.security.protocol.map` 里的 `SSL` / `SASL_SSL` | 官方明示**有性能损耗**（量级取决于 CPU 与 JVM） |
| 已有企业票据体系 | **SASL/GSSAPI（Kerberos）** | 0.9.0.0 引入 | 要依赖 KDC |
| 用户名 + 密码 | **SASL/PLAIN** | 0.10.0.0 引入 | ⚠️ **凭据明文编码（base64）传输，必须配 SSL** |
| 挑战-应答、不传密码 | **SASL/SCRAM-SHA-256/512** | 0.10.2.0 引入 | **推荐的通用选择** |
| OAuth 令牌 | **SASL/OAUTHBEARER** | 2.0 引入 | 需已有 OAuth 体系 |

> 🔴 **选型路径**：有 Kerberos → GSSAPI；没有 → **SCRAM**（通用首选）；PLAIN 仅在**全程 SSL** 时才用；OAUTHBEARER 适合已上 OAuth 的平台团队。
> ⚠️ **PLAIN 的坑**：名字里的 "PLAIN" 不是"简单"，是"明文"——**密码以 base64 编码（不是加密）在网络上传输**，单独用等于把密码写在明信片上。
...
> 🧭 第 2/2 步｜承接：上一步分清了三件事 → 本步：把这三套规则绑到不同的门（端口）上，否则一开就全开、容易把自己锁在门外
...
> 本课讲的是"同一件事（开一个门）的四种安全级别"，选哪档决定这个端口走什么规则：

| 本课说法（人话） | 行业标准叫法 | 典型配置 / 在哪遇到 | 代价 |
|---|---|---|---|
| 什么都不查、也不加密 | **`PLAINTEXT`** | 开发环境、迁移过渡期 | 谁都能连、明文传输 |
| 加密，且可用证书当身份 | **`SSL`** | 需要双向认证的场景 | 有加密开销 |
| 查身份，但不加密 | **`SASL_PLAINTEXT`** | 可信内网 | ⚠️ **凭据在网络上明文传输** |
| 又查身份又加密 | **`SASL_SSL`** | **生产推荐** | 最安全，开销也最大 |

> 🔴 **四件套必须成套配**：`listeners`（开哪个端口）+ `listener.security.protocol.map`（这个端口走哪套规则）+ `inter.broker.listener.name`（机器之间走哪个门）+ `advertised.listeners`（告诉客户端来连哪）。**错一个就是"连不上"或"连上了但不安全"**。
> ⚠️ **两个高频坑**：① `SASL_PLAINTEXT` + SASL/PLAIN = **密码在网络上裸奔**（要么换 SCRAM，要么上 `SASL_SSL`）；② 开 ACL 前**必须设 `super.users`**，否则连管理员自己都会被拦在门外。
...
> 📘 **[→ 打开应用实战：Kafka 安全体系](../../../应用实战/11-Kafka安全体系.md)**
>
> 课里讲的是「认证 / 授权 / 加密」三段式结论，实战篇带你把它装到一个真容器上：① 基础版——起一个带 SASL/SCRAM 的 Kafka，让客户端凭账号连进来（先解决「能不能进」）；② 综合版——补上 ACL 最小权限，亲眼看到「没被授权就建不出 topic」，体会为什么只做认证等于全放行。

...
> 📘 **[→ 打开应用实战："发送成功"之后机器挂了，消息还在吗](../../../应用实战/07-副本机制与故障转移.md)**
>
> 课里讲的是"ISR 名单怎么定"，实战篇把它**变成一场可控的火灾**：① 基础版——起 3 节点集群、建 RF=3 但 `min.insync.replicas` 保持默认 1 的主题，停掉一台看 leader 自动换人（这一轮**不丢**，因为 ISR 是满的）；② 综合版——给主题加 `min.insync.replicas=2` 熔断，再停**两台**，你会看到写入**明确抛出 `NotEnoughReplicasException`**，而没设熔断的主题照常返回成功。同样的 3 副本、同样的 `acks=all`，差别只在熔断值。
> ⚠️ 实战含"主动停容器"的演练，**只在本地练手集群执行**。

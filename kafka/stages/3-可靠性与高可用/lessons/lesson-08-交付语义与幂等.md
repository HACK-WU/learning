...
> 📌 **一句话本质**：本课做的事，是把**「东西送出去就完事」**改成**「先说清'算送到几次'，再用机制把这个承诺兑现」**。
>
> ⚖️ **处境对照**（沿用第一幕那组真实工单数字：217 位用户收到两条积分短信，其中 3 位被重复扣款）：
> - **不这么做（只管送到、不管送几次）**：超时重试会写出**两条一模一样**的消息——短信重复只是扰民，**重复扣款则是直接的钱损事故**（本例 3 位），且事后要靠人工对账才发现。
> - **这么做（先定承诺、再用幂等/事务兑现）**：同一次发送的**自动重试**在服务端被识别并丢弃，只落一条；"写输出"和"记账"绑成一笔，崩溃也不会出现"写了一半"。代价是要额外配齐机制、并接受一定性能开销。
>
> ⏳ **数字来源说明**：217 位 / 3 位是**本课场景设定的假设数字**（用于讲清道理的虚构工单），不是某个真实事故的统计。真实系统中重复的比例取决于网络抖动与重试策略；行业里常见的定性结论是"at-least-once + 业务幂等是通用解"，具体开销需自己压测。
...
![三种"说到做到"的承诺](../assets/lesson08-global-three-promises.svg)

> 看图：**从左到右是三档承诺**。① **最多一次**——放那儿就走，可能压根没办成，而且**没人会发现**；② **至少一次**——不打通就再送一趟，绝不漏办，代价是可能**办两遍**；③ **恰好一次**——办成且只办一遍，代价最高。差别在于：**"漏办"和"办两遍"你总得选一个认，除非肯付出额外代价**；而整条链路上每一段都有自己的承诺，**最终到手的那份由最松的一段决定**。

**四条硬约束**说明：① 本图只回答"本课要解决什么问题、靠什么思路"；② 图上不出现术语（交付语义、幂等、事务、PID 这些词都在下面才出场）；③ 已带读图指引；④ 图旁文字能独立说清同一件事。

### 本课地图（分几步走）

| 步 | 这一步要解决什么（人话） | 对应知识点 |
|:--:|------------------------|-----------|
| 1 | 先有一套话，能把"到底送了几次"说清楚 | 知识点 1：三种交付语义 |
| 2 | 让"同一件事重发一遍"不再变成两条 | 知识点 2：幂等 producer |
| 3 | 让"写出去"和"记账"绑成一笔，崩溃也不半途 | 知识点 3：事务简介 |

> 只回答"分几步走、现在在哪"；不写机制、不写结论；**不标学习状态**（进度以 `00-学习档案.md` 为准）。

### 知识点 1：三种交付语义

> 🧭 第 1/3 步｜承接：第一幕白板上"消息到底送达几次、丢了算谁的、重了算谁的" → 本步：先建立一套能描述这个取舍的语言
...
> 本课讲的就是"同一件事（送达几次）的三种承诺"，这三个词是分布式系统里的通用词汇，不只 Kafka 用：

| 本课说法（人话） | 行业标准叫法 | 典型配置 / 在哪遇到 | 代价 |
|---|---|---|---|
| 放那儿就走，丢了不知道 | **at-most-once（最多一次）** | 生产者 `acks=0`；消费者"先提交位移、后处理" | 最快；**漏办且无感知** |
| 不确认就再送一趟 | **at-least-once（至少一次）** | 生产者 `acks=all` + 重试（**3.0 起默认**）；消费者"先处理、后提交" | 不漏；**可能重复**，需下游去重 |
| 办成且只办一遍 | **exactly-once（恰好一次）** | 幂等 producer + 事务 + 下游 `read_committed` | **不漏不重**；配置最复杂、有性能开销 |

> 🔴 **这三个词是通用术语**（分布式系统通用，非 Kafka 专有）。落地时记住两点：**① 语义是配置出来的，不是 Kafka 自带的**；**② 端到端语义 = 链路上最弱的一环**——生产者做到恰好一次、消费者却先提交后处理，整体仍是"最多一次"。
> 📌 **译名说明**：这三个词在中文社区通用译名即为"最多一次 / 至少一次 / 恰好一次"（也有写作"至多一次"），**英文缩写 at-most-once / at-least-once / exactly-once 更常见**，检索与沟通建议直接用英文。
...
> 🧭 第 2/3 步｜承接：上一步确认了"至少一次"是默认姿态、重复是它的代价 → 本步：让"同一件事重发一遍"不再变成两条
...
> 看图：**从左到右走一遍**——生产者先去领一张工牌，然后给自己发的每一批**按顺序发号**；服务端按"工牌 + 号"记住最近几个号，于是能分三种情况处理：**下一个号**正常收、**重复的号**直接扔掉但仍回一声"好了"（免得你一直重发）、**跳号**就报错。去重的关键全在**中间那个"记住最近几个号"的小本子**上。
...
> 讲透后对标：这几个词在排障和调优时会反复出现。

- **幂等生产者 Idempotent Producer**：就是本课说的"领工牌 + 发号去重"。在哪遇到：配置项 `enable.idempotence`；**Kafka 3.0 起默认为 true**（KIP-679）。
- **PID（Producer ID）**：就是本课说的"工牌"。在哪遇到：**通常由客户端自动向服务端申领，你不用手写**；它随生产者重启而更换，这也是"跨重启防不住"的根因。
- **序列号 Sequence Number**：就是本课说的"取餐号"。在哪遇到：**每个分区独立从 0 递增**；判定的就是它。
- **`max.in.flight.requests.per.connection`（幂等下 ≤ 5）**：就是本课说的"最多几个号同时在飞"。在哪遇到：服务端缓存最近 5 个批次，超过就认不出重复了——**这是幂等强制要求该值 ≤ 5 的原因**。
- **⚠️ 三条边界（最常被误解）**：① **会话级**（重启换工牌，跨重启防不住）；② **分区级**（跨分区/跨 Topic 的原子性不管，那是事务的活）；③ **只挡内部自动重试**（应用层手动 `send()` 两次 = 两个号，照样两条）。
...
> 🧭 第 3/3 步｜承接：幂等只挡住了"重发变两条" → 本步：解决更难的那一半——"写出去"和"记账"是两个动作，崩溃时怎么不出现"写了一半"
...
> 看图：**从上往下按时间走**——先开一笔，然后写出去（**此刻下游还看不见**），接着把"记账"也塞进同一笔，最后一次性提交。**关键在第 2 步那句"此刻对下游不可见"**：写和提交之间无论哪一步崩了，都是整笔作废，绝不会留下"写了一半"的痕迹。
...
> 讲透后对标：事务落地最容易翻车的就是最后一条（下游没配 `read_committed`）。

- **事务 Transaction**：就是本课说的"把写输出和记账绑成一笔"。在哪遇到：`beginTransaction` / `commitTransaction` / `abortTransaction` 这组 API。
- **`transactional.id`（稳定身份证）**：就是本课说的"跨重启不变的身份"。在哪遇到：**必须每个实例唯一且长期稳定**；随机生成会导致围栏失效。
- **`initTransactions()` 与僵尸围栏（zombie fencing）**：就是本课说的"上岗登记、把旧人挡在门外"。在哪遇到：新实例启动会提升 epoch（世代号），**假死未死透的旧实例之后的一切请求都会被拒**。
- **`isolation.level=read_committed`**：就是本课说的"下游只读已提交的"。在哪遇到：消费者配置；**默认是 `read_uncommitted`，不显式设置则事务形同虚设**——这是落地最常见的翻车点。
- **`sendOffsetsToTransaction()`**：就是本课说的"把记账塞进同一笔"。在哪遇到：消费-处理-转发场景；**用 `commitSync()` 单独提交位移会破坏原子性**。
- **`transaction.timeout.ms`（默认 60 秒）**：就是本课说的"一笔最多开多久"。在哪遇到：超时会被协调者强制中止；**长事务会拖住同分区上 read_committed 消费者的进度**。
...
> 📘 **[→ 打开应用实战：积分发重了——同一笔订单加了两次分](../../../应用实战/08-交付语义与幂等.md)**
>
> 课里讲的是"幂等靠工牌 + 取餐号"，实战篇把它**落到业务代码里**：① 基础版——`points = points + 100` 累加式写法，同一条订单处理三次就加三次分（纯 SQLite 可复现，不需要 Kafka）；② 综合版——加一张 `processed_orders` 账单用 `order_id` 做幂等键，并把"查账单 + 加分 + 记账单"**圈进同一个事务**（避免"分加了没记账单"），再在生产者侧开 `enable_idempotence` 与 `transactional.id`。文末点破四条易错边界，包括**消费端必须配 `read_committed`** 否则事务白配。

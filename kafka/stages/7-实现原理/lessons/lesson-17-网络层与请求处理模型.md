...
> 📘 **[→ 打开应用实战：网络层与请求处理模型](../../../应用实战/17-网络层与请求处理模型.md)**
>
> 课里讲的是「Reactor NIO 三拨分工 + 队列是咽喉」，实战篇带你把吞吐压出来再看仪表：① 基础版——带宽没跑满就以为路不够宽，准备加机器加线程，但没看过一眼队列；② 综合版——先打流量，再按「队列 → 空闲比 → 耗时」的顺序定位瓶颈在不在网络层，并连续采样 5 次亲手拆穿那个"看着像空闲比例、其实是累计数"的陷阱指标。

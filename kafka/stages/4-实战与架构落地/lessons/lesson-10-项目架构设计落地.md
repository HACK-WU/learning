...
> 📌 **一句话本质**：本课做的事，是把**「谁要数据，你就为谁单独接一根线、改一次代码」**改成**「把'已经发生了什么'往一个公共地方一贴，谁需要谁自己来看」**。
>
> ⚖️ **处境对照**（沿用第一幕的场景：四个团队排着队来找订单团队要数据）：
> - **不这么做（逐个对接）**：第 5 个下游接入时，订单服务要**再写一份对接代码、再改一次发布**；而且它必须知道"这几个下游分别是谁"——耦合随下游数量线性增长。
> - **这么做（广播事实）**：第 5 个团队**自己起一个组订阅就完事**，订单服务零改动、甚至不需要知道有新人来了。代价是：往里放的东西必须是"已发生的事实"，不能是"指定某人去干活的命令"——否则耦合会悄悄长回来。
>
> ⏳ **数字来源说明**：以上为**定性对照**（按"加一个下游要不要改上游"比较），不是实测数据。真实收益随下游数量增长而放大；行业里常见的定性结论是"发布-订阅让新增消费者零成本"，具体成本需按自己的协作流程估算。
...
![逐个通知 vs 贴告示广播](../assets/lesson10-global-notify-vs-broadcast.svg)

> 看图：**左边**是现在的做法——来一个要用数据的人，出事的地方就**多接一根线**，而且它必须知道"这几个人都是谁"；第 4 个人（红色虚线）一来，又得改一次。**右边**是本课的做法——往告示栏**贴一条就完事**，贴的人**完全不知道有谁在看**，新来的人自己来看就行（绿色那个）。差别不在工具，而在**你往里放的是"命令"还是"事实"**：找人办事（命令）就必须知道找谁；把事说出来（事实）则谁关心谁自己来。

**四条硬约束**说明：① 本图只回答"本课要解决什么问题、靠什么思路"；② 图上不出现术语（EDA、事件、topic、消费者组这些词都在下面才出场）；③ 已带读图指引；④ 图旁文字能独立说清同一件事。

### 本课地图（分几步走）

| 步 | 这一步要解决什么（人话） | 对应知识点 |
|:--:|------------------------|-----------|
| 1 | 先立起"广播事实而非下达命令"这个核心念头 | 知识点 1：事件驱动架构 EDA |
| 2 | 看看别人在这条主干上都画出了哪几种图 | 知识点 2：常见拓扑 |
| 3 | 拿到清单，敢对不合适的场景说"不" | 知识点 3：决策清单 |

> 只回答"分几步走、现在在哪"；不写机制、不写结论；**不标学习状态**（进度以 `00-学习档案.md` 为准）。

### 知识点 1：事件驱动架构 EDA

> 🧭 第 1/3 步｜承接：第一幕里"四个团队排着队来要数据、订单团队疲于应付" → 本步：换一种通知方式，让新来的人不用打招呼
...
> 看图：**左边只有一个人往里发**，**右边三拨人各自取**——它们**互不认识、互不等候**，而且都从同一份数据里取，没有复制出三份。发的人完全不知道右面有谁。
...
> 讲透后对标：这几个词是架构评审与方案文档的高频词。

- **事件驱动架构 EDA（Event-Driven Architecture）**：就是本课说的"广播事实"。在哪遇到：架构文档、技术选型评审；**2003 年由 Gartner 分析师 Roy Schulte 提出**（核查于 2026-08），比 Kafka 早八年。
- **事件 Event vs 命令 Command**：就是本课说的"已发生的事实" vs "去找某人干活"。在哪遇到：事件命名——`OrderCreated`（事件，过去式）✅ vs `SendEmail`（命令，祈使句）❌；**把命令塞进事件流，耦合会悄悄长回来**。
- **事件主干 Event Backbone**：就是本课说的"那条公共管道"。在哪遇到：企业架构图里居中的那个组件；Kafka 是当前最主流的实现。
- **三重解耦（空间 / 同步 / 时间）**：就是本课说的"不知道是谁 / 不等它处理完 / 它积压也不影响我"。在哪遇到：讲 EDA 价值的文档；分别对应课 1 的耦合、雪崩、积压三大痛点。
- **发布-订阅 Pub/Sub**：就是本课说的"贴一条、各自来看"。在哪遇到：课 2 对比 MQ 时的核心差异点；组内分摊、组间广播（课 6）。
...
> 🧭 第 2/3 步｜承接：上一步立起了"广播事实"这个念头 → 本步：看看别人在这条主干上，都画出了哪几种图
...
> 本课讲的是"同一条主干上的四种用法"，这是架构图中最常出现的四种形态：

| 本课说法（人话） | 行业标准叫法 | 典型配置 / 在哪遇到 | 代价 |
|---|---|---|---|
| 只管把货从 A 搬到 B | **日志管道 / Kafka Connect** | 装 source / sink 连接器，配几行 JSON；**Kafka 0.9（2015-11）引入** | 最稳、最省心；不擅长做计算 |
| 边跑边算 | **流处理 / Kafka Streams、Flink** | Streams 是**库**（嵌在应用里，0.10 引入）；Flink 是独立引擎 | 主流；有状态计算的心智负担更重 |
| 进货出货分两条道 | **CQRS（命令查询职责分离）** | Greg Young 2010 年提出 | **重武器**：简单 CRUD 硬上多半是挖坑 |
| 只记每一脚油门、要查就回放 | **事件溯源 Event Sourcing** | Martin Fowler 2005 年成文；配 **log compaction** | **重武器**：查询侧要另建"读模型" |

> 🔴 **火力提示**：前两种（管道、流处理）是日常，能吃掉大多数需求；后两种（CQRS、事件溯源）是**重武器**——简单 CRUD 硬上，换来的多半是运维与一致性的双重负担。
> 📌 **`__consumer_offsets` 就是 log compaction 的现成例子**（课 6 提过）：每组只留最新一条打卡记录。
...
> 🧭 第 3/3 步｜承接：前两步学会了"怎么设计" → 本步：拿到清单，敢对不合适的场景说"不"（CTO 的第二个问题）
...
> 本课讲的是"该不该用"，下面是"该用 / 不该用"两侧的标准说法与替代方案：

| 本课说法（人话） | 行业标准叫法 / 替代方案 | 在哪遇到 |
|---|---|---|
| 高吞吐事件流、一份数据多方消费、削峰、要回放、要流处理 | ✅ **该用的五个信号** | 选型评审的"命中项" |
| 用户点了要立刻等结果 | ❌ **低延迟请求-响应** → 用 RPC / HTTP | 下单、登录换 token 这类交互 |
| 量小还要按规则分门别类 | ❌ **小流量 + 复杂路由** → 用 RabbitMQ（exchange/binding） | 工单、审批流 |
| A 扣款 B 入账必须同时成功 | ❌ **强一致跨系统事务** → 用同步事务 / 最终一致 + 对账 | 转账核心链路 |
| 一条条待办任务、要单条确认 | ❌ **纯任务队列**（历史上是短板）→ 传统 MQ；**2026 年起 share groups（KIP-932）在 4.2 GA** 才补上 | 任务分发、失败单条重投 |
| 小团队简单场景 | ❌ **运维成本远超收益** → 云托管 / 轻量 MQ | 3 人团队一个单体应用 |
| 按业务事件建 topic（不按消费方建） | ✅ **Topic 设计三原则** | `orders` ✅ vs `to-points-service` ❌ |

> 🔴 **最容易踩的坑**：按消费方建 topic（`to-points-service`）——耦合从代码**转移到了管道上**，每加一个下游仍要生产者改代码，EDA 白搭。
...
> 📘 **[→ 打开应用实战：第四个团队来要数据，订单服务又得改一次代码](../../../应用实战/10-项目架构设计落地.md)**
>
> 课里讲的是"三重解耦"这个概念，实战篇把它**变成一组可对比的实测数字**：① 基础版——订单服务挨个调用积分/风控/数据三个下游，下单接口耗时 **1.05 秒**，且风控一超时整个下单就失败；② 综合版——改成只往 `orders` 广播一条 `OrderCreated`，三个下游用各自 `group.id` 独立订阅，下单接口降到 **0.012 秒**，三个下游交错处理、互不等待。文末点破三条最容易走偏的地方，包括"**把命令塞进事件流，耦合会悄悄长回来**"。
...
```
我的 Kafka 课程已全部学完（kafka/，10 课全部完成），

...
> 📌 **一句话本质**：本课做的事，是把**「靠人坐在终端前一行行敲」**改成**「写成一个自己会跑、会判断、会记住干到哪了的程序」**。
>
> ⚖️ **处境对照**（沿用第一幕的场景：积分服务要 7x24 小时处理订单）：
> - **不这么做（靠人敲命令）**：一次只能处理一条，**人不在就停、人一走就断**；没法加业务判断（要不要加积分、加多少），出错也不会自己重试。**7x24 小时意味着要有人 7x24 小时坐在终端前**——做不到。
> - **这么做（写成程序）**：写好放着就昼夜不停地自己干；能加业务判断、出错能兜底重试、崩溃重启后**从记住的位置继续**。代价是要处理"程序自己的坑"（异步发送、提交时机、优雅退出）。
>
> ⏳ **数字来源说明**：以上为**定性对照**（按"是否需要人在场、能否加判断"比较），不是实测数据。**关键点在于：换的是"谁来干"，规矩本身一条没变**——分区、位移、事务这些机制在代码里依然成立，变的只是 API 拼写。
...
![人肉敲命令 vs 写成程序](../assets/lesson09-global-manual-vs-program.svg)

> 看图：**左边**是现在的做法——一个人坐在黑框前敲一行、出一条，人不在就停、人一走就断，也没法加"要不要处理、怎么处理"的判断；**右边**是本课的做法——写成一个程序放着，它自己取、自己判断、出错自己重试、还记住干到哪了。差别在于：**换的是"谁来干"，不是"规矩变了"**——右边那个蓝框和左边那个蓝框是同一套规矩，所以前面学的机制在代码里一条都不会变。

**四条硬约束**说明：① 本图只回答"本课要解决什么问题、靠什么思路"；② 图上不出现术语（生产者、消费者、客户端、commit 这些词都在下面才出场）；③ 已带读图指引；④ 图旁文字能独立说清同一件事。

### 本课地图（分几步走）

| 步 | 这一步要解决什么（人话） | 对应知识点 |
|:--:|------------------------|-----------|
| 1 | 先挑顺手的工具，把环境接通 | 知识点 1：选客户端与环境 |
| 2 | 写出"发货"那一半，并保证它真的发出去了 | 知识点 2：写生产者 |
| 3 | 写出"收货"那一半，并拿捏好"什么时候记进度" | 知识点 3：写消费者 |

> 只回答"分几步走、现在在哪"；不写机制、不写结论；**不标学习状态**（进度以 `00-学习档案.md` 为准）。

### 知识点 1：选客户端与环境

> 🧭 第 1/3 步｜承接：第一幕里"要写两个真正的程序" → 本步：先决定用什么工具写、并把环境接通
...
> 本课讲的是"同一件事（写程序连上它）的三种选法"：

| 本课说法（人话） | 行业标准叫法 | 典型配置 / 在哪遇到 | 代价 |
|---|---|---|---|
| 官方亲儿子（地铁） | **Java 客户端 `kafka-clients`** | Maven 依赖 `org.apache.kafka:kafka-clients`；`KafkaProducer` / `KafkaConsumer` | 要背 JVM + 构建工具；**新特性最快** |
| C 内核跑车 | **confluent-kafka（librdkafka）** | `pip install confluent-kafka`；`from confluent_kafka import Producer` | 预编译二进制，需匹配平台；**性能贴近 Java** |
| 零门槛自行车 | **kafka-python** | `pip install kafka-python`；`from kafka import KafkaProducer` | 纯 Python，**中小吞吐够用**；新特性跟进略慢 |

> 🔴 **选型口诀**：学得动、装得上 → kafka-python；Python 生产要高吞吐 → confluent-kafka；企业核心链路、要 Streams/Connect 全家桶 → Java。
> 📌 **过时信息提醒**：旧帖子里"kafka-python 停更了别用"说的是 **2020-09～2025-02** 那段时间；2025 年初复活后已迭代到 3.0.x，事务（2.2.0，2025-04）与幂等齐备（核查于 2026-08）。
> 💡 **机制通用、API 是方言**：三种客户端走**同一套公开网络协议**，课 3–课 8 的机制对所有客户端都成立，变的只是 API 拼写。
...
> 🧭 第 2/3 步｜承接：上一步挑好了工具、环境已接通 → 本步：写出"发货"这一半，并保证消息真的离开了你的电脑
...
> 看图：**上半条是去的路**——打包 → 按名字选落到哪条 → 在**本地**攒一批 → 后台线程发出去并等确认；**下面那条虚线回来的箭头**是回执，写着"落在哪条、第几位"。**注意第 3 格标着"本地"**：消息在这时还没离开你的电脑——这就是"send 完直接退出会丢消息"的根因。
...
> 讲透后对标：生产者 API 就这几个动作，但每个都对应前面某一课的机制。

- **`send()`（异步发送）**：就是本课说的"把包裹交给代收点"。在哪遇到：返回的是 **Future**（一张将来才能兑现的凭证）；**返回 ≠ 已送达**。
- **`future.get(timeout=…)`（同步等回执）**：就是本课说的"等签收单"。在哪遇到：回执对象含 `topic` / `partition` / `offset`；**每条都 get 会拖垮吞吐**，演示可以、生产慎用。
- **回调 `add_callback` / `add_errback`**：就是本课说的"送到了/失败了通知我一声"。在哪遇到：高吞吐场景的标准写法，不阻塞主线程。
- **`flush()` / `close()`**：就是本课说的"催代收点立刻发车 + 关门"。在哪遇到：**退出前不做这两步，缓冲区里未发出的消息全部蒸发**——新手"我发了但消费不到"的头号原因。
- **`enable_idempotence`（kafka-python 3.0 起默认 True）**：就是课 8 的"工牌 + 取餐号"。在哪遇到：**2.x 老版本默认不是它**，升级/降级时务必显式写。
...
> 🧭 第 3/3 步｜承接：上一步写出了"发货"的一半 → 本步：写出"收货"这一半，并把课 6 那个"什么时候记进度"的坑在代码里拿捏对
...
> 看图：**按从上到下的顺序走一圈**——取一批 → 交一条给业务 → **业务处理完** → 才写进度。**看最后那句注释**：崩溃发生在"处理完、提交前"时，重启会把这一条重读一遍（重复），而不会跳过它（丢失）。这就是"先处理后提交"换来的结果。

再补一条工程红线：`KafkaProducer` 线程安全可全局共享，**`KafkaConsumer` 非线程安全**（官方文档明确警告）——一个进程一个消费者实例，别在多线程里共用。

#### 🗣️ 行话对照

> 讲透后对标：消费者这几个配置项，每一个都是前面某课机制的代码落点。

- **`group_id`（消费者组身份）**：就是课 6 的"一伙人"。在哪遇到：同组多实例自动分摊；**换个 group_id 就是一套全新进度**（测试时想从头再消费，换组最省事）。
- **`auto_offset_reset`（无进度时从哪开始）**：就是课 6 的"找不到书签怎么办"。在哪遇到：默认 `latest`；**本地实验建议显式 `earliest`**，否则新组容易"什么也收不到"。
- **`enable_auto_commit=False` + 手动 `commit()`**：就是课 6 的"什么时候记进度"。在哪遇到：**自动提交记的是"轮询到的位置"而非"处理完的位置"，处理中途崩溃会丢消息**。
- **`isolation_level='read_committed'`**：就是课 8 的"只读已提交的"。在哪遇到：事务场景；**默认是 `read_uncommitted`，不显式设置则事务形同虚设**。
- **`consumer.close()`（优雅退出）**：就是本课说的"体面地走"。在哪遇到：主动发 LeaveGroup 让同组立刻接手；**不 close 直接杀进程，分区最长闲置 `session.timeout.ms`（默认 45 秒）**。
...
> 📘 **[→ 打开应用实战：demo 跑通了，但没人敢放到服务器上](../../../应用实战/09-代码开发实战.md)**
>
> 课里的 `code/` 演示的是"happy path 怎么跑通"，实战篇带你补上**出事时的那三条岔路**：① 基础版——发得出去收得回来就交给运气，人为注入一条坏消息，看基础版消费者如何**直接中断循环**；② 综合版——每条发送挂回执（失败可统计）、单条处理包 try/except（坏消息跳过不停摆）、捕获 `SIGTERM`/`SIGINT` 并在 `finally` 里 `flush()` + `commitSync()` + `close()`。实测对比：坏消息不再导致卡死循环，`Ctrl+C` 后打印统计数字正常退出。

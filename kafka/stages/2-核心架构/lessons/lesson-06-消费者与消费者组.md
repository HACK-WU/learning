...
> 📌 **一句话本质**：本课做的事，是把**「东西由发货方一股脑塞给你」**改成**「你自己按自己的节奏来取，并自己记住取到哪了」**。
>
> ⚖️ **处境对照**（沿用第二幕那组假设数字：积分服务 1 秒能处理 1 万条，短信服务 1 秒只能发 100 条）：
> - **不这么做（由对方推给你）**：快慢两个收货方被**同一速率**硬塞——慢的那个（100 条/秒）会被活活撑死、消息堆在内存里直到溢出；快的那个却在等米下锅。**速率不由自己掌控**。
> - **这么做（自己去取）**：各按各的节奏，慢的就让东西在柜子里多放一会儿（表现为"积压"），快的就多取几趟；**谁也不会被撑死**。
>
> ⏳ **数字来源说明**：以上为**定性对照 + 假设速率推演**（1 万 vs 100 条/秒是讲清道理的假设值），不是真实压测。真实差距取决于业务处理耗时与实例数；行业里常见的定性结论是"拉模式让消费速率由消费者掌控"，"积压（lag）"是它的可观测副产物。
...
![一伙人分工 vs 几伙人各拿各的](../assets/lesson06-global-one-team-vs-multi-team.svg)

> 看图：**左边**是"一伙人"——3 个组员各盯一条传送带，**一条带只归一个人**，所以同一件事不会被干两遍；人比带多时，多出来的人只能闲着。**右边**是"几伙人"——3 个组都要把 3 条带各看一遍，**东西只有一份，没有复制出三份**，各记各的进度、互不打扰。这两张图回答的是两个不同的问题：怕重复 → 看左边；要各拿一份 → 看右边。

**四条硬约束**说明：① 本图只回答"本课要解决什么问题、靠什么思路"；② 图上不出现术语（消费者组、分区、offset、rebalance 这些词都在下面才出场）；③ 已带读图指引；④ 图旁文字能独立说清同一件事。

### 本课地图（分几步走）

| 步 | 这一步要解决什么（人话） | 对应知识点 |
|:--:|------------------------|-----------|
| 1 | 先说清"谁去取、取到哪了"这个进度是怎么记的 | 知识点 1：消费模型与位移 offset |
| 2 | 说清一伙人怎么分活、人变了怎么重新分 | 知识点 2：消费者组与再均衡 |
| 3 | 说清"什么时候记进度"会带来重复还是丢失 | 知识点 3：位移提交与重复/丢失 |

> 只回答"分几步走、现在在哪"；不写机制、不写结论；**不标学习状态**（进度以 `00-学习档案.md` 为准）。

### 知识点 1：消费模型与位移 offset

> 🧭 第 1/3 步｜承接：第一幕里"消息是推给我的还是我去取、怎么知道读到哪了" → 本步：说清取的模式，以及"读到第几条"这个进度到底是什么
...
> 看图：**这是一个不停转的圈**——取一批 → 处理 → 记进度 → 再取一批。**关键看第 3 格**：记的是"我处理到第 N 条"。**这个"记"发生在什么时候**，直接决定了后面是"重复"还是"丢失"，那是知识点 3 的主战场。
...
> 讲透后对标：这三个词是消费侧监控与排障的第一组术语。

- **拉模式（pull / long polling）**：就是本课说的"自己去柜子里取"。在哪遇到：所有讲 Kafka 消费模型的文档；对比项是推模式（push）。**好处是消费速率由消费者掌控**。
- **位移 Offset**：就是本课说的"行号 / 书签"。在哪遇到：`--describe` 的 `CURRENT-OFFSET`；**只在单个分区内有效**，分区之间互不相干。
- **积压 Lag**：就是本课说的"柜子里还有多少件没人取"。在哪遇到：`kafka-consumer-groups.sh --describe` 的 `LAG` 列；**这是消费侧最重要的一个监控数**，告警通常直接配在它上面。
- **`max.poll.records`（默认 500）**：就是本课说的"一次最多取多少件"。在哪遇到：消费者配置；调大能提吞吐，但会让"一批处理不完"的风险变大。
...
> 🧭 第 2/3 步｜承接：上一步知道了"书签跟着读者走" → 本步：说清一伙人怎么分活，以及人多了、人少了、带子多了时怎么重新分
...
> 讲透后对标：消费者组是"一份数据多方消费"的实现方式，这几个词在现场天天见。

- **消费者组 Consumer Group**：就是本课说的"一伙人"。在哪遇到：配置项 `group.id`；`kafka-consumer-groups.sh` 管理它。
- **再均衡 Rebalance**：就是本课说的"重新分活"。在哪遇到：日志里的 `rebalance`、监控里的 rebalance 速率；**频繁再均衡 = 消费抖动**。
- **`session.timeout.ms`（默认 45000ms）**：就是本课说的"多久没心跳算人没了"。在哪遇到：消费者配置；调小能更快发现故障，但**误判率上升**。
- **`max.poll.interval.ms`（默认 5 分钟）**：就是本课说的"两趟取货之间最多隔多久"。在哪遇到：**生产环境"莫名其妙再均衡"的头号原因**——一批消息处理太久就会被踢出组。
- **`group.protocol=consumer`（KIP-848 新协议）**：就是本课说的"只调整受影响的人、不再全组踩刹车"。在哪遇到：Kafka 4.0+ 客户端配置；**存量应用要显式设才生效**（核查于 2026-08）。
- **并行度上限 = 分区数**：就是本课说的"人比带多就只能闲着"。在哪遇到：容量规划；**想更快先加分区，不是无脑加实例**。
...
> 🧭 第 3/3 步｜承接：上一步说清了"一伙人怎么分活" → 本步：回到那个最微妙的问题——"什么时候记进度"，记早了记晚了各有什么后果
...
> 本课讲的是"同一件事（记进度）的几种时机"，选哪个就决定了你会遇到"重复"还是"丢失"：

| 本课说法（人话） | 行业标准叫法 | 典型配置 / 在哪遇到 | 代价 |
|---|---|---|---|
| 机器定期替你打卡（按"拉到哪"记） | **自动提交 Auto Commit** | `enable.auto.commit=true`（**默认**）+ `auto.commit.interval.ms=5000` | 最省心；**有丢失窗口**——拉取跑到处理前面时会跳过未处理的消息 |
| 干完一批再打卡（按"处理到哪"记） | **手动同步提交 `commitSync()`** | `enable.auto.commit=false` + 处理完后 `commitSync()` | 不丢；**有重复窗口**（崩溃时重干一遍），每批多一次往返 |
| 干完一批、不等回执就打卡 | **手动异步提交 `commitAsync()`** | `enable.auto.commit=false` + `commitAsync()` | 更快；**失败不重试**，需配合回调处理异常 |
| 找不到书签时从哪开始 | **`auto.offset.reset`**（`latest` 默认 / `earliest` / `none`） | 消费者配置 | 用默认 `latest` 会让**新组静默跳过全部历史消息** |

> 🔴 **工程上的选择几乎一边倒**：手动、处理完再提交。**因为重复能兜底（业务幂等），丢失兜不回来**——漏检一笔欺诈订单是事故，同一笔检查两遍只是多耗一点算力。
...
> 📘 **[→ 打开应用实战：风控服务崩一次就漏检一批](../../../应用实战/06-消费者与消费者组.md)**
>
> 课里讲的是"自动 vs 手动提交"的道理，实战篇把它**变成可复现的现场**：① 基础版——默认自动提交，人为制造"处理到一半崩溃"，用 `kafka-consumer-groups.sh --describe` 取证 **LAG 归零但中间几条从未处理**（丢了也没报错）；② 综合版——关掉自动提交、处理完再 `commitSync()`、再均衡前交接进度，重跑后看到的是**那一批被重做一遍**（重复而非丢失）。文末点破 `max.poll.interval.ms` 这个"莫名再均衡"头号杀手。

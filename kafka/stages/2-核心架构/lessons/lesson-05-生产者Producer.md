...
> 📘 **[→ 打开应用实战：下单服务每秒发 1000 条怎么发](../../../应用实战/05-生产者Producer.md)**
>
> 课里讲的是"三个参数各自什么意思"，实战篇把它们**放进同一个演进里**：① 基础版——每条 `send()` + `flush()`、不带 key，用 CLI 亲眼看同 `user_id` 散在不同分区（乱序），且丢了也不知情；② 综合版——带上 `key=user_id` 保序、`acks=all` + 错误回执保不丢、放开 `linger_ms`/`batch_size` 攒批提吞吐，并配 `enable_idempotence` 让重试不产生重复。文末点破两个易错组合（`acks=all` 单配是假安全、加分区会打破历史分布）。

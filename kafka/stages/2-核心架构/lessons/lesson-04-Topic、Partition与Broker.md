...
> 📘 **[→ 打开应用实战：一本总账 vs 几本分册](../../../应用实战/04-Topic、Partition与Broker.md)**
>
> 课里讲的是道理，实战篇带你**亲手跑出数字**：① 用纯 Python 量化"分区数 = 并行上限"（1/3/6 分区对比）；② 亲眼看到"分区内有序、跨分区乱序"，以及怎么用相同 key 保序；③ 实测顺序写与零拷贝——**包括两个反直觉结果**（本机 SSD 上随机写反而更快、零拷贝在回环网卡上测不出优势），并解释为什么；④ 在真集群上看分区分布。
> ①~③ **不需要 Kafka**，纯 Python 即可。

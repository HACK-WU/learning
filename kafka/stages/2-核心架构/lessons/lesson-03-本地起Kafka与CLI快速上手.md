...
> 📘 **[→ 打开应用实战：我要一个"练手专用"的 Kafka](../../../应用实战/03-本地起Kafka与CLI快速上手.md)**
>
> 课里讲的是"一条命令能起来"，实战篇带你走完**从一次性到可复用**的两步：① 基础版——一条 `docker run` 起起来、建 topic、发一条收一条，同时看清它的四个坑（数据在容器内、集群 ID 易漂移、单节点无副本、长命令每次重敲）；② 综合版——用 `compose.yml` 固化配置 + 卷挂载把数据留在宿主机，并**实测 `down` → `up` 之后消息仍在**，顺带加上两个防呆（`AUTO_CREATE_TOPICS=false`、`healthcheck`）。

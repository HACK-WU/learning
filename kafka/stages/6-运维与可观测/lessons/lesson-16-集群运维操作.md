...
> 📘 **[→ 打开应用实战：集群运维操作](../../../应用实战/16-集群运维操作.md)**
>
> 课里讲的是「三板斧 + 限流两坑 + 优先副本」，实战篇带你把搬迁全流程走一遍：① 基础版——加完机器干等（新 broker 一个分区都分不到）或凭感觉硬搬（挤爆生产流量 / 限太狠搬不完）；② 综合版——`--generate` 出方案并保存回滚信息 → `--execute --throttle` 带限速执行 → `--verify` 复查并**清除限流** → 优先副本选举把 leader 交还。
...
并用 10-场景解法库/INDEX.md 的开放设计题检验我的掌握程度。
```
...
➡️ **下一课**：阶段 6 收官 → [课程手册](../../../final-课程手册.md) · [场景解法库](../../../10-场景解法库/INDEX.md)

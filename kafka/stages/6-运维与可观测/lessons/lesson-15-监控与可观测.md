...
> 📘 **[→ 打开应用实战：监控与可观测](../../../应用实战/15-监控与可观测.md)**
>
> 课里讲的是「指标从哪来、该盯哪几格、lag 怎么算」，实战篇带你把故障造出来看指标动：① 基础版——一千多个读数全在跳、全绿，但没一格标过"越界就叫人"，关键格变了色也没人知道；② 综合版——圈出该叫人的少数几格并逐格写清正常值（含最易写错的"恰好一个为 1"），再用 CLI 把"写到哪 − 读到哪"这个 lag 亲手算出来。

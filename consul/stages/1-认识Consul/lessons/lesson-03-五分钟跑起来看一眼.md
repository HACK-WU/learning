...
![一眼全局图：先让它跑起来，看看它到底在记什么](../assets/lesson-03-global-first-run.svg)

> **看图**：三步——先敲一行命令让它起来（关掉就干净消失），再报个到（它记住"名字 → 地址"），最后让它自己盯着（挂了就从名单上划掉）。走完你就知道：**它记的是名字和地址的对应关系，而且会自己更新。**
...
- *"dev 模式这么方便，小项目直接上生产"* —— 明确禁止：数据纯内存（重启全丢）、单节点无冗余、无鉴权。生产形态的最小拓扑是 3 台 Server（课 2 的"总部账本"），课 11 算账。
...
> 🎯 **选型视角**：把"五分钟上手"当作选型的真实维度——Consul（单二进制、零依赖、自带 UI）与 ZooKeeper（先装 JDK、改 zoo.cfg、配 myid）的上手成本差距是结构性的。但样板间体验好 ≠ 生产便宜，后者课 11 用账本说话。
...
flowchart TD
    ...
    K2["知识点 2 注册与查询<br/>登记处与查号台"] --> NEXT4b["课 4 查询接口机制<br/>课 10 接口对比矩阵"]

...
![一眼全局图：同一件事的两种做法——地址写死，还是先问一声](../assets/lesson-01-global-service-addressing.svg)

> **看图**：上面是把地址写死在本子上——那台机器停了就打不通；下面是先问一声"库存现在在哪"——停掉一台，下次问就自动绕开。差别在于：**换多少台机器，问的人都不用改。**
...
- *"上了 K8s 是不是就没这个问题了"* ——问题没有消失，是被 Kubernetes 内化了（它自带了一套服务发现）。你的系统里若还有 K8s 之外的 VM、外部服务，问题立刻回来。这个判断将在课 12 的决策框架中正式展开。
...
哪个更疼，取决于业务：**电商大促中"打到坏实例导致下单失败"更疼，还是"两分钟查不到新实例"更疼？** 这没有标准答案——它正是 Consul（CP 系）与 Eureka（AP 系）的分野，也是课 5、课 10 的主线之一。

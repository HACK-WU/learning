...
疑点 1 需要拆开它的**能力与身材**（知识点 1–3）；疑点 2、3 是同一件事的两面——**一份"婚前协议"要逐条读**（知识点 4，深度拆解在课 11）。

## 第三幕：层层揭示

![一眼全局图：一个工具，还是一组能力](../assets/lesson-02-global-capability-panorama.svg)

> **看图**：左边是三个只干一件事的工具，要装三回、学三套；右边是一个东西，问路、体检、存配置、门禁、加密通话都在里面。差别在于：**一件事顺手就都带上了，还是三件事各装各的。**
...
> 🎯 **选型视角**：把拓扑直接换算成成本——3 台 Server（要养机器/实例+监控+备份+升级）、每业务节点一个 agent（发布流程要纳入）、DC 数量 = Server 组数。**"用 Consul"的真实运维账单从这里开始算**，课 11 算总账。
...
- *"这事跟我无关"* —— 换证与收购至少影响三件事：法务入库流程（不再是 OSI 开源）、部分开源镜像源停止分发 BUSL 版本、以及**退出成本**（若哪天要换掉它，看课 11）。选型者必须把它当作成本项入账。

**一句话记住**：身份证换了（BUSL），日子照过（内部用无碍），禁区只有一条（别拿它跟 HashiCorp 抢生意），每版 4 年自动转正。

> 🎯 **选型视角**：立即把三件事写进决策清单：①你的用法是否踩"竞争性提供"红线（99% 的内部场景不踩）；②公司开源软件入库政策是否接受 source-available；③1.21 LTS 是 1.x 终点，新部署直接从 2.0.x 起步。课 11 再算退出成本的总账。
...
flowchart TD
    ...
    K4["知识点 4 版本与许可证<br/>BUSL 快照"] --> NEXT10["课 11 许可证成本与风险<br/>(退出成本总账)"]

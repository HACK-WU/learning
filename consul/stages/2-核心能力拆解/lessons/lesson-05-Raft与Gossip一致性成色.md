...
![一眼全局图：同一件事，两拨人用两种办法传话](../assets/lesson-05-global-raft-gossip.svg)

> **看图**：要紧的事（改名单）走上面——一个管事的加两个跟班，过半点头才算数，慢但不会记错；不要紧的事（发现有人倒了）走下面——一个传一个、一传十，快但可能晚半拍。所以"问它"也有三种问法，但**别把"快"当成"保险"**：第三种图的是换班那几秒不断线，不是灾备。
...
### 4.2 应用实战：把三种读模式放到"真出事的那一刻"去检验

> 🎯 **练一练**：上面这套命令证明的是"机制确实如此工作"。但如果老周追问一句——**"集群正在换 leader 的那几秒，我的服务读到的到底是什么？"**——命令清单答不了：它需要你起三节点、写数据、掐着表强杀 leader、逐秒采样三种模式的返回值。这就是实战篇 A 干的事。
>
> 👉 [实战篇 A：一致性读模式实测](../../../practices/实战A-读模式实测/README.md)｜全索引见 [`应用实战/INDEX.md`](../../../应用实战/INDEX.md)
...
- 🎯 **练一练**：[实战篇 A：一致性读模式实测](../../../practices/实战A-读模式实测/README.md)（三节点强杀 leader，量化 9.5 秒不可用窗口）｜[全部应用实战](../../../应用实战/INDEX.md)

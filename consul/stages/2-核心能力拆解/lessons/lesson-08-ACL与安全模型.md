...
![一眼全局图：门装上了，可谁都能进——因为默认就是"不设防"](../assets/lesson-08-global-acl-model.svg)

> **看图**：进门三步——先亮证件（只是一张卡，自己不带权限），再看证件上的说明（真正管用的是这层），最后才具体到哪一格。右下角是最该怕的事：**问"有没有这个东西"它回"没有"，实际是"你不配看"**——你以为还没配，就这么一直用旧的跑下去。所以发完证件别急着走，拿它真去试一次。
...
### 4.2 应用实战：给三个角色设计最小权限，并证明越权确实被拒

> 🎯 **练一练**：本课的验证 4 让你"撞"了一次权限，知道 403 长什么样。但真实生产不是三个 curl，而是一张**权限表**：web 能读什么、api 能写什么、ops 能看什么，每一行都要有人签字，且**签完还得证明它真的生效**——因为本课反复强调的那句话在这里最要命：**ACL 的拒绝是静默的**，`200 []` 和 `404` 都可能是"你没权限"。实战篇 C 用 21 项权限矩阵逐条验证，并在过程中抓出两个静默失效的坑（递归读越权返回 404、`operator` 写成 `_prefix` 不报错但不生效）。
>
> 👉 [实战篇 C：ACL 生产权限模型](../../../practices/实战C-ACL生产权限模型/README.md)｜全索引见 [`应用实战/INDEX.md`](../../../应用实战/INDEX.md)
...
- 🎯 **练一练**：[实战篇 C：ACL 生产权限模型](../../../practices/实战C-ACL生产权限模型/README.md)（21 项权限矩阵；`operator` 静默失效坑）｜[全部应用实战](../../../应用实战/INDEX.md)

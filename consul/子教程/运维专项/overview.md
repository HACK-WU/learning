...
**与「子教程 · py-consul 使用」的分工**：本教程讲**怎么把 Consul 集群养住**（服务端 / 集群视角：部署、健康、备份、升级），[py-consul 使用](../py-consul使用/overview.md)讲**怎么写 Python 代码操作它**（客户端 / 应用视角：KV、注册发现、锁、生产化）。两者共用主线阶段 2 作前置，互不重叠。

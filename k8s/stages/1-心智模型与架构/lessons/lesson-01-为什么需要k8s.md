...
> 📖 结论已按官方文档核对（核查于 2026-09）：Kubernetes 官方「十年回顾」与 Google Open Source 项目页（起源与时间点）。课内冲突处已在正文标注与旧说法的差异。
...
> 💡 **一句话本质**（本课把什么从 A 变成 B）：
> 本课把你的服务**从"在某个盒子里跑起来了"，变成"不管哪台机器出事，它都还在跑"**——你只说"我要它一直有几个"，不再管它们摆在哪。

> ⚖️ **处境对照**（不这么做 vs 这么做）：
>
> | | 不这么做（自己盯） | 这么做（交给编排系统） |
> |---|---|---|
> | 机器宕机 | 上面的服务**直到你发现前一直是断的**——半夜爬起来手动搬 | 系统检测到少了，自动在别的机器上补一个 |
> | 扩到 3 个实例 | 手动在每台机器起容器，**并自己记住哪台跑着什么** | 改一个数字，系统自己找地方摆 |
> | 别人怎么找到服务 | 容器 IP 每次重建都变，得手动改调用方配置 | 给一个固定门牌，背后换多少次都不影响 |
> | 你的角色 | **你要知道每个盒子在哪、还活着吗** | 你只说"要什么"，系统负责"怎么摆" |
> | 规模代价 | 机器从 4 台到 40 台，你的工作量**同比例增长** | 说的是同一句话，机器多少都一样 |
>
> ⏳ 定性对比说明：上表是**机制层面的对照**（谁负责什么、随规模怎么变），不是压测数字。真实收益随集群规模与故障频率变化，此处不给具体百分比以免误导。
...
进入细节之前，先建立整体图景 —— 本课要解决的就一件事：**盒子从"你盯着"变成"系统盯着"**。

![课1 一眼全局图](../assets/lesson-01-为什么需要编排.svg)

**看图指引**：左边是你自己盯四台机器的日子（机器 3 瘫了，上面的盒子就没了，得你手动搬）；右边是你只跟"总管家"说一句话、它替你盯着全部机器的结果（机器 3 瘫了，盒子自动补到别处）。**中间那个箭头，就是本课要走的全部路程。**

### 本课地图（3 步）

| 步 | 这一步要解决什么 | 对应知识点 |
|---|---|---|
| 第 1 步 | 先说清"自己管一堆容器"到底难在哪 —— 把鸿沟摆出来 | 知识点 1：容器编排要解决的鸿沟 |
| 第 2 步 | 再划清边界：这套系统**能**做什么、**不能**做什么（免得什么都往上套） | 知识点 2：k8s 是什么与不是什么 |
| 第 3 步 | 最后看它靠什么做到 —— 拆开机器，看看各部件谁干什么 | 知识点 3：集群架构总览 |

> 现在你在：**第 1 步**（刚看完全局图，接下来先把"难在哪"说透）。

---

### 知识点 1：容器编排要解决的鸿沟

> 🧭 第 1/3 步｜承接：第二幕留下的问题 —— "`--restart=always` 只能管同一台机器上的同一个容器，那跨机器的这一堆谁来管？" → 本步：把"单机 docker 能做的"和"生产真正需要的"之间的**五类鸿沟**摆到桌面上。
...
> 🧭 第 2/3 步｜承接：上一步摆出了五类鸿沟，也说 k8s 有对应解法 —— 容易顺势以为"上了 k8s 就都解决了" → 本步：先把**边界**划清楚，说清它管什么、不管什么，免得拿它当万能药。
...
> 🧭 第 3/3 步｜承接：上两步说清了"要解决什么"和"它的边界在哪" → 本步：拆开这套系统的外壳，看看它靠哪些部件分工协作，把前面的"总管家"落成一个你能画出来的结构。

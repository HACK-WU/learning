...
> 📖 结论已按官方文档核对（核查于 2026-09）：Kubernetes 官方文档 · Init 容器 / Sidecar 容器 / 容器生命周期回调。课内冲突处已在正文标注与旧说法的差异。
...
> 💡 **一句话本质**（本课把什么从 A 变成 B）：
> 本课把一个 Pod **从"只装得下正主一个人"，变成"开张前有人备场、干活时有人打下手、收摊时还走得体面"**。

> ⚖️ **处境对照**（不这么做 vs 这么做）：
>
> | | 不这么做（啥都塞进正主里） | 这么做（分开安排） |
> |---|---|---|
> | 启动前的准备 | 写进启动脚本 —— **和镜像焊死**，换个镜像就得重写 | 独立成一个个"干完就走"的容器，换镜像不影响 |
> | 准备步骤失败 | 混在启动日志里，**难定位** | 卡在那一步就停，直接看那一个容器的输出 |
> | 加日志采集等功能 | 要改业务代码、重新打镜像 | 挂个打下手的，**正主一行代码不用改** |
> | 发版删实例 | 手上的活直接扔掉 → **用户看到报错**（就是那个 502） | 先不再派新活 → 干完手上的 → 再走 |
> | 收摊超时 | 一直等，也可能一直卡着 | 给个宽限期，超时才强杀 |
>
> ⏳ 说明：以上是**机制层面**对照（职责怎么分、出错怎么定位）。"优雅终止能消灭多少次 502"取决于你的请求时长与宽限期设置，不同业务差异极大，此处不给具体数字以免误导。
...
进入细节之前，先建立整体图景 —— 本课要安排三件事：**开张前谁备场、干活时谁打下手、收摊时怎么走得体面**。

![课4 一眼全局图](../assets/lesson-04-开张干活收摊.svg)

**看图指引**：**横向**是一个整体的时间线 —— ① 开张前，几个"干完就走"的帮手一个接一个把事办好（前一个成了才轮到下一个），全办完才轮到正主；② 干活时，正主旁边配一个跟着一起的打下手，加功能不用改正主；③ 收摊时，直接拔电源会把手上的活扔掉（外面就看到报错），得先打招呼、干完手上的再走。**下方黄色区块**解释了那个 502 是怎么来的：一边已经开始收摊，另一边还不知道、仍在派活。

### 本课地图（3 步）

| 步 | 这一步要解决什么 | 对应知识点 |
|---|---|---|
| 第 1 步 | 先解决"开张前的准备" —— 那些"干完就走"的容器怎么用 | 知识点 1：init 容器 |
| 第 2 步 | 再解决"干活时的帮手" —— 打下手的怎么挂上去，以及老做法的坑 | 知识点 2：sidecar 与适配器模式 |
| 第 3 步 | 最后解决"收摊时的体面" —— 那个 502 怎么消灭 | 知识点 3：优雅终止与生命周期钩子 |

> 现在你在：**第 1 步**（刚看完全局图，接下来先安排开张前的准备）。

---

### 知识点 1：init 容器

> 🧭 第 1/3 步｜承接：第二幕留下的问题 —— "Pod 里既然是共享命运，那它们是不是同时启动的？" → 本步：看清 Pod 内部其实有严格的先后顺序，那些**先跑、干完就走**的就是它。
...
> 🧭 第 2/3 步｜承接：上一步的帮手是"干完就走"的 —— 那需要**一直跟着**的打下手（比如日志采集）怎么办？ → 本步：看挂上去的两种方式，以及老做法为什么会让"干完就收工"的任务**永远结束不了**。
...
> 🧭 第 3/3 步｜承接：前两步把开张和干活安排好了 —— 最后回到开头那个 502：发版时到底是谁在扔活？ → 本步：把收摊的完整时序拆开，看清**报错发生在哪一段时间**，以及怎么补上。
...
### 4.2 应用实战：更新上线不丢请求（入口）

> 🎯 **本课应用实战独立成篇**（配套实战册）：[第 4 课实战 · 多容器协作与优雅退场](../../../应用实战/04-多容器Pod与优雅终止.md)
> 含**分步设计图**（每步一张）与"基础 → 综合"的完整演进与代码；通览全部：📚 [应用实战索引](../../../应用实战/INDEX.md)
> 实战场景：一个依赖后端的服务，从"更新就断流"改到"连续 60 次请求零失败"。实测给出一条**容易误解**的结论：**单副本加 `preStop` 只是把断流推后，并不能消除**——真正的解是多副本。
> **课内不重复实战正文**。

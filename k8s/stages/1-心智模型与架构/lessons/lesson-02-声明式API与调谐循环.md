...
> 📖 结论已按官方文档核对（核查于 2026-09）：SQL 起源（Wikipedia · SQL、IEEE · Early History of SQL）与 k8s「声明式」思想来源。课内冲突处已在正文标注与旧说法的差异。
...
> 💡 **一句话本质**（本课把什么从 A 变成 B）：
> 本课把你和系统打交道的方式**从"我一步步教你怎么干"，变成"我只说我要什么、你负责一直对齐"**。

> ⚖️ **处境对照**（不这么做 vs 这么做）：
>
> | | 不这么做（写脚本、发指令） | 这么做（写目标、让它对齐） |
> |---|---|---|
> | 中途出错 | **不知道停在哪一步**，重跑可能重复执行、也可能更乱 | 不用管"停在哪"，它下次照旧拿"现实"和"目标"对一遍 |
> | 有人误删了一个 | 就少一个，**直到你下次手动补** | 自己发现少了，**自己补回来**（这就是第二幕那个现象） |
> | 扩缩容 | 再写一条指令、再执行一次 | 改目标里的那个数字，交给它 |
> | 机器/实例换成别的 | 脚本里的地址、步骤可能要跟着改 | 目标没变，重新对齐即可 |
> | 你需要记住的 | **"现在是什么状态"** | 只要"我要什么" |
>
> ⏳ 定性说明：以上是**机制层面**的对照（谁负责对齐、出错后谁兜底），不给出具体的耗时或人力百分比 —— 这类数字随团队与系统规模差异极大，凭印象给反而误导。
...
进入细节之前，先建立整体图景 —— 本课要解决的就一件事：**把"我教你怎么干"换成"我说我要什么"**。

![课2 一眼全局图](../assets/lesson-02-说目标还是说步骤.svg)

**看图指引**：**上半**是你一步一步教的方式 —— 第 3 步失败后，没人知道接下来该从哪继续；**下半**是你只写下一个目标，然后有个圈（看看现在 → 对一对 → 动手补齐）**一直转**，转到现实和目标一致为止。**下半那个不停打转的圈，就是本课的主角。**

### 本课地图（3 步）

| 步 | 这一步要解决什么 | 对应知识点 |
|---|---|---|
| 第 1 步 | 先把两种吩咐方式的**差别**讲透：一个说"怎么干"，一个说"要什么" | 知识点 1：声明式 vs 命令式 |
| 第 2 步 | 再拆开"那个一直转的圈"是怎么转的 —— 谁在看、怎么看、多久看一次 | 知识点 2：调谐循环 Reconcile |
| 第 3 步 | 最后落到你手上的工具：你敲的每条命令，到底改的是"目标"还是"现实" | 知识点 3：kubectl 与 API 资源模型 |

> 现在你在：**第 1 步**（刚看完全局图，接下来先把两种方式的差别说透）。

---

### 知识点 1：声明式 vs 命令式

> 🧭 第 1/3 步｜承接：第二幕留下的问题 —— "我明明下了删除指令，它为什么又变回来？" → 本步：先分清**两套吩咐方式**的根本差别，你会看到"删除指令"和"目标"压根不在一个层面上。
...
> 🧭 第 2/3 步｜承接：上一步说清了"目标"和"动作"是两回事 —— 那谁来盯这个目标？ → 本步：拆开那个一直转的圈，看它**看什么、比什么、比完做什么**。
...
> 🧭 第 3/3 步｜承接：上两步把"圈"讲完了 —— 那你手上敲的 `kubectl` 命令，到底是在改"目标"还是改"现实"？ → 本步：把你每天用的命令**对回这张图**，以后每敲一条你都知道它落在哪一侧。

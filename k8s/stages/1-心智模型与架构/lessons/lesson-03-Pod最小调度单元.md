...
> 📖 结论已按官方文档核对（核查于 2026-09）：Kubernetes 官方文档 · Pod，以及 Google Borg 论文中的 alloc 抽象。课内冲突处已在正文标注与旧说法的差异。
...
> 💡 **一句话本质**（本课把什么从 A 变成 B）：
> 本课把调度单位**从"一个个单独的容器"，变成"一伙必须待在一起、共享命运的东西"** —— 边界划在哪，决定了谁能直接喊话、谁能共享文件、谁会一起没。

> ⚖️ **处境对照**（不这么做 vs 这么做）：
>
> | | 不这么做（直接调度容器） | 这么做（打包成一个整体再调度） |
> |---|---|---|
> | 两个搭档程序 | 可能被分到**不同机器**，一个根本找不到另一个 | 一定在同一台，能直接互相喊话、能共享文件 |
> | 想让它们挨着 | 只能**硬塞进同一个容器**（起两个进程），得自己写进程管理器 | 各是各的容器，又能保证同机 |
> | 独立升级 / 独立排障 | 塞一起后，**升级只能一起升、崩了可能一起崩** | 仍然可以各管各的：单独升级、单独看日志 |
> | 判断"还活着吗" | 只看进程在不在 —— **"活着但不干活"看不出来** | 可以定时问三种问题：起了没 / 活不活 / 能不能接客 |
> | 调度器要操心的对象 | 每个容器一个决策 | 一个整体一次决策 |
>
> ⏳ 说明：以上是**机制层面**对照（谁能跟谁通信、能不能独立升级），不给性能数字 —— "多包一层"的开销在本课程环境内可忽略，且随运行时不同而异，不在此给具体值。
...
进入细节之前，先建立整体图景 —— 本课要解两个麻烦：**一伙东西怎么保证不分家**，以及**它"活着但不干活"怎么看出来**。

![课3 一眼全局图](../assets/lesson-03-为什么多包一层.svg)

**看图指引**：**上方三格**是三种做法的对比 —— 各跑各的会分家（✕）；硬塞进一个盒子虽然挨着了，但要升级只能一起升；**只有打包成一个虚线框里的整体**，才能保证一定在同一台，又能各管各的。**下方**是第二个麻烦：光看"在不在"不够，得定时问它三种问题。

### 本课地图（3 步）

| 步 | 这一步要解决什么 | 对应知识点 |
|---|---|---|
| 第 1 步 | 先说清为什么非要"多包这一层" —— 不包会怎样 | 知识点 1：为什么需要 Pod 这一层 |
| 第 2 步 | 再看这个整体从生到死要过哪些关口、会停在哪些状态上 | 知识点 2：Pod 生命周期与状态机 |
| 第 3 步 | 最后解决"活着但不干活" —— 那三种定时问的问题怎么用 | 知识点 3：探针三兄弟 |

> 现在你在：**第 1 步**（刚看完全局图，接下来先把"为什么非要这一层"说透）。

---

### 知识点 1：为什么需要 Pod 这一层

> 🧭 第 1/3 步｜承接：第二幕留下的问题 —— "Pod 里通常也只有一个容器，那它不就是多包了一层吗？" → 本步：从"什么时候必须挨在一起"倒推，看清这一层到底**多包出了什么**。
...
> 🧭 第 2/3 步｜承接：上一步定下了"一伙东西"这个边界 —— 那这个整体从被安排下去到真正能干活，要过哪些关口？ → 本步：把**从生到死的各个状态**摊开，你会看到 `Running` 并不等于"已经在干活"。
...
> 🧭 第 3/3 步｜承接：上一步看到 `Running` 也不代表能干活 —— 那到底谁来判断"能不能干活"？ → 本步：请出三种定时问的问题，并说清**问错问题的代价**（这也是本课最容易踩的坑）。
...
### 4.2 应用实战：给一个「假死」的服务配齐探针（入口）

> 🎯 **本课应用实战独立成篇**（配套实战册）：[第 3 课实战 · 探针三件套](../../../应用实战/03-Pod最小调度单元.md)
> 含**分步设计图**（每步一张：这一版长什么样、比上一版改了什么）与"基础 → 综合"的完整演进与代码；通览全部：📚 [应用实战索引](../../../应用实战/INDEX.md)
> 实战场景：一个**启动慢（25 秒）且会假死**的服务，从"状态显示正常但用户一直报错"，到配齐三种探针实现发现与自愈。全部结论为本机 kind 集群实测定，可对照复现。
> **课内不重复实战正文**。

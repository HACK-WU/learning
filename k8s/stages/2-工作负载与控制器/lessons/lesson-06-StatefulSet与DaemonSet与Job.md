...
> 📖 结论已按官方文档核对（核查于 2026-09）：Kubernetes 官方文档 · StatefulSet / DaemonSet / Job / CronJob / TTL。课内冲突处已在正文标注与旧说法的差异。
...
> 💡 **一句话本质**（本课把什么从 A 变成 B）：
> 本课把"一律多备几个一模一样的"**变成"按脾气分类安排"** —— 有身份的给固定名字和专属存储，每台都要有的按机器铺，干完就走的让它真的走。

> ⚖️ **处境对照**（不分类、一律用同一套 vs 分类安排）：
>
> | | 一律"多备几个一模一样的" | 按脾气分类安排 |
> |---|---|---|
> | 部署数据库 | 重建后**名字变了**，其他节点认不出来 → 集群裂开；**数据也没了** | 重建后还叫原来的名字，专属存储还在 |
> | 铺日志采集 | 可能几个都挤在同一台机器，**其他机器一个都没有** | 每台机器恰好一个，新加机器自动补上 |
> | 跑定时备份 | 跑完又被**重启一个** → 无限循环备份 | 干完就结束，不会再被拉起来 |
> | 新加一台机器 | 要手动上去补 agent | 自动铺上去 |
> | 跑完的任务残留 | 越积越多，**攒着占地方** | 可设定时间自动打扫 |
>
> ⏳ 说明：以上是**机制层面**对照（身份是否稳定、是否按机器铺、结束行为如何）。"残留对象会攒到多少、多久撑爆"取决于你的执行频率与集群规模，此处不给具体数字。
...
进入细节之前，先建立整体图景 —— 本课要解的麻烦是：**有的东西不能当成"一模一样的备胎"**。

![课6 一眼全局图](../assets/lesson-06-三种脾气三种安排.svg)

**看图指引**：**左上灰框**是上一课那套"谁都行、补个新的来"的做法 —— 它成立的前提是"谁都一样"。**右边三格**是三类脾气不同的东西：① 有身份的（三号没了，补来的还得叫三号、记事本还得在）；② 每台机器上都要有（不管几台机器，每台恰好一个，新加的自动补）；③ 干完就收工（千万别再重启一个）。**下方**是对应的三种安排法，一句话分清。

### 本课地图（4 步）

| 步 | 这一步要解决什么 | 对应知识点 |
|---|---|---|
| 第 1 步 | 先解决"有身份的"：固定名字、专属存储、按序起停怎么来 | 知识点 1：StatefulSet |
| 第 2 步 | 再解决"每台都要有的"：怎么铺满每台机器、新机器怎么自动补 | 知识点 2：DaemonSet |
| 第 3 步 | 然后解决"干完就走的"：一次性任务与定时任务怎么用 | 知识点 3：Job 与 CronJob |
| 第 4 步 | 最后收尾：干完的东西堆着不打扫会怎样、怎么自动清 | 知识点 4：Job 的 TTL 自动清理 |

> 现在你在：**第 1 步**（刚看完全局图，接下来先安排"有身份的"这类）。

---

### 知识点 1：StatefulSet

> 🧭 第 1/4 步｜承接：第二幕留下的问题 —— "Deployment 能自愈，为什么不能拿来部署数据库？" → 本步：看清它补出来的是个**陌生人**，而数据库需要的恰恰是"还是原来那个人、记事本还在"。
...
> 🧭 第 2/4 步｜承接：上一步解决的是"每个要有身份" —— 但日志采集这类东西不在乎身份，它在乎的是**每台机器都得有一个** → 本步：看这种"按机器铺"的安排怎么做，以及它为什么连"要几个"这个字段都没有。
...
> 🧭 第 3/4 步｜承接：前两类都追求"一直有人在" —— 可备份这种活儿，**干完就该收工**，被重启反而成灾 → 本步：看一次性任务和定时任务怎么表达"干完就算成功，别再拉我起来"。
...
> 🧭 第 4/4 步｜承接：上一步的任务能干完就收工了 —— 但"干完了"这个记录本身还留着，天天跑就天天攒 → 本步：看这些残留怎么自动打扫，以及它和"定时任务保留几份历史"不是一回事。
...
### 4.2 应用实战：接手一个系统，三类东西要部署

上面验证的是「三种控制器各自是什么」。如果你想知道**用错控制器会怎样、又怎么改对**，做一个完整场景：

- 👉 [应用实战 · 三种命运的工作负载](../../../应用实战/06-StatefulSet与DaemonSet与Job.md)

场景是：你接手一个系统，要部署数据库、备份任务、日志采集三样东西，图省事全用了 Deployment——然后三件事陆续翻车。你会亲手测出**数据库重建后数据彻底找不到**、**备份被无限重启**、**3 个采集副本全挤在同一台机器**，再逐个换成对的控制器。

> 🎯 **课内不做 repetition**：上面的机制验证讲「是什么」，实战讲「一个真实场景从犯错到改对的完整过程」。全部实战见 [应用实战索引](../../../应用实战/INDEX.md)。

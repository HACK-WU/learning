...
> 📖 结论已按官方文档核对（核查于 2026-09）：Kubernetes 官方文档 · Deployment / ReplicaSet / 控制器 / 垃圾回收。课内冲突处已在正文标注与旧说法的差异。
...
> 💡 **一句话本质**（本课把什么从 A 变成 B）：
> 本课把"我要 3 个"**从一次性的创建动作，变成一句一直算数的话** —— 有人一直盯着，少了就补；还能安全地换新、安全地退回。

> ⚖️ **处境对照**（不这么做 vs 这么做）：
>
> | | 不这么做（自己建、自己守） | 这么做（交给控制器） |
> |---|---|---|
> | 实例挂了一个 | **就少一个**，直到你发现并手动补 | 有人一直在数，少一个立刻补 |
> | 半夜节点宕机 | 服务降级到你第二天早上才发现 | 自动在别处补齐，你不用醒 |
> | 换镜像版本 | 先删旧的再建新的 → **中间那一阵是断的** | 一个个换，任何时刻都还有能干活的顶着 |
> | 新版本有 bug | 凭记忆手动还原旧配置，**容易还原错** | 每次换都留一份存档，一句话退回去 |
> | 你要记住的 | 现在有几个、分别是什么版本 | 只要"我要几个、什么版本" |
>
> ⏳ 说明：以上是**机制层面**对照（谁负责补、换的时候断不断）。"滚动更新具体不影响多少请求"取决于你的副本数、就绪判断与业务启动耗时，不同服务差异极大，此处不给具体百分比。
...
进入细节之前，先建立整体图景 —— 本课要解三个麻烦：**少了一个谁补、换新版怎么不断、换错了怎么退回去**。

![课5 一眼全局图](../assets/lesson-05-我要三个怎么保证.svg)

**看图指引**：**①** 有个一直盯着的人在数"够不够 3 个"，少一个就补；**②** 换新时，一次全换会让中间那阵子一个都没有（✕），一个个换才能保证始终有能干活的顶着；**③** 每次换都留一份存档，出问题一句话退回去。**下方灰框**是这些事的承担者 —— 一层管一层，每层只多干一件事。

### 本课地图（4 步）

| 步 | 这一步要解决什么 | 对应知识点 |
|---|---|---|
| 第 1 步 | 先说清"一直盯着的那个人"是谁 —— 以及为什么光有 Pod 不够 | 知识点 1：控制器与 ReplicaSet |
| 第 2 步 | 再看换新版时，怎么做到**任何时刻都还有能干活的** | 知识点 2：滚动更新 |
| 第 3 步 | 然后解决"换错了怎么退" —— 存档存在哪、怎么退 | 知识点 3：回滚与版本历史 |
| 第 4 步 | 最后是本课最反直觉的一环：删掉上面那层，下面的会跟着怎样 | 知识点 4：垃圾回收与级联删除 |

> 现在你在：**第 1 步**（刚看完全局图，接下来先认识那个"一直盯着的人"）。

---

### 知识点 1：控制器与 ReplicaSet

> 🧭 第 1/4 步｜承接：第二幕留下的问题 —— "既然是声明式的，我把 Pod 的 YAML 交上去，它为什么不会自己复活？" → 本步：看清"创建一次"和"一直维持"是两回事，两者之间缺的就是**控制器**。
...
> 🧭 第 2/4 步｜承接：上一步有了"一直盯着、保证个数"的人 —— 那要**换新版本**时，它会怎么做才不至于断？ → 本步：拆开"一个个换"的具体节奏，看清新旧为什么会短暂共存。
...
> 🧭 第 3/4 步｜承接：上一步能安全地换新了 —— 可新版本真出了 bug 怎么办？ → 本步：看那些"存档"存在哪，以及一个反直觉的事实：退回旧版本**不会**把存档编号退回去。
...
> 🧭 第 4/4 步｜承接：前三步看到了"上面那层建出下面那层" —— 那反过来，**删掉上面那层，下面的会跟着消失吗**？ → 本步：讲清这种连带关系是怎么记的，以及"删不掉"时多半卡在哪。
...
### 4.2 应用实战：一次「明明没报错」的发布事故

上面验证的是「滚动更新和回滚是什么」。如果你想知道**一次真实发布是怎么把服务搞断的、又怎么修**，做一个完整场景：

- 👉 [应用实战 · Deployment：无状态应用的自愈与更新](../../../应用实战/05-Deployment无状态应用.md)

场景是：一个启动要 15 秒的下单服务，发布时 `deploy` 全程显示 `3/3`、没有任何报错，但用户投诉下单失败。你会亲手测出**发布期间 249 次请求里有 55 次失败**，然后用就绪探针把它修到 **332 次全成功**，最后配出「出事能一键退回去」的版本说明。

> 🎯 **课内不做 repetition**：上面的机制验证讲「是什么」，实战讲「一个真实故障从发生到修好的完整过程」。全部实战见 [应用实战索引](../../../应用实战/INDEX.md)。

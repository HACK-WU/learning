...
> 📖 结论已按官方文档核对（核查于 2026-09）：Kubernetes 官方文档 · NetworkPolicy（networking.k8s.io/v1）与网络模型说明。课内冲突处已在正文标注与旧说法的差异。
...
🎬 **场景**：把集群想成一栋办公楼。为了"装东西时完全不用操心通不通"，这栋楼从第一天起**每扇门都不锁**。

代价是：前台（对外服务）被攻破 → 一路走到账房（数据库）；一个跑测试的房间出 bug → 能把整栋楼扫一遍。

> 💡 **一句话本质**（本课把什么从 A 变成 B）：
> 本课把集群内部**从"谁都能走到谁屋里去"，变成"谁能进谁的屋由你写清楚"** —— 但要注意：你写的是规矩，真正锁门的是另一个部件。

> ⚖️ **处境对照**（不这么做 vs 这么做）：
>
> | | 不这么做（保持默认全通） | 这么做（加这套规矩） |
> |---|---|---|
> | 内部横向移动 | **毫无阻挡** —— 前台被攻破就能直达账房 | 只放行写明允许的，其余走不过去 |
> | 一个 Pod 出 bug | 可扫遍整个集群 | 影响范围被限制在写明的通路上 |
> | 上手成本 | 零 —— 装东西根本不用管网络 | 得逐条写清楚"谁能找谁" |
> | 生效确认 | 不存在这个问题 | **没有"已生效"标记**（本课实测：该资源连状态字段都没有） |
> | **代价** | 安全边界为零 | 只有"允许"没有"禁止"，思维方式要换；且**依赖另一个部件去执行** |
>
> ⏳ 说明：以上是**机制层面**对照（能不能拦、怎么表达、怎么确认生效）。"拦住多少攻击"取决于你的策略划分粒度，此处不给具体比例。
...
进入细节之前，先建立整体图景 —— 本课要解的麻烦是：**楼里每扇门都不锁，谁都能走到谁屋里去**。

![课10 一眼全局图](../assets/lesson-10-楼里每扇门都不锁.svg)

**看图指引**：**左边**是现状 —— 任意两间都能直接走到（前台被攻破就能一路走到账房）；**右边**是加了规矩之后 —— 账房只放业务间进来，前台那条路被打断（✕）。注意绿框里的提醒：**它只有"允许"，没有"禁止"**，想全封就挂上规矩但一条允许都不写。**下方黄色区块**是本课最大的坑：规矩写好了 ≠ 门真锁上了，而且**没有"已生效"的标记可看**。

### 本课地图（3 步）

| 步 | 这一步要解决什么 | 对应知识点 |
|---|---|---|
| 第 1 步 | 先看清"默认全通"到底意味着什么风险 | 知识点 1：默认全通的隐患 |
| 第 2 步 | 再学规矩怎么写 —— "只有允许、没有禁止"该怎么表达 | 知识点 2：NetworkPolicy 语义 |
| 第 3 步 | 最后解决最要命的：怎么确认它**真的生效了** | 知识点 3：CNI 支持与验证 |

> 现在你在：**第 1 步**（刚看完全局图，接下来先看清默认全通的风险）。

---

### 知识点 1：默认全通的隐患

> 🧭 第 1/3 步｜承接：第二幕留下的问题 —— "我写的规矩明明 apply 成功了，为什么什么也没发生？" → 本步：先把"默认全通"这个前提看清楚，才知道规矩是加在什么之上的。
...
> 🧭 第 2/3 步｜承接：上一步看清了风险 —— 那规矩到底怎么写？ → 本步：接受一个反直觉设定：**它只有"允许"没有"禁止"**，想拦住谁就得"不写它"。
...
> 🧭 第 3/3 步｜承接：上一步会写规矩了 —— 可回到最要命的那个问题：**它到底生效了没有**？ → 本步：给出一套实测判定法，并复现那个几乎人人都会踩的坑（DNS 被一起拦掉）。
...
### 4.2 应用实战：只让该连的连上，数据库不再裸奔

> 🎯 **想看它解决什么真实问题？** → [应用实战 · 只让该连的连上，数据库不再裸奔](../../../应用实战/10-NetworkPolicy微隔离.md)
>
> 课内验证的是「机制对不对」；实战里是一个**真实的防护场景**：三层应用里数据库只允许应用访问，并用**因果对照法**（删→恢复、改端口→再拦）证明确实是策略在起作用。
>
> 课内是**单节点 + kindnet**，跨节点与按网段放行无法验证；实战跑在**三节点 + Calico v3.31** 上，补上了这部分：跨节点 Pod、`namespaceSelector` 跨命名空间放行、`ipBlock` 与 `except` 反选。
>
> 另含两个实测抓到的陷阱：① **只写 `namespaceSelector` 会把同命名空间成员也挡在门外**；② **只放行 TCP 53 一定失败、只放 UDP 53 才够用**（严格单变量对照）。详见 [应用实战索引](../../../应用实战/INDEX.md)。

...
> 📖 结论已按官方文档核对（核查于 2026-09）：Kubernetes 官方博客 · Ingress NGINX 退役声明，与 SIG Network / 安全响应委员会联合声明。课内冲突处已在正文标注与旧说法的差异。
...
> 💡 **一句话本质**（本课把什么从 A 变成 B）：
> 本课把外部访问**从"每个服务各开一个门、还认不出网址"，变成"只开一个门、门口站个看得懂网址的来分派"**。

> ⚖️ **处境对照**（不这么做 vs 这么做）：
>
> | | 不这么做（一个服务一个门） | 这么做（一个门 + 前台分派） |
> |---|---|---|
> | 20 个微服务 | **20 个对外地址 + 20 份网关费用 + 20 份证书** | 1 个对外地址、1 份网关费用、证书集中一处 |
> | 按路径区分（`/api/user` vs `/api/order`） | **做不到** —— 它只认"哪台机器哪个口子"，看不懂网址 | 前台看得懂网址，按域名 / 路径分派 |
> | 加一个新服务 | 再开一个门、再配一份证书 | 在前台加一条分派规则 |
> | 收口与审计 | 分散在 20 处，**改一处容易漏** | 集中在一处，统一看、统一改 |
> | **代价** | 简单直白，装上就能用 | 得**另外装一个"前台"**，官方只给规则不给实现 |
>
> ⏳ 说明：以上是**机制层面**对照（入口数量、能否按网址分派、是否要额外装组件）。具体省下多少费用取决于你的云厂商计价与服务数量，此处不给具体金额。
...
进入细节之前，先建立整体图景 —— 本课要解两个麻烦：**入口太多太贵**，以及**看不懂网址、只认门牌号**。

![课8 一眼全局图](../assets/lesson-08-一个门外加认路的前台.svg)

**看图指引**：**左边**是笨办法 —— 每个服务各开一个门，20 个服务就是 20 个对外地址、20 份费用、20 份证书，而且这些门**只认"哪台机器哪个口子"，看不懂你要 `/api` 还是 `/web`**；**右边**是正解 —— 只开一个门，门口站个**看得懂网址的前台**按路径分派。**下方黄色区块**是本课最容易踩的坑：你写的那份只是"交通规则"，**还得另外装一个"交警"才有人执行**。

### 本课地图（4 步）

| 步 | 这一步要解决什么 | 对应知识点 |
|---|---|---|
| 第 1 步 | 先说清最反直觉的一点：规则和干活的人是**分开的** | 知识点 1：Ingress 与 Controller 的分离 |
| 第 2 步 | 再看规则怎么写 —— 按域名、按路径怎么分派 | 知识点 2：Ingress 规则与路径匹配 |
| 第 3 步 | 然后是生产最常用的一招：让一小部分人先试新版 | 知识点 3：灰度发布与流量切分 |
| 第 4 步 | 最后是必须知道的现状：最常用的那个前台**已经退役了**，以及怎么选 | 知识点 4：ingress-nginx 退役与 Ingress 现状 |

> 现在你在：**第 1 步**（刚看完全局图，接下来先讲"规则"和"执行者"为什么是两回事）。

---

### 知识点 1：Ingress 与 Controller 的分离

> 🧭 第 1/4 步｜承接：第二幕留下的问题 —— "我明明 apply 了一份规则，为什么服务还是访问不了？" → 本步：看清这份规则**只是一张愿望清单**，真正干活的那个得另外装。
...
> 🧭 第 2/4 步｜承接：上一步装好了"执行者" —— 那规则该怎么写？ → 本步：看清按**域名 + 路径**两个维度怎么分派，以及 `pathType` 这个字段为什么最容易写错。
...
> 🧭 第 3/4 步｜承接：上一步能按路径分派了 —— 那"让 10% 的人先试新版"这种按**比例**分怎么做？ → 本步：看清原生规则其实**不支持权重**，只能靠各家自己扩展，以及这带来的"换个前台就全得重写"问题。
...
> 🧭 第 4/4 步｜承接：前三步把这套用法讲完了 —— 但开头那个结局还没交代：最常用的那个"前台"**已经退役**。 → 本步：讲清退役的到底是哪一部分、影响面有多大，以及你现在该怎么判断要不要动。
...
### 4.2 应用实战：一个域名挂多个服务，还能灰度放量

> 🎯 **想看它解决什么真实问题？** → [应用实战 · 一个域名挂多个服务，还能灰度放量](../../../应用实战/08-Ingress七层路由与灰度发布.md)
>
> 课内验证的是「机制对不对」；实战里是一个**完整的演进**：每服务开一个 NodePort（能跑但端口数随服务数增长）→ 收敛成一个 Ingress 入口按路径分发 → 用灰度规则把新版本按 10% → 30% → 50% → 100% 逐步放出去，出问题改回 0% 秒级撤回。
>
> 全部结论为本机 3 节点集群 + ingress-nginx 实测，含「路径是原样透传、不会被重写」这一高频 404 陷阱的排查方法。详见 [应用实战索引](../../../应用实战/INDEX.md)。

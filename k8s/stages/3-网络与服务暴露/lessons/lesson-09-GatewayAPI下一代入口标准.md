...
> 📖 结论已按官方文档核对（核查于 2026-09）：Gateway API 官方文档（GatewayClass / Gateway / HTTPRoute）与 v1.0 GA 说明。课内冲突处已在正文标注与旧说法的差异。
...
🎬 **场景**：你要做"让一成的人先试新版"这件事。按课 8 的做法，你得先查自己用的是哪家的实现，然后写上那家特有的注解。

- 换一家实现 → **注解全废，得重写**
- 写错了 → **不报错，直接被当成没看见**（课 8 实测：nginx 的灰度注解用到 Traefik 上，100% 流量全进新版）

> 💡 **一句话本质**（本课把什么从 A 变成 B）：
> 本课把高级功能**从"各家自定义的暗号"，变成"大家都认的标准字段"**，并按"谁管哪一块"拆成三层，让换实现不再需要重写规则。

> ⚖️ **处境对照**（不这么做 vs 这么做）：
>
> | | 不这么做（靠各家暗号） | 这么做（写成标准字段 + 分层） |
> |---|---|---|
> | 换个实现 | 注解**全废，得重写一遍** | 规则不用动，只换最底下那层 |
> | 写错 / 用错实现 | **静默失效、不报错**（课 8 实测：100% 流量全进新版） | 有标准字段，各家都得按规矩实现 |
> | 谁能改什么 | 一份文件里挤着"守门的"和"指路的"，**谁都能改到别人的部分** | 三层分属三个角色，各管各的 |
> | 功能演进 | 老接口**已冻结**，新能力不再往里加 | 新能力都往这边加 |
> | **代价** | 一份文件搞定，简单 | 三层**缺一层就不通**，且状态可能一直是"不清楚"，得会看状态 |
>
> ⏳ 说明：以上是**机制层面**对照（是否可移植、失效是否报错、职责是否分离）。迁移工作量取决于你现有规则数量与用到的扩展程度，此处不给具体人天估算。
...
进入细节之前，先建立整体图景 —— 本课要解的麻烦是：**同一件事各家各写一套，换个牌子就全废，而且错了还不告诉你**。

![课9 一眼全局图](../assets/lesson-09-暗号变成规矩再分三层.svg)

**看图指引**：**上半**是老办法的窘境 —— 同一个"一成的人先试新版"，三家有三种暗号，把牌子 A 的暗号交给牌子 B，**不报错、直接当没看见**。**下半**是新办法 —— 把暗号换成大家都认的标准字段，再按"谁管哪块"拆三层：管设施的人定用哪家实现 → 管集群的人开门 → 写服务的人定往哪走。**底部红框**是新办法的代价：三层缺一层就不通，而且它不报错。

### 本课地图（3 步）

| 步 | 这一步要解决什么 | 对应知识点 |
|---|---|---|
| 第 1 步 | 先把三层讲清楚：每层是谁的、彼此怎么对上 | 知识点 1：Gateway API 三层模型 |
| 第 2 步 | 再看最常写的那层：路由规则怎么写、高级功能怎么用标准字段表达 | 知识点 2：HTTPRoute 与流量治理 |
| 第 3 步 | 最后回到现实：要不要从老的那套迁过来、怎么判断 | 知识点 3：从 Ingress 迁移与选型 |

> 现在你在：**第 1 步**（刚看完全局图，接下来先把三层模型拆开看）。

---

### 知识点 1：Gateway API 三层模型

> 🧭 第 1/3 步｜承接：第二幕留下的问题 —— "我明明 apply 成功了，为什么就是连不上？" → 本步：看清它和 Ingress 根本不是同一个形状：**三层缺任何一层，流量都不通**。
...
> 🧭 第 2/3 步｜承接：上一步把三层搭通了 —— 那最常写的那层（规则）具体怎么写？ → 本步：看清它如何**用标准字段**表达课 8 那些只能靠暗号才能实现的功能。
...
> 🧭 第 3/3 步｜承接：前两步把新标准讲完了 —— 可回到现实：**我现在该不该从老的那套迁过来**？ → 本步：给出判断依据，以及迁移时最容易低估的地方。
...
### 4.2 应用实战：多团队共用一个入口，还能各管各的

> 🎯 **想看它解决什么真实问题？** → [应用实战 · 多团队共用一个入口，还能各管各的](../../../应用实战/09-GatewayAPI与流量治理.md)
>
> 课内验证的是「机制对不对」；实战里是一个**真实的协作场景**：多个团队共用一个入口怎么做到互不干扰（删掉一条路由其他照常、写错时状态里直接指名是哪个 backend），以及**灰度从私有注解变成标准字段**意味着什么（含标准 `URLRewrite`，对比课 8 要写各家私有注解）。
>
> 全部结论为本机 3 节点集群 + Envoy Gateway 实测。另含一个实测踩到的陷阱：**同一个 host 上挂多条 HTTPRoute 时，先创建的那条胜出，后建的被静默忽略且状态不报冲突**。详见 [应用实战索引](../../../应用实战/INDEX.md)。

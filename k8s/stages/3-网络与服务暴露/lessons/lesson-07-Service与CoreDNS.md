...
> 📖 结论已按官方文档核对（核查于 2026-09）：Kubernetes 官方文档 · Service / EndpointSlice / DNS for Services / 集群中的代理。课内冲突处已在正文标注与旧说法的差异。
...
> 💡 **一句话本质**（本课把什么从 A 变成 B）：
> 本课把调用方手里的东西**从"一串会变的地址"，变成"一个永不变的名字"** —— 后面换多少个、换成什么地址，前面都不用动。

> ⚖️ **处境对照**（不这么做 vs 这么做）：
>
> | | 不这么做（把地址抄下来） | 这么做（前面放个固定门牌） |
> |---|---|---|
> | 重建 / 滚动更新后 | **地址全变，一个都对不上**（本课实测：`.140/.141/.142` → `.143/.144/.145`） | 名字不变，调用方零改动 |
> | 扩缩容 | 又得改配置、**又要重启调用方** | 名字不变，自动感知新加入的 |
> | 谁来处理"挑一个" | **每个调用方自己实现一遍** | 中间人统一分摊，调用方不管 |
> | 后端挂了一个 | 调用方可能还在往坏地址发 | 自动从名单里摘掉，不再分给它 |
> | 你要维护的 | 一份**随时会过期**的地址列表 | 一个名字 |
>
> ⏳ 说明：以上是**机制层面**对照（地址变不变、要不要改配置、谁来分摊）。转发本身的性能差异（不同模式、不同规模）此处不给具体数字，避免脱离你的实际环境误导。
...
进入细节之前，先建立整体图景 —— 本课要解的麻烦是：**后面一直在换，前面怎么还得是同一个门牌**。

![课7 一眼全局图](../assets/lesson-07-固定门牌挡住会变的地址.svg)

**看图指引**：**左边**是笨办法 —— 把地址抄在纸条上，明天换了一批（`.140` → `.143`）就一个都对不上；**右边**是正解 —— 前面立一个**名字永不变的固定门牌**，后面的随便换、随便加减，前面都不用动。**下方**是这个门牌由什么搭起来的：门牌本身（靠标签挑人）→ 记名单的 → 真正送活过去的 → 把名字翻译成地址的。

### 本课地图（6 步）

| 步 | 这一步要解决什么 | 对应知识点 |
|---|---|---|
| 第 1 步 | 先说清"直接记地址"为什么行不通 | 知识点 1：为什么 Pod IP 不能直接依赖 |
| 第 2 步 | 门牌要认人，靠什么认？—— 标签怎么贴、怎么选 | 知识点 2：标签与选择算符 |
| 第 3 步 | 门牌有哪几种开法？各适合什么场合 | 知识点 3：Service 类型与选型 |
| 第 4 步 | 活是怎么真正送过去的？每台机器上谁在转 | 知识点 4：kube-proxy 与转发机制 |
| 第 5 步 | "现在谁在干活"这份名单存在哪、怎么更新 | 知识点 5：EndpointSlice 与后端列表 |
| 第 6 步 | 最后解决"名字怎么变成地址" —— 不用记数字也能找到它 | 知识点 6：CoreDNS 与服务发现 |

> 现在你在：**第 1 步**（刚看完全局图，接下来先把"为什么不能直接记地址"说透）。

---

### 知识点 1：为什么 Pod IP 不能直接依赖

> 🧭 第 1/6 步｜承接：第二幕留下的问题 —— "给 Pod 配个固定 IP 不就完了，为什么要多这一层？" → 本步：看清"固定 IP"为什么在能自由调度的前提下根本做不到。
...
> 🧭 第 2/6 步｜承接：上一步确定要立一个门牌 —— 可门牌怎么知道后面哪些算"自己人"？ → 本步：看清它认人**不靠地址、只靠标签**，以及标签贴错了会直接导致"门牌后面一个都没有"。
...
> 🧭 第 3/6 步｜承接：上一步门牌会认人了 —— 但这个门牌是**只给内部看的，还是也让外面进来**？ → 本步：看清它有几种开法，各自适合什么场合（这也是面试与选型最常问的一处）。
...
> 🧭 第 4/6 步｜承接：上一步选好了门牌的款式 —— 可活究竟是**怎么送到后面去的**？中间是不是有个"总机"在转接？ → 本步：看清每台机器上都有个写规则的，以及一个反直觉的事实：**它只写规则，不碰你的流量**。
...
> 🧭 第 5/6 步｜承接：上一步弄清了"怎么送过去" —— 那"要送给谁"这份名单**存在哪、谁在更新**？ → 本步：看清这份名单长什么样，以及为什么**排障时要先看它**（连不上多半是名单是空的）。
...
> 🧭 第 6/6 步｜承接：前面五步把"名字 → 门牌 → 名单 → 送过去"串起来了 —— 可调用方**只写一个名字**，谁把名字翻成地址？ → 本步：看清集群里的"查号台"怎么工作，以及一个常见的坑：短名好写，但可能查得慢。
...
> 🎯 **想看它解决什么真实问题？** → [应用实战 · 让前端稳定找到后端](../../../应用实战/07-Service与CoreDNS.md)
>
> 课内验证的是「机制对不对」；实战里是一个**完整故障的演进**：把后端 IP 写进前端配置（上线时百分百能跑）→ Pod 一重建就静默全挂 → 改用 Service 名字 → 再加就绪探针解决「发布头几秒报错」。
>
> 全部结论为本机 3 节点 Calico 集群实测，含「有探针 0 失败 / 无探针 2~4 失败」的对照数据。详见 [应用实战索引](../../../应用实战/INDEX.md)。

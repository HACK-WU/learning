...
### 4.2 应用实战：不设资源边界，一个应用能吃掉整台机器

> 🎯 **想看它解决什么真实问题？** → [应用实战 · 资源边界与自动扩缩容](../../../应用实战/13-资源边界与自动扩缩容.md)
>
> 课内验证的是「QoS 怎么分类、Quota/LimitRange 怎么拦」；实战里是一个**真实的交付场景**：一个服务从"跑就行"到"跑得稳、跑得省"。核心结论是 **`requests` 是账本、`limits` 是天花板**，两者分工完全不同。
>
> 另含四个实测抓到的坑：① **CPU 超限只是被限流**（实测 415 个周期里 414 个被掐住，但状态仍 `Running`），**内存超限直接 `OOMKilled`**（退出码 137）——前者可压缩、后者不可压缩；② **`requests` 填太大会把账本划光**（9 个 Pod 划走 18 核 → 节点 91%，实际用量却是 0），机器闲着也调度不进；③ **不写 `requests` 的裸奔 Pod 不占账**，账本满了它照样进，却第一个被杀；④ **只写 `limits` 会被自动补成同值**，QoS 反而变成 Guaranteed（判 QoS 要看 `status.qosClass`，不能凭 spec 猜）。详见 [应用实战索引](../../../应用实战/INDEX.md)。

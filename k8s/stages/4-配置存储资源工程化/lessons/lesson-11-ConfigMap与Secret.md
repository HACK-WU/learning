...
> 📖 结论已按官方文档核对（核查于 2026-09）：Kubernetes 官方文档 · ConfigMap / Secret，以及 The Twelve-Factor App · Config。课内冲突处已在正文标注与旧说法的差异。
...
> 💡 **一句话本质**（本课把什么从 A 变成 B）：
> 本课把配置**从"焊死在镜像里、换个环境就得重新做一个"，变成"拿出来单独放、用时再塞进去"** —— 同一个镜像，挂哪套配置就是哪套环境。

> ⚖️ **处境对照**（不这么做 vs 这么做）：
>
> | | 不这么做（配置焊在镜像里） | 这么做（配置外置） |
> |---|---|---|
> | 三套环境 | **同一个程序要做三份镜像** | 一份镜像，三套配置 |
> | 你验的是哪个 | 验的是开发镜像、上的是生产镜像 —— **严格说是两个制品** | 上线和验证的是同一个制品 |
> | 改一个密码 | 重新构建、重新推送、重新拉取 —— **只为了改一行字符串** | 改配置即可（但要注意下面那条） |
> | 密码的暴露面 | **封在镜像层里**，能拉镜像的人就能翻出来，且删不掉 | 单独存放，可独立管控访问 |
> | 环境数量增长 | 镜像份数**随环境数线性增长** | 镜像始终一份 |
> | **代价** | 简单，不用理解注入机制 | **改了配置，应用可能还读到旧值** —— 这是设计如此，得懂机制 |
>
> ⏳ 说明：以上是**机制层面**对照（制品数量、改配置的代价、暴露面）。"Secret 只是编码不是加密"这类安全性细节在第三幕知识点 2 展开，此处不展开以免夸大。
...
进入细节之前，先建立整体图景 —— 本课要解的麻烦是：**同一个程序换个地方摆，为什么要重新做一份**。

![课11 一眼全局图](../assets/lesson-11-设置从成品里拿出来.svg)

**看图指引**：**左边**是焊死的做法 —— 程序一模一样，只因设置不同就要做三份成品，而且密码被封在成品里、删不掉；**右边**是外置做法 —— **成品只有一份**，换哪套设置就是哪套环境，密码单独放。**下方黄色区块**是外置之后新冒出来的坑：改了设置，程序读到的还是旧值。

### 本课地图（4 步）

| 步 | 这一步要解决什么 | 对应知识点 |
|---|---|---|
| 第 1 步 | 先解决"不敏感的那部分"怎么外置 —— 怎么放、怎么塞进去 | 知识点 1：ConfigMap |
| 第 2 步 | 再解决"敏感的那部分" —— 以及它的名字为什么有点误导 | 知识点 2：Secret |
| 第 3 步 | 然后看一个更好用的写法：把好几处配置合成一份再挂进去 | 知识点 3：投射卷 projected volume |
| 第 4 步 | 最后解决开头那个坑：改了配置，怎么让它**真的生效** | 知识点 4：配置变更与滚动更新联动 |

> 现在你在：**第 1 步**（刚看完全局图，接下来先看普通配置怎么外置）。

---

### 知识点 1：ConfigMap

> 🧭 第 1/4 步｜承接：第二幕的第一个冲突 —— "配置既然要外置，那普通配置放哪、怎么进到程序里？" → 本步：看清它有哪几种塞法，以及各自在"改了之后会不会自动更新"上的差别。
...
> 🧭 第 2/4 步｜承接：上一步解决了普通配置 —— 可密码这类东西也能这么放吗？ → 本步：看清专门放它的地方，以及一个必须打破的幻想：**它的名字叫"秘密"，其实只是换了个写法**。
...
> 🧭 第 3/4 步｜承接：前两步把两类配置分别放好了 —— 可一个程序往往要同时用好几个，一个个挂太啰嗦 → 本步：看怎么把好几处**合成一份**再挂进去。
...
> 🧭 第 4/4 步｜承接：前三步把配置都外置好了 —— 现在回到开头那个坑：**改了配置，程序为什么还读到旧值**？ → 本步：用实测钉死原因，并给出让它真正生效的做法。
...
### 4.2 应用实战：配置拿出来了，改完还得能生效

> 🎯 **想看它解决什么真实问题？** → [应用实战 · 配置拿出来了，改完还得能生效](../../../应用实战/11-配置与密钥管理.md)
>
> 课内验证的是「机制对不对」；实战里是一个**真实的交付场景**：同一份代码跑在三个环境，改配置要能可靠生效。核心是 **checksum 注解自动滚动**——把配置内容的哈希写进 Pod 模板，内容一变就自动重建，不用人记得敲 `rollout restart`。
>
> 另含两个实测抓到的陷阱：① **文件更新 ≠ 应用生效**（实测：挂载文件 80 秒后变了，应用内存里永远是用启动时读到的旧值）；② **`envFrom` 的泄密面比想象大**——不止 `env` 可见，`/proc/<pid>/environ` 里也留着明文。详见 [应用实战索引](../../../应用实战/INDEX.md)。

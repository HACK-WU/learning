...
> 📖 结论已按官方文档核对（核查于 2026-09）：Kubernetes 官方文档 · Volumes / PV / PVC / StorageClass。课内冲突处已在正文标注与旧说法的差异。
...
> 💡 **一句话本质**（本课把什么从 A 变成 B）：
> 本课把"数据存哪儿"**从拍脑袋选，变成先问一句"它得活过什么"** —— 按要活过的坎，一级一级往上选，选错层次要么浪费、要么丢数据。

> ⚖️ **处境对照**（不分层、随手就写 vs 按存活能力选）：
>
> | | 不这么做（随手写在容器里） | 这么做（按"要活过什么"选） |
> |---|---|---|
> | 进程崩溃 / OOM 重启 | **写的东西全没** | 第 2 层起就能活过重启 |
> | 滚动更新 / 被驱逐重建 | 同样全没 | 第 3、4 层能活过重建 |
> | 整台节点挂掉 | 全没 | 只有第 4 层（放在机器外）还活着 |
> | 选错的代价 | **选低了 = 数据丢失**，且往往到故障时才发现 | 按坎选，不浪费也不丢 |
> | 另一头的代价 | 不存在 | **选高了**会浪费资源、拖慢调度、还得管一份生命周期 |
> | 用的人和给的人 | 混在一起 | 分开 —— 用的人不用知道底下是哪家的存储 |
>
> ⏳ 说明：以上是**机制层面**对照（能活过哪些坎、选错的代价）。具体性能（读写速度、延迟）取决于你的存储类型与本课环境，此处不给具体数值。
...
进入细节之前，先建立整体图景 —— 本课要解决的麻烦是：**写在这儿的东西，到底能活多久**。

![课12 一眼全局图](../assets/lesson-12-写的东西能活多久.svg)

**看图指引**：**上方四格**是四层，能活过的坎一级比一级多 —— 第 1 层活不过进程重启；第 2 层能活过重启但整体重建就没了；第 3 层能活过重建却钉死在这一台机器上；第 4 层放在机器外，机器塌了也还在。**中间灰框**是选择方法：先问"它得活过什么"，别直接问"用哪个"。**下方**是第 4 层多出来的一道手续：用的人写申请单，管的人按单子配，两边分开。

### 本课地图（4 步）

| 步 | 这一步要解决什么 | 对应知识点 |
|---|---|---|
| 第 1 步 | 从第 1、2 层开始实测：重启后到底还在不在 | 知识点 1：Volume 与 emptyDir |
| 第 2 步 | 再看第 3 层：直接用机器上的目录，什么时候该用、有什么风险 | 知识点 2：hostPath |
| 第 3 步 | 然后是最常用的第 4 层：申请单和空位怎么对上 | 知识点 3：PV 与 PVC |
| 第 4 步 | 最后解决"每次都得手工准备空位吗" —— 怎么让它自动备好 | 知识点 4：StorageClass 动态供给 |

> 现在你在：**第 1 步**（刚看完全局图，接下来先用对照实验钉死前两层）。

---

### 知识点 1：Volume 与 emptyDir

> 🧭 第 1/4 步｜承接：第二幕的第一个冲突 —— "容器重启后，数据到底是丢还是不丢？" → 本步：不做推测，用同一个对照实验把"活不过重启"和"活过重启"这两层**实测钉死**。
...
> 🧭 第 2/4 步｜承接：上一步的临时暂存能活过重启，却活不过重建 —— 那直接**写到机器本身上**呢？ → 本步：看清它确实能活过重建，代价是**钉死在这一台机器上**，以及它带来的风险。
...
> 🧭 第 3/4 步｜承接：前两层一个活不过重建、一个钉死在单台机器 —— 要"机器塌了也还在"，就得往机器外面放 → 本步：看懂那道"申请单 ↔ 空位"的手续，这是第 4 层的核心。
...
> 🧭 第 4/4 步｜承接：上一步能申请了 —— 可每次都得有人**先手工备好空位**吗？ → 本步：看它怎么按需自动备好，以及"备在哪儿、备成什么样"由谁说了算。

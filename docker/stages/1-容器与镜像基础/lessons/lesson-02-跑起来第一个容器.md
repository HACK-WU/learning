...
> 📖 结论已按官方文档核对（核查于 2026-09-15 ｜ 来源：[docker container run](https://docs.docker.com/reference/cli/docker/container/run/)）
...
> 📌 **一句话本质**：`docker run` 是一次“按模板创建并启动进程”的组合动作，不是启动一个永久运行的虚拟机。
>
> ⚖️ **处境对照**：把镜像当成可重复的模板，你可以启动多个互相隔离的容器；把容器误当成虚拟机，就会误以为“容器在后台就一直活着”。
...
![从命令到运行中的容器](../assets/lesson-02-overview.svg)

> 看图：沿着“输入—准备—启动”三步看 `docker run`，最后把前台进程退出和容器停止连起来。

### 本课地图

| 步 | 要回答的问题 | 对应知识点 |
|---|---|---|
| 1 | 镜像和容器为什么能一对多？ | 镜像与容器的关系 |
| 2 | 一条 `docker run` 具体做了什么？ | `docker run` 背后发生了什么 |
| 3 | 容器如何查看、停止、删除？ | 容器生命周期命令 |

### 知识点 1：镜像与容器的关系

> 本知识点关键点：类与对象 / 只读模板 + 可写层 / 一个镜像可以起多个容器
> 🧭 第 1/3 步｜承接：从“环境要一起交接”进入实际运行 → 本步：先分清镜像这个模板和容器这个实例。
...
> 🧭 第 2/3 步｜承接：已有可运行模板还不等于服务已启动 → 本步：沿着 `docker run` 的动作链追踪一次启动。
...
> 🧭 第 3/3 步｜承接：容器已经启动并可能随时退出 → 本步：用生命周期命令观察、接管和收尾。

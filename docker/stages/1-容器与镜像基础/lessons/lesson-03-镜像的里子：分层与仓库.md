...
> 📖 结论已按官方文档核对（核查于 2026-09-15 ｜ 来源：[Build images](https://docs.docker.com/get-started/docker-concepts/building-images/)）
...
> 📌 **一句话本质**：镜像是可复用的只读层集合，标签是人类指路牌，摘要才是内容身份。
>
> ⚖️ **处境对照**：用标签沟通方便但会漂移；用摘要部署可复现但可读性差——工程上经常两者一起使用。
...
![镜像层与复用关系](../assets/lesson-03-overview.svg)

> 看图：先理解“层如何叠加和共享”，再判断标签、摘要和磁盘占用分别回答什么问题。

### 本课地图

| 步 | 要回答的问题 | 对应知识点 |
|---|---|---|
| 1 | 文件为什么删了却还占空间？ | 联合文件系统与分层 |
| 2 | `latest` 和摘要哪个更可靠？ | 仓库、标签与摘要 |
| 3 | 磁盘到底被哪些镜像材料占用？ | 镜像体积从哪来 |

### 知识点 1：联合文件系统与分层

> 本知识点关键点：只读层栈 + 一个容器层 / Copy-on-Write 的读写路径 / 层在镜像之间共享
> 🧭 第 1/3 步｜承接：上一课看到容器依赖镜像 → 本步：拆开镜像内部的层，解释读写和删除的真实路径。
...
> 🧭 第 2/3 步｜承接：理解了镜像由多层组成 → 本步：建立仓库命名和内容身份的两套视角。

#### 🧩 图解
![镜像引用从名字到内容身份](../assets/lesson-03-name-address.svg)
...
> 🧭 第 3/3 步｜承接：知道层会共享也会累积 → 本步：把磁盘占用定位到来源，并划清清理边界。

...
> 📖 结论已按官方文档核对（核查于 2026-09-15 ｜ 来源：[Docker overview](https://docs.docker.com/get-started/docker-overview/)）
...
> 📌 **一句话本质**：Docker 要解决的不是“把代码打包”，而是把代码依赖的运行环境也变成可交接的对象。
>
> ⚖️ **处境对照**：没有容器时，你交接的是一份代码和一串口头约定；有容器后，你交接的是一份可重复准备的运行材料，但仍要承担内核、数据和权限边界。
...
![从不同机器到可交接环境](../assets/lesson-01-overview.svg)

> 看图：先从“为什么同一份服务会变样”出发，再逐步看容器解决了哪一段问题。

### 本课地图

| 步 | 要回答的问题 | 对应知识点 |
|---|---|---|
| 1 | “能跑”为什么依赖整台机器？ | 环境不一致的代价 |
| 2 | 容器和虚拟机到底隔离了什么？ | 容器与虚拟机的区别 |
| 3 | Docker 在这条链路中站在哪？ | Docker 的起源与生态位 |

### 知识点 1：环境不一致的代价

> 本知识点关键点：依赖矩阵地狱 / 配置漂移 / "只在我机器上能跑"的复现成本
> 🧭 第 1/3 步｜承接：从第二幕“代码不是全部”出发 → 本步：先把环境不一致的代价拆成可观察的问题。
...
> 🧭 第 2/3 步｜承接：已经看见环境差异的成本 → 本步：比较两种隔离路径各自把边界放在哪里。
...
> 🧭 第 3/3 步｜承接：知道容器和虚拟机的边界不同 → 本步：把 Docker 放回历史与生态位置，形成全局坐标。

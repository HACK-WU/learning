...
> 📖 结论已按官方文档核对（核查于 2026-09-15 ｜ 来源：[Resource constraints](https://docs.docker.com/engine/containers/resource_constraints/)）
...
> 📌 **一句话本质**：生产进程管理的核心是给资源、恢复和退出都设定可观察、可验证的边界。
>
> ⚖️ **处境对照**：没有边界时，一个进程的异常可能扩散成整机故障；边界过紧又会造成频繁 OOM 和重启，所以限制必须由峰值、依赖和退出行为共同校准。
...
![生产容器的资源、信号与恢复](../assets/lesson-10-overview.svg)

> 看图：先设资源边界，再辨认故障信号，最后设计有序退出和恢复动作。

### 本课地图

| 步 | 要回答的问题 | 对应知识点 |
|---|---|---|
| 1 | 如何防止一个容器拖垮宿主机？ | cgroups 资源限制 |
| 2 | 容器退出后是否应该自动回来？ | 重启策略与自愈 |
| 3 | 如何让进程收到信号并完成清理？ | 优雅停止与 PID 1 |

### 知识点 1：cgroups 资源限制

> 本知识点关键点：--memory / --cpus 的语义 / OOMKilled 与退出码 137 / 容器内 free、nproc 看到的是宿主机这个经典坑
> 🧭 第 1/3 步｜承接：本地环境已经能跑 → 本步：先给资源设边界，避免单个进程拖垮整台机器。

#### 🧩 图解
![资源限制与故障信号](../assets/lesson-10-resource-boundary.svg)
...
> 🧭 第 2/3 步｜承接：资源边界让故障可控 → 本步：决定哪些退出应该自动恢复，并分清恢复与健康检查。

#### 🧩 图解
![重启策略比较](../assets/lesson-10-restart-policy.svg)
...
> 🧭 第 3/3 步｜承接：服务可能被限制、重启或替换 → 本步：让退出有序，让信号和清理真正抵达应用。

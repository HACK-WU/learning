...
> 📖 结论已按官方文档核对（核查于 2026-09-15 ｜ 来源：[Networking overview](https://docs.docker.com/engine/network/)）
...
> 📌 **一句话本质**：容器网络同时解决“谁能进来”和“服务怎么找到彼此”两件事，不能只盯着端口映射。
>
> ⚖️ **处境对照**：单机开发通常只需自定义 bridge；跨主机通信则需要更高一层的网络或编排能力，不能把本机的 `-p` 当成多机方案。
...
![容器网络的入口与边界](../assets/lesson-08-overview.svg)

> 看图：先区分外部入口和服务间通信，再把“可达”与“可隔离”一起作为网络设计目标。

### 本课地图

| 步 | 要回答的问题 | 对应知识点 |
|---|---|---|
| 1 | 不同网络驱动各解决什么范围的问题？ | 网络驱动全景 |
| 2 | 外部端口如何发布且不冲突？ | bridge 网络与端口映射 |
| 3 | 服务如何按名字找到彼此？ | 自定义网络与 DNS 服务发现 |

### 知识点 1：网络驱动全景

> 本知识点关键点：bridge / host / none / overlay / macvlan 各自解决什么问题 / 单机默认用 bridge
> 🧭 第 1/3 步｜承接：服务需要被交接和复现 → 本步：先选择它们应该插入哪种网络范围。

#### 🧩 图解
![网络驱动范围](../assets/lesson-08-network-driver-map.svg)
...
> 🧭 第 2/3 步｜承接：网络范围已经选定 → 本步：处理外部入口、端口发布和冲突。
...
> 🧭 第 3/3 步｜承接：外部端口只解决“进来” → 本步：用自定义网络让服务按名字互相找到，并明确隔离边界。

...
> 📖 结论已按官方文档核对（核查于 2026-09-15 ｜ 来源：[Storage volumes](https://docs.docker.com/engine/storage/volumes/)）
...
> 📌 **一句话本质**：容器适合承载可替换的程序，持久数据必须明确交给容器之外的存储生命周期。
>
> ⚖️ **处境对照**：把数据写在容器里最省事但最脆弱；把数据挂出去需要管理备份、权限和清理，却换来可替换、可迁移。

## 第三幕：层层揭示

### 一眼全局图

![程序与独立数据](../assets/lesson-07-overview.svg)

> 看图：沿着“产生数据—独立存放—替换程序仍保留”的链路，判断每一类数据该跟谁一起走。

### 本课地图

| 步 | 要回答的问题 | 对应知识点 |
|---|---|---|
| 1 | 容器删掉时，哪些内容会消失？ | 容器文件系统的临时性 |
| 2 | volume、bind mount、tmpfs 怎么选？ | 三种挂载方式 |
| 3 | 卷怎么命名、备份、清理才不误删？ | 卷的生命周期与清理 |

### 知识点 1：容器文件系统的临时性

> 本知识点关键点：容器删则可写层删 / 写时复制带来的写放大 / 哪些数据必须外置
> 🧭 第 1/3 步｜承接：镜像和容器的可替换性已经明确 → 本步：先识别哪些数据会随容器生命周期消失。

#### 🧩 图解
![容器数据层与外部存储](../assets/lesson-07-container-data-layers.svg)
...
> 🧭 第 2/3 步｜承接：知道数据不能默认留在可写层 → 本步：比较三种把数据放到容器外的方式。
...
> 🧭 第 3/3 步｜承接：挂载方式已经选定 → 本步：把卷的命名、备份、迁移和清理纳入生命周期管理。

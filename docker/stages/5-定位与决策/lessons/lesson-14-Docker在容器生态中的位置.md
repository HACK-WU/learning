...
> 📖 结论已按官方文档核对（核查于 2026-09-15 ｜ 来源：[Docker Engine](https://docs.docker.com/engine/)、[Docker security](https://docs.docker.com/engine/security/)）
...
> 📌 **一句话本质**：Docker 是一套面向开发与交付的体验和工具链，底层运行能力还要放回 OCI、运行时和编排的分层里理解。
>
> ⚖️ **处境对照**：小团队可以用一套简单工具完成交付；规模、隔离或调度要求上升后，要增加平台能力，而不是机械地把所有系统都搬到同一层。

## 第三幕：层层揭示

### 一眼全局图

![从上层操作到底层执行](../assets/lesson-14-overview.svg)

> 看图：从人熟悉的命令向下追踪到管理层和执行层，再回到“谁负责什么”的选型问题。

### 本课地图

| 步 | 要回答的问题 | 对应知识点 |
|---|---|---|
| 1 | 一条容器命令由哪些层协同完成？ | OCI 与运行时栈 |
| 2 | 什么时候需要编排，替代方案差异是什么？ | 编排与替代方案 |
| 3 | 容器和虚拟机如何按边界取舍？ | 容器 vs 虚拟机 |

### 知识点 1：OCI 与运行时栈

> 本知识点关键点：runc / containerd / dockerd 三层分工 / CRI 是什么 / 为什么 Kubernetes 不再需要 dockerd
> 🧭 第 1/3 步｜承接：已经会使用高层命令 → 本步：向下拆开一次运行到底经过哪些层。
...
> 🧭 第 2/3 步｜承接：知道工具链各层职责 → 本步：按规模和调度需求比较编排与替代方案。

#### 🧩 图解
![编排方案取舍](../assets/lesson-14-orchestration-choice.svg)
...
> 🧭 第 3/3 步｜承接：平台选择已经有候选 → 本步：把隔离、性能和长期运维成本放在同一张取舍表里。

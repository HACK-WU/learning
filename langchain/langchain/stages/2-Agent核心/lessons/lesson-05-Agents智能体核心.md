...
**应用实战（4.2）**：把本课知识点组装成一条「让 agent 跑得可控、结果可接」的演进路线——独立成册：

> 🤖 **应用实战 5：让客服 agent 跑得可控、结果可接、过程可见** → [打开实战篇](../../../应用实战/05-Agents智能体核心.md)
>
> 从「默认配置一把梭」到「加护栏 + 结构化输出」再到「流式进度 + 运行账本」的三个版本演进：每一步解决了什么问题、还剩什么问题、下一步怎么被逼出来——配 3 张分步设计图（每张标注本步新增了什么）。

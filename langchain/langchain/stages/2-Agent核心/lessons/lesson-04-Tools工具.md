...
**应用实战（4.2）**：把本课知识点组装成一条「内部 API 变成敢用的工具」的演进路线——独立成册：

> 🔧 **应用实战 4：把内部订单 API 变成 agent 敢用的工具** → [打开实战篇](../../../应用实战/04-Tools工具.md)
>
> 从「裸函数当工具」到「结构化契约 + 异常转译」再到「按角色装配工具集」的三个版本演进：每一步解决了什么问题、还剩什么问题、下一步怎么被逼出来——配 3 张分步设计图（每张标注本步新增了什么）。

...
**应用实战（4.2）**：把本课知识点组装成一条「横切能力抽离」的演进路线——独立成册：

> 🔌 **应用实战 8：给 agent 加日志、限额、重试，别再往业务里塞** → [打开实战篇](../../../应用实战/08-Middleware中间件.md)
>
> 从「横切逻辑散落在业务代码」到「单个中间件抽离」再到「多中间件组合与顺序确定」的三个版本演进：每一步解决了什么问题、还剩什么问题、下一步怎么被逼出来——配 3 张分步设计图（每张标注本步新增了什么）。

...
> 说明：本课程的应用实战机制自**课 13** 起启用；此前课次未配独立实战篇（按"旧课不强制回溯"口径）。

- [实战 13：给客服 agent 建一道「交付门禁」](应用实战/13-Testing与Observability.md)（配套课 13）—— 3 版演进：人肉验证 → 单测工序 → 完整门禁（每步配分步设计图）
...
- [场景解法库](10-场景解法库/INDEX.md)：8 个高难度场景 × 每场景多解法权衡（先想后看；经典设计题 + 规模压力题两类）

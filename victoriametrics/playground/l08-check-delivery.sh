...
F="stages/4-怎么横向扩展/8-集群三件套与最小集群实战.md"
MISS=0
...
chk "### 关键伏笔"            "A20 关键伏笔"
chk "### 什么时候该上集群"     "A21 选型决策清单"

echo
echo "=============================================="
echo " B 组 - 脚本存在性 / 链接可达"
echo "=============================================="
for s in $(grep -oE 'l08-[a-z0-9-]+\.sh' "$F" | sort -u); do
  file "playground/$s" "脚本 $s"
done
file "stages/3-凭什么快凭什么省/7-内存模型与容量规划.md" "上一课讲义"
...
echo " C 组 - 繁简检查"
echo "=============================================="
for w in 本課 課件 存儲 壓縮 機制 數據 內存 緩存 集群版變成 擴展; do
  n=$(grep -oF "$w" "$F" | wc -l)
...
echo "=============================================="
echo "  集群组件存活:"
for pair in "vminsert-learn:8480" "vmselect-learn:8481" "vmstorage-learn:8482" "vmstorage-learn2:8492"; do
  name="${pair%%:*}"; port="${pair##*:}"
  c=$(curl -s -o /dev/null -w '%{http_code}' --max-time 3 "http://localhost:$port/health" 2>/dev/null)
  echo "    $name (port $port): HTTP $c"
done

echo "  各 vmstorage tsid 条目:"
curl -s --max-time 15 'http://localhost:8482/metrics' 2>/dev/null | grep 'vm_cache_entries{type="storage/tsid"' | head -1
curl -s --max-time 15 'http://localhost:8492/metrics' 2>/dev/null | grep 'vm_cache_entries{type="storage/tsid"' | head -1

echo "  vmselect 聚合查询 count(l08_shard_value):"
...
print("    ", int(float(r[0]["value"][1])) if r else "空")' 2>/dev/null

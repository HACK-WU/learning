...
| 解法 | 能解决到 | 代价 | 适用边界 |
|------|---------|------|---------|
| **A · `rate_limit='10/s'`** | 单 worker 限速 | ⚠️ **per-worker 生效**，多 worker 会翻倍 | 单 worker 场景 |
| **B · 控制并发数 = 限流值** | 简单可靠地压住 QPS | 吞吐受限 | 任务耗时稳定时 |
| **C · Redis 令牌桶** | 真正的全局限流 | 要写代码 | **推荐** |
| **D · 指数退避 + jitter** | 防重试风暴 | 拉长总耗时 | **必做** |
| **E · 批量接口** | 调用次数直接降到 1/100 | 依赖对方支持 | 首选（如果对方有） |
...
def acquire_token(key, rate=10):
    """每秒计数限流：返回 True 表示拿到令牌"""
    now = int(time.time())

...
const raw = JSON.parse('{"id":"o1"}');
const order = raw; // ✅ 编译通过，缺 amount 也照样
try {
    console.log("as ->", order.amount.toFixed(2));
}
catch (e) {
    console.log("as ->", e.message); // 💥 运行时炸
}
// B：双重断言能把完全不对的东西"变成" Order
const notAnOrder = "hello"; // ✅ 编译通过
try {
    console.log("double as ->", notAnOrder.amount.toFixed(2));
}
catch (e) {
    console.log("double as ->", e.message); // 💥 运行时炸
}

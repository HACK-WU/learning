...
> 📖 结论已按官方文档核对（核查于 2026-09-16 ｜ 来源：[Classes / Type Compatibility 路由表](../../../web-index/typescript/index.md)）
...
> 本课把业务对象的字段、职责和行为边界写清，让类之间可以安全协作。

### 处境对照

| 所有成员都能随意改 | 成员可见性和职责明确 |
|---|---|
| 状态修改路径不可控 | 违规访问更早暴露，行为可复用 |
...
![本课一眼全局图](../assets/lesson-07-entry.svg)

> 看图：先把业务对象的字段和行为分开看，再明确职责，最后得到可复用且边界清楚的对象。

### 本课地图

| 步骤 | 要解决什么 | 对应知识点 |
|---|---|---|
| 第 1 步 | 规定对象有哪些字段、哪些不能随意改 | 知识点 1：类的成员与修饰符 |
| 第 2 步 | 表达必须实现的职责和共享骨架 | 知识点 2：抽象类与 implements |
| 第 3 步 | 理解类的兼容规则和链式返回类型 | 知识点 3：类的结构化兼容与 this 类型 |

> ⚠️ **本课的默认环境**（与前六课一致）：所有示例在 `playground/lesson-07/` 目录下执行，**没有 `tsconfig.json`**，直接 `npx tsc xxx.ts` 编译单个文件。TS 7.0.2 默认 `strict: true`。

### 知识点 1：类的成员与修饰符

> 🧭 第 1/3 步｜承接：第二幕要给订单对象加上受保护的状态和行为 → 本步：先划定成员的可见性与可变性。
...
> 🧭 第 2/3 步｜承接：成员边界明确后，还需要规定一组对象必须履行的职责 → 本步：比较抽象类与接口实现。
...
> 🧭 第 3/3 步｜承接：对象能实现职责还不够，多个类之间还要安全协作 → 本步：理解兼容关系和链式 API 的返回类型。
...
|---|---|---|---|
| 成员与修饰符 | 字段和职责边界 | access modifier / parameter property | `public`、`private`、`protected`、`readonly` |
| 抽象类与接口 | 共享骨架与职责约束 | abstract class / implements clause | 领域服务、适配器、插件契约 |
| 类兼容与 `this` | 形状兼容与链式返回 | class compatibility / polymorphic `this` | fluent API、继承链、赋值检查 |

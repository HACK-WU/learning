...
> 📖 结论已按官方文档核对（核查于 2026-09-16 ｜ 来源：[Handbook 路由表](../../../web-index/typescript/index.md)）
...
> 本课把“一个位置可能是什么”写成有限、可检查的状态集合。

### 处境对照

| 不做精确建模 | 做精确建模 |
|---|---|
| `string` 接受拼写错误和未知状态 | 合法状态成为可检查边界 |
...
![本课一眼全局图](../assets/lesson-04-entry.svg)

> 看图：先把可能状态放在一起，再用线索辨认当前状态，最后只让合法状态继续。

### 本课地图

| 步骤 | 要解决什么 | 对应知识点 |
|---|---|---|
| 第 1 步 | 表达一个值可能属于哪些状态 | 知识点 1：联合类型 |
| 第 2 步 | 把有限取值和字段模式写清楚 | 知识点 2：字面量类型与模板字面量类型 |
| 第 3 步 | 在枚举与轻量替代方案之间做选择 | 知识点 3：枚举与替代方案 |

> ⚠️ **本课的默认环境**（与前三课一致）：所有示例在 `playground/lesson-04/` 目录下执行，**没有 `tsconfig.json`**，直接 `npx tsc xxx.ts` 编译单个文件。TS 7.0.2 默认 `strict: true`。

### 知识点 1：联合类型

> 🧭 第 1/3 步｜承接：第二幕在问“string 为什么等于什么都没说” → 本步：先把合法可能性收成一个集合。
...
> 🧭 第 2/3 步｜承接：有了可能性集合，还需要表达精确值和命名模式 → 本步：把值本身提升为可组合的约束。
...
> 🧭 第 3/3 步｜承接：精确取值已经能表达，但不同方案的运行时代价不同 → 本步：比较 enum 与字面量联合等替代方案。
...
|---|---|---|---|
| 联合类型 | 多种可能的集合 | union type / discriminated union | 状态模型、API 响应 |
| 字面量类型 | 精确到某个值 | literal type / template literal type | 配置值、事件名、字段模式 |
| 枚举与替代方案 | 带运行时取舍的命名集合 | `enum` / string literal union | 公共 API、编译产物、协议状态 |

...
> 📖 结论已按官方文档核对（核查于 2026-09-16 ｜ 来源：[Everyday Types / Narrowing 路由表](../../../web-index/typescript/index.md)）
...
> 本课把“未知输入能不能直接信”变成一道明确的边界：先保留不确定，再验证后使用。

### 处境对照

| 直接相信输入 | 先验证再使用 |
|---|---|
| 编译器看不见运行时造假的数据 | 错误在边界处变成可控结果 |
...
![本课一眼全局图](../assets/lesson-06-entry.svg)

> 看图：外部输入不能直接相信，必须先检查和收窄，确认后才进入业务逻辑。

### 本课地图

| 步骤 | 要解决什么 | 对应知识点 |
|---|---|---|
| 第 1 步 | 看清放弃检查的代价 | 知识点 1：any：逃生舱与传染性 |
| 第 2 步 | 用安全的未知值承接外部输入 | 知识点 2：unknown：安全的顶层类型 |
| 第 3 步 | 在系统边界完成验证再向内传递 | 知识点 3：信任边界：类型从哪来 |
...
> 🧭 第 1/3 步｜承接：第二幕中 `as` 让错误消失，却没有增加证据 → 本步：先看 `any` 如何让整个数据流失去约束。
...
> 🧭 第 2/3 步｜承接：完全放弃检查太危险，需要一个能接住未知值但不允许直接使用的容器 → 本步：用 `unknown` 保留安全边界。
...
> 🧭 第 3/3 步｜承接：`unknown` 只是把问题留在入口，还要真正证明外部值的形状 → 本步：建立运行时校验到业务类型的边界。
...
|---|---|---|---|
| `any` | 逃生舱与传染性 | top-like escape hatch / implicit any | 遗留 JS、未声明库、`noImplicitAny` |
| `unknown` | 安全的未知值 | top type / unknown type | `JSON.parse`、`catch`、外部输入 |
| 信任边界 | 进入系统前先验证 | trust boundary / runtime validation | 网络响应、表单、数据库、schema 校验 |

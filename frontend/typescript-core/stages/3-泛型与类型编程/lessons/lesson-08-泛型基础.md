...
> 📖 结论已按官方文档核对（核查于 2026-09-16 ｜ 来源：[Generics / keyof 路由表](../../../web-index/typescript/index.md)）
...
> 本课把“一份逻辑适配多种类型”从重复复制，变成调用时确定且不丢信息的模板。

### 处境对照

| 重复写或使用 `any` | 使用泛型 |
|---|---|
| 要么维护多份，要么丢失具体类型 | 一份逻辑复用，调用处仍保留信息 |
...
![本课一眼全局图](../assets/lesson-08-entry.svg)

> 看图：同一份逻辑先暂缓确定具体类型，调用时再明确形状，同时保留类型信息。

### 本课地图

| 步骤 | 要解决什么 | 对应知识点 |
|---|---|---|
| 第 1 步 | 让一份函数逻辑服务多种类型 | 知识点 1：泛型函数与类型参数推导 |
| 第 2 步 | 限制类型参数可以访问的结构 | 知识点 2：泛型约束与 keyof |
| 第 3 步 | 把泛型放进接口、别名和默认参数 | 知识点 3：泛型类型：接口、别名与默认值 |

> ⚠️ **本课的默认环境**（与前七课一致）：所有示例在 `playground/lesson-08/` 目录下执行，**没有 `tsconfig.json`**，直接 `npx tsc xxx.ts` 编译单个文件。TS 7.0.2 默认 `strict: true`。

### 知识点 1：泛型函数与类型参数推导

> 🧭 第 1/3 步｜承接：第二幕中 `any` 让信息消失、重载又造成重复 → 本步：让调用时再确定类型参数。
...
> 🧭 第 2/3 步｜承接：类型参数能保留信息，却还不知道它有哪些成员 → 本步：用约束和键集合限制可访问范围。
...
> 🧭 第 3/3 步｜承接：泛型函数已经能复用逻辑 → 本步：把这种复用能力提升到类型容器和 API 设计层。
...
|---|---|---|---|
| 泛型函数 | 调用时确定类型 | generic function / type parameter inference | 工具函数、集合 API、响应包装 |
| 泛型约束 | 限定可访问结构 | generic constraint / `keyof` / indexed access | `T extends`、`T[K]` |
| 泛型类型 | 带参数的类型容器 | generic interface / generic type alias | SDK、领域模型、容器类型 |

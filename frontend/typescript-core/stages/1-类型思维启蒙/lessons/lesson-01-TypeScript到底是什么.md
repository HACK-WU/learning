...
> 📖 结论已按官方文档核对（核查于 2026-09-16 ｜ 来源：[TypeScript Handbook 路由表](../../../web-index/typescript/index.md)、[TypeScript 7.0 官方公告](../../../web-index/typescript-blog/index.md)）
...
> 本课把“运行时才发现的类型问题”提前变成“写代码时就能看见的约束”，再说明这套约束为什么仍不能替代运行时校验。

### 处境对照

| 做法 | 你会面对什么 | 结果 |
|---|---|---|
| 只靠运行时发现 | 问题要等到某条路径真正执行才出现 | 修复时已经离开写错的位置 |
| 提前做静态检查 | 写代码时就看到不一致 | 可以在提交前修复，仍需守住外部数据边界 |
...
![本课一眼全局图](../assets/lesson-01-entry.svg)

> 看图：左边是信息散落、运行前看不出问题；中间把不一致提前核对；右边才进入运行阶段。

### 本课地图

| 步骤 | 要解决什么 | 对应知识点 |
|---|---|---|
| 第 1 步 | 先弄清类型在什么时候存在、什么时候消失 | 知识点 1：TS 的本质 |
| 第 2 步 | 再衡量提前检查带来的收益与代价 | 知识点 2：静态类型的价值与代价 |
| 第 3 步 | 最后亲手跑通从源码到产物的路径 | 知识点 3：第一个 TS 程序与编译流程 |

### 知识点 1：TS 的本质：擦除式类型超集

> 🧭 第 1/3 步｜承接：第二幕在问“类型究竟在哪里保护程序” → 本步：先确认类型存在于编写/检查阶段，而不是运行时。
...
| 本课说法 | 行业标准叫法 | 在哪里遇到 |
|---|---|---|
| 类型编完就消失 | type erasure / erasable syntax | 编译产物、TS 7 的可擦除语法讨论 |
| 把 TS 变成 JS | transpilation / emit | `tsc` 输出、构建日志 |

---

### 知识点 2：静态类型的价值与代价

> 🧭 第 2/3 步｜承接：知道类型只在检查阶段存在后，还要回答“提前检查值不值得” → 本步：比较发现时机、收益与代价。
...
| 本课说法 | 行业标准叫法 | 在哪里遇到 |
|---|---|---|
| 提前发现错误 | static type checking | IDE 红线、`tsc --noEmit`、CI |
| 类型带来的收益 | refactoring / editor tooling | 编辑器重构、自动补全、类型导航 |

---

### 知识点 3：第一个 TS 程序与编译流程

> 🧭 第 3/3 步｜承接：前两步说明了“为什么要检查”以及“检查不等于运行” → 本步：把源码、检查、产物和运行串成一条可复现链路。
...
| 本课说法 | 行业标准叫法 | 在哪里遇到 |
|---|---|---|
| 编译配置文件 | `tsconfig.json` | 项目根目录、`tsc -p` |
| 只检查不输出 | `noEmit` | CI、编辑器任务、预提交检查 |

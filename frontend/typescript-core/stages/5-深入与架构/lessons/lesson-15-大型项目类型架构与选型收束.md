...
> 📖 结论已按官方文档核对（核查于 2026-09-16 ｜ 来源：[Project References / Declaration Files 路由表](../../../web-index/typescript/index.md)、[TypeScript 7.0 官方公告](../../../web-index/typescript-blog/index.md)）
...
> 本课把“类型该放哪、迁移怎么走、要不要上 TS”收束成可解释的架构和选型决策。

### 处境对照

| 类型边界随意共享 | 按职责和变化半径放置 |
|---|---|
| 一个改动牵动全项目，迁移容易失控 | 影响面可控，团队知道何时严格、何时收手 |
...
![本课一眼全局图](../assets/lesson-15-entry.svg)

> 看图：大型项目先划分类型边界，再控制变化半径，最后形成可以继续演进的架构。

### 本课地图

| 步骤 | 要解决什么 | 对应知识点 |
|---|---|---|
| 第 1 步 | 决定类型放在哪一层、由谁负责 | 知识点 1：类型的分层与放置 |
| 第 2 步 | 设计稳定的公开类型边界 | 知识点 2：公开 API 的类型设计 |
| 第 3 步 | 为老项目安排渐进迁移路线 | 知识点 3：JS → TS 渐进迁移 |
| 第 4 步 | 综合判断何时上 TS、上多严或不上 | 知识点 4：决策收束：什么时候不该上 TS |

> ⚠️ **本课的实测环境**：所有数字都在 `playground/lesson-15/` 下**实际跑出来**。分层成本用「冷启动 → 空转基线 → 改动后增量」三步法测量（课 13 定下的规矩：先测基线再扣）。

### 知识点 1：类型的分层与放置

> 🧭 第 1/4 步｜承接：第二幕的大项目被共享类型牵连 → 本步：先划分领域、边界和 DTO 的职责。
...
> 🧭 第 2/4 步｜承接：类型边界确定后，还要让外部使用者看到稳定契约 → 本步：设计公开 API 的类型形状和兼容策略。
...
> 🧭 第 3/4 步｜承接：新架构不能要求老项目一次重写 → 本步：用混编、叶子到入口和渐进严格度完成迁移。
...
> 🧭 第 4/4 步｜承接：所有技术方案都有成本 → 本步：把收益、工具链、数据边界和团队维护能力放进最终决策。
...
|---|---|---|---|
| 类型分层 | 领域、边界和 DTO 分开 | domain type / boundary type / DTO | 分层架构、模块目录、依赖图 |
| 公开 API | 对外稳定契约 | public API / declaration surface | `.d.ts`、semver、破坏性变更 |
| 渐进迁移与选型 | 按风险控制严格度 | allowJs / checkJs / migration strategy | JS→TS、JSDoc、项目决策 |
...
> 🧭 课程导航将在本课的示例文件清单之后给出，确保读者先看到可复现实验，再进入下一阶段。

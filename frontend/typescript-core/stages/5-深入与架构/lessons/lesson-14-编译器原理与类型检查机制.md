...
> 📖 结论已按官方文档核对（核查于 2026-09-16 ｜ 来源：[Compiler Notes](https://github.com/microsoft/TypeScript-Compiler-Notes)、[TSConfig 路由表](../../../web-index/typescript/index.md)）
...
> 本课把“报错很长、编译很慢”拆成可观察的编译阶段、兼容规则和性能瓶颈。

### 处境对照

| 只看最后一条报错 | 拆分阶段与证据 |
|---|---|
| 修复靠猜，优化没有方向 | 能定位是谁检查、哪里耗时、为何兼容 |
...
![本课一眼全局图](../assets/lesson-14-entry.svg)

> 看图：报错或变慢只是最终症状，把流程拆开后才能看到真正的阶段和瓶颈。

### 本课地图

| 步骤 | 要解决什么 | 对应知识点 |
|---|---|---|
| 第 1 步 | 看懂源码如何经过编译流程 | 知识点 1：编译流程全景 |
| 第 2 步 | 用可赋值性和变体解释兼容结果 | 知识点 2：可赋值性与变体 |
| 第 3 步 | 找到构建慢的阶段并选择提速手段 | 知识点 3：为什么慢 & 怎么提速 |
| 第 4 步 | 把复杂报错还原成可定位的结构 | 知识点 4：读懂复杂类型报错 |

> ⚠️ **本课的实测环境**：所有数字都在 `playground/lesson-14/` 下**实际跑出来**。阶段耗时用 `tsc --diagnostics`（编译器自报）；并行度用「预热后取 3 次最小值」的方法（课 10 定下的规矩）。

### 知识点 1：编译流程全景

> 🧭 第 1/4 步｜承接：第二幕只有“报错很长/构建很慢”的表象 → 本步：先拆出编译器各阶段。
...
> 🧭 第 2/4 步｜承接：知道检查流程后，还要解释某些值为什么能赋给另一些值 → 本步：用可赋值性和变体规则定位。
...
> 🧭 第 3/4 步｜承接：知道谁在检查后，才能判断慢在哪里 → 本步：用诊断数据和并行/项目引用缩短瓶颈。
...
> 🧭 第 4/4 步｜承接：数据和规则都清楚后，最后把报错文本翻译成可行动的定位线索。
...
|---|---|---|---|
| 编译流程 | 多阶段处理源码 | Scanner / Parser / Binder / Checker / Emitter | `--diagnostics`、Compiler Notes |
| 可赋值性与变体 | 判断能否替换 | assignability / covariance / contravariance | `strictFunctionTypes`、函数参数 |
| 慢与报错 | 从症状定位阶段 | performance tracing / diagnostic message | `--generateTrace`、`--explainFiles`、TS2321 |
...
➡️ **下一课**：[课 15：大型项目类型架构与选型收束](lesson-15-大型项目类型架构与选型收束.md)（阶段 5 收官）

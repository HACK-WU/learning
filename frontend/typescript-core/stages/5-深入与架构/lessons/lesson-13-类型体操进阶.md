...
> 📖 结论已按官方文档核对（核查于 2026-09-16 ｜ 来源：[Release Notes 路由表](../../../web-index/typescript/index.md)、[TypeScript 7.0 官方公告](../../../web-index/typescript-blog/index.md)）
...
> 本课不是教你把类型写得更复杂，而是教你判断复杂类型的收益是否值得它的编译和维护成本。

### 处境对照

| 只看表达能力 | 同时评估代价 |
|---|---|
| 类型可能变慢、变难懂或静默失败 | 复杂度有收手线，方案能被团队维护 |
...
![本课一眼全局图](../assets/lesson-13-entry.svg)

> 看图：复杂类型先经过代价评估，再决定保留、简化或改用生成方案。

### 本课地图

| 步骤 | 要解决什么 | 对应知识点 |
|---|---|---|
| 第 1 步 | 让递归和字符串类型计算可控 | 知识点 1：递归条件类型与字符串操作 |
| 第 2 步 | 从结构中提取和组合类型信息 | 知识点 2：高级推断技巧 |
| 第 3 步 | 建立性能、可读性和维护性收手线 | 知识点 3：类型体操的代价与可读性底线 |

> ⚠️ **本课的实测环境**：所有数字都在 `playground/lesson-13/` 下**实际跑出来**。递归深度用两种独立构造交叉验证；编译耗时用"预热后取最小值"的方法（课 10 定下的规矩）。

### 知识点 1：递归条件类型与字符串操作

> 🧭 第 1/3 步｜承接：第二幕中“类型能算但可能算过头” → 本步：先观察递归计算的能力边界。
...
> 🧭 第 2/3 步｜承接：递归让类型能走得更深，还需要更精确地从结构中取信息 → 本步：用 `infer` 等技巧控制推断。
...
> 🧭 第 3/3 步｜承接：能力越强，代价越可能转移到编译器和团队 → 本步：形成“什么时候别写”的判断标准。
...
|---|---|---|---|
| 递归类型 | 类型自己调用自己 | recursive conditional type / tail recursion | TS2589、模板字面量类型 |
| 高级推断 | 从结构中提取信息 | `infer` / template literal inference | 条件类型、路由字符串解析 |
| 代价与收手 | 复杂度不能无限堆 | type-level programming cost / maintainability | 编译耗时、报错长度、代码评审 |
...
➡️ **下一课**：[课 14：编译器原理与类型检查机制](lesson-14-编译器原理与类型检查机制.md)

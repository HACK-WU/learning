...
> 📖 结论已按官方文档核对（核查于 2026-09-16 ｜ 来源：[TSConfig 路由表](../../../web-index/typescript/index.md)、[TypeScript 7.0 官方公告](../../../web-index/typescript-blog/index.md)）
...
> 本课把源码、目标运行环境和编译产物放进同一套配置契约，避免“能编译但不能运行”。

### 处境对照

| 沿用旧配置 | 按运行环境配置 |
|---|---|
| 升级后撞上默认值和硬错误 | 配置选择与产物运行位置一致 |
...
![本课一眼全局图](../assets/lesson-10-entry.svg)

> 看图：源码先根据目标环境选择配置规则，再生成既能检查又能运行的产物。

### 本课地图

| 步骤 | 要解决什么 | 对应知识点 |
|---|---|---|
| 第 1 步 | 识别 TS 7 默认值变化和旧配置硬错误 | 知识点 1：TS7 的新默认与硬错误 |
| 第 2 步 | 根据运行环境选择模块和目标 | 知识点 2：目标与模块配置 |
| 第 3 步 | 分批打开严格性开关 | 知识点 3：严格性开关族与渐进收紧 |
| 第 4 步 | 控制大型项目的构建成本 | 知识点 4：构建性能与项目引用 |
...
> 🧭 第 1/4 步｜承接：第二幕的老配置在 TS 7 下撞墙 → 本步：先识别哪些默认值变了、哪些选项已成硬错误。
...
> 🧭 第 2/4 步｜承接：配置能通过还不够，产物必须匹配实际运行环境 → 本步：联动选择 target、module 和 moduleResolution。
...
> 🧭 第 3/4 步｜承接：环境匹配后，还要决定检查强度和迁移节奏 → 本步：把严格性拆成可渐进收紧的开关。
...
> 🧭 第 4/4 步｜承接：检查越完整，工程规模越大时成本越明显 → 本步：用并行和项目引用控制变化半径。
...
|---|---|---|---|
| 新默认与硬错误 | 配置升级的破坏性变化 | compiler option defaults / deprecated option removal | TS 7 公告、TS5102、TS5108 |
| 目标与模块 | 产物匹配运行环境 | `target` / `module` / `moduleResolution` | `nodenext`、`bundler`、Node ESM |
| 严格性与性能 | 检查强度和构建成本 | strictness flags / project references | `strict`、`--checkers`、`tsc -b` |
...
➡️ **下一课**：[课 11：模块与声明文件](lesson-11-模块与声明文件.md)

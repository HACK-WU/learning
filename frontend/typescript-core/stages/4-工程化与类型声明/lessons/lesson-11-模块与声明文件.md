...
> 📖 结论已按官方文档核对（核查于 2026-09-16 ｜ 来源：[Modules / Declaration Files / TSConfig 路由表](../../../web-index/typescript/index.md)）
...
> 本课把“文件在哪里、库长什么样”变成编译器和编辑器都能找到的工程契约。

### 处境对照

| 只让编译器暂时通过 | 同时处理解析和声明 |
|---|---|
| 运行时可能找不到路径，补全也可能失真 | 类型、路径和真实实现更容易对齐 |
...
![本课一眼全局图](../assets/lesson-11-entry.svg)

> 看图：模块或外部库先经过路径解析和声明补齐，编辑器看到的形状才有机会与运行时对齐。

### 本课地图

| 步骤 | 要解决什么 | 对应知识点 |
|---|---|---|
| 第 1 步 | 让导入路径和运行环境使用同一套规则 | 知识点 1：模块解析与路径别名 |
| 第 2 步 | 为没有类型的实现补充可信声明 | 知识点 2：声明文件 `.d.ts` 与 `declare` |
| 第 3 步 | 控制全局类型和标准库类型的来源 | 知识点 3：`@types` 生态与 `types`·`lib` 配置 |
...
> 🧭 第 1/3 步｜承接：第二幕中“编译成功但运行找不到模块” → 本步：先把解析规则和运行时别名分开。
...
> 🧭 第 2/3 步｜承接：路径找到了，外部库的形状仍可能未知 → 本步：用声明文件补齐编辑器和编译器看到的契约。
...
> 🧭 第 3/3 步｜承接：声明来源变多后，哪些全局类型会自动进入也需要控制 → 本步：区分 `types`、`lib` 和 `@types`。
...
|---|---|---|---|
| 模块解析 | 找到导入对应的文件 | module resolution / `paths` | `traceResolution`、NodeNext、bundler |
| 声明文件 | 给 JS 实现补类型 | declaration file / ambient declaration | `.d.ts`、`declare module`、声明合并 |
| 环境类型 | 控制全局和标准库来源 | `@types` / `types` / `lib` | Node、DOM、测试框架、Vite |
...
➡️ **下一课**：[课 12：工具链集成与团队协作](lesson-12-工具链集成与团队协作.md)

...
> 📖 **结论已按官方文档核对**：本课涉及的错误对象、Promise 拒绝、调试器与 Source Map 表述，复核入口见 [`web-index/`](../../../web-index/INDEX.md)；运行时输出仍以当前 Node.js/浏览器版本为准。
...
**错误处理要同时对齐抛出时机、捕获边界和定位信息；同步调用靠 `try`，异步链路靠 `await`/Promise 处理与全局兜底，生产定位再依赖 Source Map。**

> ⚖️ **处境对照**：只在外层包一个 `try`，异步回调和 Promise 拒绝仍可能漏出；沿“抛出 → 捕获 → 定位”的链路设计，才能把线上异常变成可处理、可追踪的问题。
...
![错误处理与调试总览](../assets/lesson-11-overview.svg)

> 📖 **读图**：从左到右看：先识别错误，再沿同步或异步路径捕获，最后把压缩堆栈映射回源码；三段链路缺一段，排障就会中断。

### 本课地图

| 学习顺序 | 先回答的问题 | 进入位置 |
|---|---|---|
| 1 | 应该抛什么错误、如何保留错误链？ | 知识点 1 |
| 2 | `try` 的边界在哪里，异步错误怎么接？ | 知识点 2 |
| 3 | 如何从线上堆栈定位到源码？ | 知识点 3 |

> 🧭 **第 1/3 步**：先认识错误对象和抛出策略，再进入捕获边界。

### 知识点 1：Error 体系与 throw

> 🧭 **第 1/3 步**：先建立“错误类型 + 错误链 + 业务上下文”的信息模型，再学习捕获边界。
...
> 📖 **读图**：从左到右看，错误先分类，再补充可读信息与原始原因，最后交给边界层决定响应、上报或退出。
...
上方的内置错误全景、自定义错误类和 `cause` 链示例，分别演示“分类、定位、上下文”三层信息如何组合。

#### 常见误区

不要把字符串或 `null` 当作标准错误抛出；捕获后也不要假设任意值都有 `message` 和 `stack`。

#### 一句话记住

> **抛 `Error` 保留可定位信息，用错误类型表达类别，用 `cause` 和业务字段保留上下文。**

---

### 知识点 2：try·catch·finally 与异步错误

> 🧭 **第 2/3 步**：已经知道错误应携带什么信息；现在看捕获边界和异步时序。
...
上方的 `setTimeout`、Promise、`await` 与全局兜底案例，分别演示同步错误、异步错误和边界层处理的差异。

#### 常见误区

不要把“代码写在 `try` 花括号里”当成充分条件；判断关键是错误发生时，当前捕获栈帧是否仍然存在，以及 Promise 是否被 `await` 或 `.catch()` 接住。

#### 一句话记住

> **同步错误在还没离开栈帧时捕获；异步错误沿 Promise 链捕获，最后才用全局兜底。**

---

### 知识点 3：调试工具链与 Source Map

> 🧭 **第 3/3 步**：最后从捕获到定位，把日志、断点、调试器与 Source Map 串成排障闭环。
...
flowchart LR
    A[线上异常] --> B[日志发现线索]
    B --> C[断点观察状态]
    C --> D[Source Map 还原源码位置]
    D --> E[修复并回归验证]
```

> 📖 **读图**：从左到右看，排障从低成本线索开始，逐步深入运行时状态，再把产物坐标还原到源码并验证修复。
...
上方的 `console`、`debugger`、`node --inspect` 与 Source Map 实验，串起“观察—暂停—定位—还原源码”的排障流程。

#### 常见误区

不要把大量 `console.log` 当成完整调试方案，也不要把含 `sourcesContent` 的 Source Map 无条件公开到生产环境。

#### 一句话记住

> **日志发现线索，断点观察状态，Source Map 把线上坐标翻译回源码。**
...
| 上一课 | 返回目录 | 下一课 |
|--------|---------|--------|
| [第 10 课：模块化](lesson-10-模块化.md) | [课程目录](../../../02-课程目录.md) | [第 12 课：内存·性能与选型收束](lesson-12-内存性能与选型收束.md) |

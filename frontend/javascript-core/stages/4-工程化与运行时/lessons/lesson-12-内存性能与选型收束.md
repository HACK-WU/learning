...
> ✅ 状态：已完成（2026-09-03）｜ 历史实操环境：Node.js v22.14.0（文中输出来自当时本机实测）；本轮复核环境：Node.js v22.22.2

> 📖 **结论已按官方文档核对**：本课涉及的 GC、TypeScript、Node.js、Deno、Bun 与 webpack 信息，复核入口见 [`web-index/`](../../../web-index/INDEX.md)；带版本号的事实均为 2026-09-16 核查时点。
...
**先用可达性定位对象是否仍被根持有，再用项目约束决定工具；性能与选型都应由证据和边界条件驱动，而不是由流行度驱动。**

> ⚖️ **处境对照**：只盯着某个变量或某个工具，容易把暂时的内存波动当泄漏、把厂商基准当选型结论；画出引用图和约束图，才能按证据定位与决策。
...
![内存与选型总览](../assets/lesson-12-overview.svg)

> 📖 **读图**：从左到右看：先判断对象是否可达，再定位泄漏来源，最后把规模、团队、维护周期与生态约束合并成选型结论。

### 本课地图

| 学习顺序 | 先回答的问题 | 进入位置 |
|---|---|---|
| 1 | GC 为什么按可达性判断？ | 知识点 1 |
| 2 | 内存泄漏如何定位与修复？ | 知识点 2 |
| 3 | JS/TS、运行时和工具链如何选？ | 知识点 3 |

> 🧭 **第 1/3 步**：先建立可达性模型，再进入泄漏排查和工具选型。

### 知识点 1：垃圾回收机制

> 🧭 **第 1/3 步**：先用 GC Roots 和可达性理解“对象为什么还活着”，再定位泄漏路径。
...
上方的循环引用、`FinalizationRegistry`、分代回收与 `process.memoryUsage()` 案例，分别演示算法判断、回收观察和指标解读。

#### 常见误区

不要把“引用计数为零”当作现代 JS 的唯一回收标准，也不要把一次 `gc()` 后内存未立刻下降直接判为泄漏。

#### 一句话记住

> **GC 看从根出发是否可达；环可以回收，时机则由运行时调度。**

---

### 知识点 2：常见内存泄漏与排查

> 🧭 **第 2/3 步**：已经掌握“可达性”；现在沿常见引用路径定位泄漏。
...
flowchart LR
    A[内存持续增长] --> B[重复操作并拍快照]
    B --> C[比较构造器增量]
    C --> D[查看 Retainers 持有链]
    D --> E[清理引用并复测]
```

> 📖 **读图**：从左到右看，先制造可比较的增长，再由构造器和 Retainers 逐层回溯，最后清理引用并复测终态。
...
上方的定时器、监听器、闭包、脱离 DOM 与堆快照案例，分别演示“制造泄漏—观察增长—沿 Retainers 回溯—剪断引用”的完整路径。

#### 常见误区

不要把 `rss` 上升、GC 尚未立刻下降或一次快照变大直接等同于泄漏；先看 `heapUsed`、重复操作后的增量和持有链。

#### 一句话记住

> **泄漏不是对象变大，而是本该消失的对象仍可达；用快照找持有链，再剪断多余引用。**

---

### 知识点 3：选型收束：JS vs TS、运行时与工具链

> 🧭 **第 3/3 步**：最后把内存问题的“关系视角”迁移到工具选型：按约束比较，不按宣传下结论。
...
```mermaid
flowchart LR
    A[项目约束] --> B[规模 / 团队 / 周期 / 生态]
    B --> C[核对当前官方事实]
    C --> D[比较候选工具]
    D --> E[记录结论与翻转条件]
```

> 📖 **读图**：从左到右看，先收集约束，再核对时点事实，最后输出带有翻转条件的选型结论。
...
把选型想成**给一支队伍配装备**：项目规模、团队经验、维护周期和生态支持是地形与负重；没有脱离任务的“最强装备”，只有在当前约束下能稳定完成任务的组合。

#### 核心原理

选型先收集约束，再核对官方事实，最后写出建议与翻转条件；性能基准只能作为同场景证据，不能直接替代项目验证。
...
| | **Node.js** | **Deno** | **Bun** |
|---|---|---|---|
| 当前版本（核查于 2026-09-16） | **26.8.2 Current / 24.21.0 LTS** | **2.9.6**（deno.com） | **1.4.2**（bun.sh） |
...
上方的厂商基准对照、JS/TS 判断表和运行时取舍表，演示如何区分事实、约束与最终建议。

#### 常见误区

不要把厂商自测基准当成跨项目结论，也不要把“最新版本”写成永久事实；版本号、兼容性和插件支持都要带核查日期。

#### 一句话记住

> **先列项目约束，再核对当前生态，最后给带翻转条件的选型建议。**
...
| 上一课 | 返回目录 | 下一站 |
|--------|---------|--------|
| [第 11 课：错误处理与调试](lesson-11-错误处理与调试.md) | [课程目录](../../../02-课程目录.md) | **Phase 3 综合实战项目**（见上方接力提示词） |

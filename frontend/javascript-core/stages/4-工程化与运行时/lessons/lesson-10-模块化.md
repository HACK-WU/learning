...
> 📖 **结论已按官方文档核对**：本课涉及的模块加载、Node.js 模块行为与 tree-shaking 表述，复核入口见 [`web-index/`](../../../web-index/INDEX.md)；版本号和工具链动态信息以官方页面为准。

## 🎯 本课目标

- 说清 ESM 的"**实时绑定**"与 CommonJS 的"**缓存导出值 / 解构快照**"差异及后果，解释为什么 ESM 更适合 tree-shaking
...
**CommonJS 以运行时加载和 `module.exports` 为中心，ESM 以静态依赖图和实时绑定为中心；选型先看运行环境与工具链，再看语法偏好。**

> ⚖️ **处境对照**：只按语法记忆，混用 CJS/ESM 时容易把导入失败、绑定只读和循环依赖混成一个问题；按加载时机和绑定关系推演，才能在换 Node 配置或 bundler 时稳定判断。
...
![模块化学习总览](../assets/lesson-10-overview.svg)

> 📖 **读图**：从左到右看：先比较加载模型，再理解导入导出，最后处理循环依赖与动态导入；每个节点都对应第三幕的一个知识点。

### 本课地图

| 学习顺序 | 先回答的问题 | 进入位置 |
|---|---|---|
| 1 | CJS 与 ESM 的加载和绑定有何不同？ | 知识点 1 |
| 2 | 如何正确组织导入、导出与转发？ | 知识点 2 |
| 3 | 循环依赖如何定位，何时使用 `import()`？ | 知识点 3 |

> 🧭 **第 1/3 步**：先建立 CommonJS 与 ESM 的心智模型，再进入具体语法。

### 知识点 1：CommonJS 与 ESM

> 本知识点关键点：语法差异 / `module.exports` 与解构快照 vs 实时绑定（`import`） / 运行时加载 vs 编译时静态分析 / tree-shaking 的边界 / 在 Node 中如何启用 ESM（`package.json` 的 `type` 字段与 `.mjs`/`.cjs`）

> 🧭 **第 1/3 步**：先比较加载时机、导出对象与导入绑定，再看 Node 的启用规则。

#### 一句话定义

**CommonJS 在运行时"边加载边执行"，`require()` 得到的是当次返回的 `module.exports` 对象；如果再解构原始值，可能形成快照。ESM 先建好依赖图与绑定、再统一求值，导出的是实时绑定（live binding）。** 这个差别不在语法，而在**加载时机**。
...
> 📖 **读图**：先定位图中的起点和终点，再按箭头方向核对本段文字描述的关系。

| | CommonJS | ESM |
|---|----------|-----|
| **阶段** | 一步：require 时**立刻加载并执行** | 三步：**构建**（解析依赖图）→ **实例化**（建立绑定）→ **求值**（执行代码） |
| **发生时机** | 运行时 | 构建在**编译期**，实例化与求值在运行期 |
| **导出的是什么** | `require()` 返回 `module.exports`；解构出的原始值可能是**快照** | **实时绑定**（由导出方持有） |
| **能否静态分析** | 动态形态难分析；受限静态形态可能被 bundler 支持 | ✅ 更适合静态分析 |
...
> 🔑 **后果**：CommonJS 中 `const { count } = require(...)` 会把当次属性值绑定到本地变量，后续模块内更新不会回写这个本地变量；ESM 导入名则保持对导出方绑定的只读引用。CommonJS 不是“永远只能拿到拷贝”，关键要区分 `module.exports` 对象、属性访问和解构快照。

**③ 为什么 ESM 更适合 tree-shaking**

**Tree-shaking**（摇树）= 打包时把**没被用到的导出**剔除掉。ESM 的静态结构天然适合它；CommonJS 的动态写法会削弱分析能力，但现代 bundler 对部分静态 CommonJS 形态也可能做有限分析，不能简单概括成“只有 ESM 才成立”。

| 前提 | ESM | CommonJS |
|------|-----|----------|
| **能静态确定"依赖了谁"** | ✅ `import` 必须在**顶层**、路径必须是**字符串字面量**（实测：写在 `if` 里直接 `SyntaxError: Unexpected token '{'`） | ⚠️ `require` 可以写在任何地方、路径可以是**变量**（实测：`require('./' + name)` 正常工作），因此动态形态不稳定 |
| **能静态确定"谁被用了"** | ✅ `export const x` 是声明式的，没被 import 更容易删 | ⚠️ 动态挂属性难分析；静态、受限形态可能被 bundler 部分识别 |

```js
// CJS：这些都能跑 → 打包器难以可靠预知，通常需要保守保留
...
> ⚠️ **工程建议**：想让产物体积优化更稳定，优先选择提供 ESM 入口的库；CommonJS 并非绝对不能摇树，但分析范围更依赖 bundler 与代码写法。
...
3. **"CommonJS 完全不能 tree-shaking"** → 过度绝对。动态写法确实难分析，现代 bundler 对部分静态形态可能有限支持；想要稳定优化仍优先 ESM。
...
> **CJS 运行时加载并返回 `module.exports`，解构原始值可能成为快照；ESM 先建图再求值、导出实时绑定——静态结构让 ESM 更适合稳定地 tree-shaking。**
...
> 🧭 **第 2/3 步**：已经理解两种模块系统的边界；现在练习 ESM 的导出、导入、转发与只读绑定。
...
> 📖 **读图**：从左到右看，先按导出形式选择对应导入语法，最后都落到“导入绑定只读”这一共同约束。
...
> 🧭 **第 3/3 步**：最后把加载时序用于排查循环依赖，并决定何时延迟到 `import()`。
...
flowchart LR
    ...
    B --> A
    A --> C{访问时机}
    C -->|顶层过早访问| D[TDZ / 不完整 exports]
    C -->|函数内延迟访问| E[运行时可用]
```

> 📖 **读图**：先沿 A→B→A 看出环，再从“访问时机”分支判断是过早读取，还是延迟到初始化完成后读取。
...
| 导出的是 | `module.exports`；解构原始值可能是**快照** | **实时绑定** |
| 静态可分析 | 动态写法难分析；部分静态形态可能被 bundler 支持 | ✅（**更适合稳定 tree-shaking**） |
| 顶层 `await` | 普通 CJS 不支持；Node 对部分 `.js` 会按 ESM 语法检测 | ✅ |
| `import.meta` | ❌ | ✅ |
...
| CommonJS 的 `require` 能写在任何地方 | **设计取舍**（代价：动态写法难静态分析，tree-shaking 不稳定；收益：**同步**、灵活，插件系统天然好用） |
...
3. **"CommonJS 完全不能 tree-shaking"** → 过度绝对；动态写法难分析，部分静态形态可能被 bundler 有限支持，稳定优化仍优先 ESM。
...
graph TD
    ...
    K1 --> K1b["CJS 返回 module.exports<br/>解构原始值可能是快照<br/>ESM 导出实时绑定"]
    K1 --> K1c["静态结构 → ESM 更适合稳定 tree-shaking"]

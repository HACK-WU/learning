...
> 📖 结论已按官方文档核对（核查于 2026-09 ｜ 来源：MDN 函数 / 箭头函数 / 数组方法，详见 `../../../web-index/mdn/index.md`）
...
> 📌 **一句话本质**：把“要做什么”封装成可传递的做法，让同一套处理流程可以接收不同规则。
>
> ⚖️ **处境对照**：不抽出可传递的做法，3 个需求要维护 3 段循环；抽出来后，筛选、映射和汇总可以复用同一套数据处理管线。
...
![本课核心问题的直觉图](../assets/lesson-04-overview.svg)

> 📖 **读图**：从左到右看，先写下一套做法，再把它交给调用方，最后用同一流程批量处理数据。

### 本课地图

| 步骤 | 要解决什么 | 对应知识点 |
|---|---|---|
| 第 1 步 | 认识做法本身也是一种值 | 函数的四种定义方式与差异 |
| 第 2 步 | 处理输入、默认值和剩余参数 | 参数机制 |
| 第 3 步 | 把做法交给别的流程执行 | 高阶函数与回调 |

> 🧭 **路线提示**：先认识函数这个值，再处理它的输入，最后观察控制权如何转移。

### 知识点 1：函数的四种定义方式与差异
> 🧭 第 1/3 步｜承接：第二幕问“箭头函数和普通函数能否互换” → 本步：先比较不同定义方式的行为边界。
...
### 知识点 2：参数机制
> 🧭 第 2/3 步｜承接：知道函数可以被传递后，还要明确它如何接收数据 → 本步：拆开参数、默认值和剩余参数。
...
> 📖 **读图**：从左到右看，实参先形成形参绑定，再决定是否走默认值分支，最后进入函数体。
...
### 知识点 3：高阶函数与回调
> 🧭 第 3/3 步｜承接：参数机制解决了“传什么”，但没解决“谁来调用” → 本步：理解高阶函数把执行权交给谁。
...
> 📖 **读图**：从左到右看，函数先进入“参数”或“返回值”角色，二者都让外层函数成为高阶函数。

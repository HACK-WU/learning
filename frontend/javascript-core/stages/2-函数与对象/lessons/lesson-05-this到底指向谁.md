...
> 📖 结论已按官方文档核对（核查于 2026-09 ｜ 来源：MDN `this` / `call` / `bind`，详见 `../../../web-index/mdn/index.md`）
...
> 📌 **一句话本质**：把“函数被谁调用”与“函数要使用的身份”分开看，再决定是否需要显式绑定。
>
> ⚖️ **处境对照**：不处理时，交给定时器的同一个方法可能在 3000ms 后丢失身份；处理后，方法既能延迟执行，也能保留正确上下文。
...
![本课核心问题的直觉图](../assets/lesson-05-overview.svg)

> 📖 **读图**：先看函数被拿到哪里调用，再看调用位置如何改变身份，最后决定是否要把固定身份一起带上。

### 本课地图

| 步骤 | 要解决什么 | 对应知识点 |
|---|---|---|
| 第 1 步 | 根据调用位置判断函数身份 | this 的四种绑定规则 |
| 第 2 步 | 识别箭头函数不会创建新身份 | 箭头函数的 this |
| 第 3 步 | 需要时显式固定身份并支持新建对象 | 手写 call·apply·bind 与 new |

> 🧭 **路线提示**：先判断调用方式，再识别箭头函数例外，最后掌握显式绑定和 `new`。

### 知识点 1：this 的四种绑定规则
> 🧭 第 1/3 步｜承接：第二幕问“换个调用方式为什么就丢了” → 本步：建立调用点到 `this` 的判断规则。
...
### 知识点 2：箭头函数的 this
> 🧭 第 2/3 步｜承接：普通函数的身份由调用点决定，但箭头函数看起来例外 → 本步：确认它如何沿外层环境查找。

> 本知识点关键点：词法 `this`，**定义时**确定 / 与四种规则无关，也无法被 `call`·`apply`·`bind` 改变 / 常见误用场景：对象方法、事件回调、原型方法

```mermaid
flowchart LR
    A[箭头函数定义处] --> B[捕获外层 this]
    B --> C[调用时保持不变]
    D[call / apply / bind] -.不能改写.-> C
```

> 📖 **读图**：从左到右看，箭头函数在定义时捕获外层 `this`；右侧虚线表示后续显式绑定不会覆盖它。
...
> 🧭 第 3/3 步｜承接：知道身份如何产生后，还需要控制它并理解 `new` 的覆盖规则 → 本步：实现并验证显式绑定。

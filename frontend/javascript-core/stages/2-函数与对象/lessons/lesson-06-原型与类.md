...
> 📖 结论已按官方文档核对（核查于 2026-09 ｜ 来源：MDN 对象原型 / 类 / 继承，详见 `../../../web-index/mdn/index.md`）
...
> 📌 **一句话本质**：把每个对象的资料、共享的做法和对象之间的关系拆开组织，避免重复存储又能沿关系找到能力。
>
> ⚖️ **处境对照**：不理解这层关系时，一个方法不在实例上却能调用会像魔法；理解后，能画出实例、共享方法和查找路径，老写法也能读懂。
...
![本课核心问题的直觉图](../assets/lesson-06-overview.svg)

> 📖 **读图**：先给每个对象放自己的资料，再让对象共享一套做法，找不到时沿关系向上补查。

### 本课地图

| 步骤 | 要解决什么 | 对应知识点 |
|---|---|---|
| 第 1 步 | 解释方法不在实例上却能调用 | 原型与原型链 |
| 第 2 步 | 把熟悉的类写法拆回底层关系 | class 语法与本质 |
| 第 3 步 | 比较不同继承方式的代价 | 继承的五种写法与取舍 |

> 🧭 **路线提示**：先找到共享方法，再看 `class` 如何表达它，最后比较不同组织方式。

### 知识点 1：原型与原型链
> 🧭 第 1/3 步｜承接：第二幕问“实例没有 `say` 为什么还能调用” → 本步：沿属性查找路径找到答案。
...
### 知识点 2：class 语法与本质
> 🧭 第 2/3 步｜承接：知道共享方法来自原型后，还要解释 `class` 在其中做了什么 → 本步：拆解语法和底层行为。
...
> 📖 **读图**：从左到右看，`class` 同时产生实例侧的原型方法与类本身的静态成员，二者访问入口不同。
...
### 知识点 3：继承的五种写法与取舍
> 🧭 第 3/3 步｜承接：底层关系已经清楚，但实现方式不止一种 → 本步：比较继承写法与组合方案的边界。
...
flowchart LR
    A[复用需求] --> B{需要 is-a 关系?}
    B -->|是| C[class extends / 原型链]
    B -->|否| D[组合：委托能力]
    C --> E[检查构造与共享状态风险]
    D --> F[按需拼装行为]
```

> 📖 **读图**：从左到右看，先判断关系是否真的是“is-a”；否则进入组合分支，避免继承链和共享状态带来的耦合。

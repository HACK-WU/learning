...
> 📖 结论已按官方文档核对（核查于 2026-09 ｜ 来源：MDN 执行模型 / WHATWG Timers / Node.js 事件循环，详见 `../../../web-index/mdn/index.md` 与 `../../../web-index/node/index.md`）
...
> 📌 **一句话本质**：把“正在做的事”和“等待完成的事”分开，再按固定顺序把完成的工作取回来。
>
> ⚖️ **处境对照**：不理解调度顺序时，`setTimeout(fn, 0)` 仍会排在同步代码之后；理解后，可以用队列级别和清空时机推演输出，而不是反复试运行。
...
![本课核心问题的直觉图](../assets/lesson-07-overview.svg)

> 📖 **读图**：先完成当前工作，再把等待中的工作放入队列，最后按规则取回并继续处理。

### 本课地图

| 步骤 | 要解决什么 | 对应知识点 |
|---|---|---|
| 第 1 步 | 先理解同一时刻谁能执行 | 单线程与调用栈 |
| 第 2 步 | 再区分不同等待队列 | 宏任务与微任务 |
| 第 3 步 | 最后走完一轮完整调度 | 事件循环全流程 |

> 🧭 **路线提示**：先看“谁在执行”，再看“谁在排队”，最后看“队列何时被取出”。

### 知识点 1：单线程与调用栈
> 🧭 第 1/3 步｜承接：第二幕问“单线程怎么一边等待一边工作” → 本步：先界定执行者与调用栈。
...
> 📖 **读图**：从左到右看，所有同步代码先经过同一调用栈；任务过重会停在阻塞分支，解释了“单线程但可能卡顿”。
...
### 知识点 2：宏任务与微任务
> 🧭 第 2/3 步｜承接：知道等待的工作不会直接上栈 → 本步：区分不同队列的优先级和清空时机。
...
> 📖 **读图**：沿环路阅读；每次调用栈清空后先排空微任务，再取一个宏任务，宏任务执行完又回到微任务检查。
...
### 知识点 3：事件循环全流程
> 🧭 第 3/3 步｜承接：队列分类完成后，还要把一轮调度串起来 → 本步：走完从同步代码到下一次任务的完整流程。

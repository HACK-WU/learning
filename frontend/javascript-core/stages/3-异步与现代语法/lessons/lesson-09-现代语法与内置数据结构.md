...
> 📖 结论已按官方文档核对（核查于 2026-09 ｜ 来源：MDN 解构 / 展开 / Map / Set / 迭代协议，详见 `../../../web-index/mdn/index.md`）
...
> 📌 **一句话本质**：把复杂输入按需拆开、重组和遍历，让代码更短但不牺牲对数据边界的控制。
>
> ⚖️ **处境对照**：不理解这些语法的边界时，`0` 可能被当成“没传”、嵌套配置可能整层覆盖；理解后，能保留合法的假值并选择正确的数据容器。
...
![本课核心问题的直觉图](../assets/lesson-09-overview.svg)

> 📖 **读图**：从一份可能不完整的输入出发，先按需拆开，再选择容器和遍历方式，最后只取需要的内容。

### 本课地图

| 步骤 | 要解决什么 | 对应知识点 |
|---|---|---|
| 第 1 步 | 拆开输入并安全地重组数据 | 解构·展开·模板字符串 |
| 第 2 步 | 为不同键和值选择合适容器 | Map·Set·WeakMap·WeakSet |
| 第 3 步 | 控制遍历、暂停和缺失值处理 | 迭代器·生成器·可选链 |

> 🧭 **路线提示**：先处理数据形状，再选择存储结构，最后控制读取和遍历方式。

### 知识点 1：解构·展开·模板字符串
> 🧭 第 1/3 步｜承接：第二幕问“短写法为什么会覆盖嵌套配置” → 本步：先明确拆解和展开各自只负责什么。
...
> 🧭 第 2/3 步｜承接：数据能被拆开后，还要决定如何保存和去重 → 本步：按键类型、顺序和生命周期选择容器。
...
flowchart LR
    ...
    D -->|是| E[WeakMap]
    D -->|否| C
    F[去重需求] --> G[Set / WeakSet]
```

> 📖 **读图**：从左到右看，先按键的类型与生命周期选 Map 系列，再把单纯去重需求分到 Set 系列。
...
> 📖 **读图**：沿环路从迭代器走到 `next()`；只要 `done` 为假就继续取值，变为真才结束。

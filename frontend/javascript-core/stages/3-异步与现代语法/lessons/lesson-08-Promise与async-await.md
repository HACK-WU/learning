...
> 📖 结论已按官方文档核对（核查于 2026-09 ｜ 来源：MDN Promise / `async` / `await`，详见 `../../../web-index/mdn/index.md`）
...
> 📌 **一句话本质**：把未来结果变成可组合的状态，再决定是逐个等待、并发等待，还是只取其中一种结果。
>
> ⚖️ **处境对照**：不拆开等待关系，5 个请求可能耗时约 5 倍；拆开后，可以把独立任务并发发出，并明确失败是否影响整体。
...
![本课核心问题的直觉图](../assets/lesson-08-overview.svg)

> 📖 **读图**：先登记一个未来结果，再等它成功或失败落定，最后根据业务需要组合多个结果。

### 本课地图

| 步骤 | 要解决什么 | 对应知识点 |
|---|---|---|
| 第 1 步 | 描述未来结果及其变化 | Promise 状态机 |
| 第 2 步 | 决定失败如何传播、多个结果如何组合 | 错误处理与组合方法 |
| 第 3 步 | 用更直观的语法组织等待关系 | async/await |

> 🧭 **路线提示**：先认识结果容器，再处理失败与组合，最后把等待写成可读的同步流。

### 知识点 1：Promise 状态机
> 🧭 第 1/3 步｜承接：第二幕问“`await` 等的到底是什么” → 本步：先建立未来结果的状态模型。
...
### 知识点 2：错误处理与组合方法
> 🧭 第 2/3 步｜承接：有了未来结果后，还要决定一个失败是否拖累整体 → 本步：比较错误传播与组合策略。
...
> 📖 **读图**：从左到右看，组合方法共享同一组输入，但完成条件不同；先明确终态需求，再选方法。
...
### 知识点 3：async/await
> 🧭 第 3/3 步｜承接：组合方法解决了“怎么等”，但代码仍可能难读 → 本步：用 `async/await` 表达等待与并发。

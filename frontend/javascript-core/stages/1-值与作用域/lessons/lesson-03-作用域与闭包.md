...
> 📖 结论已按官方文档核对（核查于 2026-09 ｜ 来源：MDN 闭包 / `let` / 函数，详见 `../../../web-index/mdn/index.md`）
...
> 📌 **一句话本质**：把“代码写在哪里”和“回调以后还能看见什么”从时间错觉，变成可追踪的作用域关系。
>
> ⚖️ **处境对照**：不拆开作用域，5 个按钮可能全部显示第 6 个；拆开后，每轮绑定和回调环境都能逐一解释。
...
![本课核心问题的直觉图](../assets/lesson-03-overview.svg)

> 📖 **读图**：从左到右看，先确定代码留下的范围，再沿范围查找名字，最后理解回调为什么还能使用旧环境。

### 本课地图

| 步骤 | 要解决什么 | 对应知识点 |
|---|---|---|
| 第 1 步 | 判断名字按什么路径被找到 | 词法作用域与作用域链 |
| 第 2 步 | 解释循环和声明为何产生不同绑定 | 变量提升与块级作用域 |
| 第 3 步 | 解释回调结束后为何还能读到变量 | 闭包 |

> 🧭 **路线提示**：先看查找路径，再看绑定数量，最后看环境如何被回调保留。

### 知识点 1：词法作用域与作用域链
> 🧭 第 1/3 步｜承接：第二幕问“`let` 为什么能影响很久之后的回调” → 本步：先确定名字的查找路径。
...
### 知识点 2：变量提升与块级作用域
> 🧭 第 2/3 步｜承接：知道名字沿作用域链查找后，还要解释每轮到底有几个绑定 → 本步：拆开提升和块级绑定。
...
### 知识点 3：闭包
> 🧭 第 3/3 步｜承接：知道每轮绑定不同仍不够，还要解释回调执行时环境为何仍在 → 本步：建立闭包模型。

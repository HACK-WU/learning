...
> 📖 结论已按官方文档核对（核查于 2026-09 ｜ 来源：MDN 数据结构 / 相等性判断 / `structuredClone()`，详见 `../../../web-index/mdn/index.md`）
...
> 📌 **一句话本质**：把“复制”和“比较”从看起来相似的表面操作，拆成可追踪的引用关系与转换规则。
>
> ⚖️ **处境对照**：不拆开看，改一份配置可能污染原配置，4 个看似矛盾的比较结果只能靠死记；拆开后，能明确哪些修改共享、哪些比较会转换。
...
![本课核心问题的直觉图](../assets/lesson-02-overview.svg)

> 📖 **读图**：先看复制后是继续指向原物还是另造一份，再看最后该用哪种比较方式判断结果。

### 本课地图

| 步骤 | 要解决什么 | 对应知识点 |
|---|---|---|
| 第 1 步 | 判断复制后是否仍共享同一份对象 | 原始值 vs 引用值 |
| 第 2 步 | 需要独立副本时选择复制深度 | 深浅拷贝 |
| 第 3 步 | 面对不同类型时推演比较过程 | == vs === 与隐式转换 |

> 🧭 **路线提示**：先追踪“指向谁”，再决定“复制多深”，最后推演“如何比较”。

### 知识点 1：原始值 vs 引用值
> 🧭 第 1/3 步｜承接：第二幕问“复制到底复制了什么” → 本步：先画出值与对象之间的指向关系。
...
### 知识点 2：深浅拷贝
> 🧭 第 2/3 步｜承接：知道浅复制仍会共享嵌套对象 → 本步：按数据结构深度选择独立副本方案。
...
### 知识点 3：== vs === 与隐式转换
> 🧭 第 3/3 步｜承接：复制问题解决后，比较结果仍然反直觉 → 本步：逐步执行转换算法而不是凭感觉猜。

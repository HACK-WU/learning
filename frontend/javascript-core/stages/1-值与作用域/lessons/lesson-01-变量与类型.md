...
> 📖 结论已按官方文档核对（核查于 2026-09 ｜ 来源：MDN `var` / `let` / `typeof`，详见 `../../../web-index/mdn/index.md`）
...
typeof user;           // 'undefined' ← 完全未声明时，typeof 仍然安全
```

三个现象同时出现。别急，**这三个现象的根因，全都在这门课里**。

> 📌 **一句话本质**：把“名字怎么保存、能看多远、代表什么”从凭直觉猜，变成按规则判断。
>
> ⚖️ **处境对照**：不先分清这些规则，3 个回调可能输出同一个序号、空值可能被误判；分清后，变量范围和类型结果都能在运行前推演。
...
2. **为什么 `typeof` 一会儿安全一会儿炸？** 完全未声明的名字会返回 `'undefined'`，但处于 TDZ 的 `let`/`const` 变量会抛 `ReferenceError`。
...
| `typeof` 为什么会炸 | **知识点 1**：`let`/`const` 在声明前有暂时性死区（TDZ） |
...
![本课核心问题的直觉图](../assets/lesson-01-overview.svg)

> 📖 **读图**：从左到右看，先登记名字，再限制它能被谁看见，最后确认它实际代表哪类内容。

### 本课地图

| 步骤 | 要解决什么 | 对应知识点 |
|---|---|---|
| 第 1 步 | 先判断名字何时出现、能看多远 | var·let·const 与 TDZ |
| 第 2 步 | 再认识值本身有哪些类别 | 数据类型全景 |
| 第 3 步 | 最后选择合适的检测工具 | 类型检测的四种方式 |

> 🧭 **路线提示**：本课从“名字的规则”走到“值的类别”，再走到“如何验证”。

### 知识点 1：var·let·const 与 TDZ
> 🧭 第 1/3 步｜承接：第二幕的困惑是“名字为什么还活着、`typeof` 为什么有时会报错” → 本步：先拆开作用域、提升与初始化时机。
...
> ✅ **困惑 1、2 已解**：`i` 活着是因为 `var` 是函数作用域（不是块级）；`typeof` 在**完全未声明**的名字上返回 `'undefined'`，但访问 TDZ 中的 `let`/`const` 仍会抛 `ReferenceError`。
...
### 知识点 2：数据类型全景
> 🧭 第 2/3 步｜承接：知道名字何时可用后，还不知道它代表什么 → 本步：建立原始值、对象与特殊值的全景。
...
### 知识点 3：类型检测的四种方式
> 🧭 第 3/3 步｜承接：知道值的类别仍不等于能可靠识别它 → 本步：把检测工具和适用场景对上号。
...
flowchart LR
    ...
    B --> E[粗粒度类型]
    C --> F[原型链关系]
    D --> G[Array.isArray / Number.isNaN]
```

> 📖 **读图**：从左到右看，先拿到粗粒度结果，再按对象原型或具体业务类型选择更专用的判断工具。

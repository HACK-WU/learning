#!/usr/bin/env bash
# 课 6 知识点 3：异步复制风险的机制性证据（简化稳定版）
# 修正：后台批量写入过快导致 BrokenPipe，改为前台分批写入 + 采样
BASE=/tmp/redis-course-l06-risk2
...
echo "  SET + WAIT 2 0:  $(redis-cli -p 6401 wait 2 0) 个从库确认（只有1个从库，超时后返回实际数）"
echo ""

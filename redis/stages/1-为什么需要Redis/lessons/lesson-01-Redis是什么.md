...
> 📖 结论已按官方文档核对（核对于 2026-09 ｜ 来源：redis.io/docs/latest/develop/data-types/、operate/oss_and_stack/management/persistence/）
...
> 📌 **一句话本质**：把"每次读数据都要去磁盘翻一遍"变成"热点数据常驻内存、按结构直接取用"。
>
> ⚖️ **处境对照**：不这么做——90% 的请求反复读同一批热点数据，磁盘 IO 打满而 CPU 才用了 30%，列表页转 3 秒，加机器也只能缓解寻址开销；这么做——热点数据放内存、并且**在服务器端提供列表/哈希/集合等结构**，原来要"取出来在应用里拼"的操作，变成一条命令在服务端做完。代价是内存比磁盘贵、且断电会丢（这正是阶段 3 持久化要解决的问题）。
...
![课 1 一眼全局图](../assets/lesson-01-what-is-redis-overview.svg)

> 看图指引：左边是"每次都去磁盘翻"的老路子——磁盘跑满了、CPU 却很闲；右边是把热点数据放进内存的路子。整张图只讲一件事：**同样的读取，为什么换个地方放就快了**，以及它换来的代价是什么。

### 本课地图（分几步走）

| 步骤 | 这一步要解决什么 | 对应知识点 |
|------|------------------|-----------|
| 第 1 步 | 先搞清楚它到底是为解决什么而生的 | 知识点 1：Redis 的诞生动机 |
| 第 2 步 | 再理解"内存数据结构服务器"到底多了什么 | 知识点 2：内存数据结构服务器 |
| 第 3 步 | 最后划清边界——什么时候不该用它 | 知识点 3：Redis 不是什么 |

### 知识点 1：Redis 的诞生动机——瓶颈在磁盘，不在 CPU

> 🧭 第 1/3 步｜承接：第二幕留下的问题——"如果只是把数据放内存，用 Memcached 不就好了？" → 本步：先把"为什么非得有个新东西"这件事讲透，从它真正的诞生原因说起。
...
> 🧭 第 2/3 步｜承接：上一步证明了"数据放内存"有价值，但没回答"Memcached 也是内存，凭什么还要它" → 本步：讲清楚"数据结构在服务器端"这个关键差异，这才是它真正的立身之本。
...
> 📌 **行话锚定**：本课说的"内存数据结构服务器"，官方原话是 **in-memory data structure store**（Redis 全称 **REmote DIctionary Server**）。在哪遇到：官方文档首页的自我描述与 `redis-server --version`；`TYPE` / `OBJECT ENCODING` 看到的就是"服务器端的数据结构"；监控指标 `used_memory`。它的三种常见身份业界叫 **cache（缓存）/ database（数据库）/ message broker（消息中间件）**。
>
> 📚 官方文档：[Redis 官方介绍](https://redis.io/docs/latest/) ｜ [Redis data types](https://redis.io/docs/latest/develop/data-types/)
...
> 🧭 第 3/3 步｜承接：上两步都在讲它"能做什么、为什么快"，容易让人觉得什么都能往里塞 → 本步：反过来划清边界，知道什么时候**不该**用它——这往往比会用它更值钱。
...
```mermaid
flowchart TB
    A["新需求：要不要放进 Redis"] --> B{"数据量远超内存预算吗"}
    B -->|"是"| C["不适合：内存是硬约束"]
    B -->|"否"| D{"需要多条件组合查询吗"}
    D -->|"是"| E["不适合：它不是查询引擎"]
    D -->|"否"| F{"需要强一致事务与回滚吗"}

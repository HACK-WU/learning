...
> 📖 结论已按官方文档核对（核对于 2026-09 ｜ 来源：redis.io/docs/latest/operate/oss_and_stack/install/install-stack/、develop/tools/cli/、commands/）
...
> 📌 **一句话本质**：把"装好能用"变成"连得上、管得住、关得干净"——并且知道哪条命令一敲下去会把整站拖垮。
>
> ⚖️ **处境对照**：不这么做——照着默认配置上线，`KEYS order:*` 这种命令在 100 万 key 的实例上会让其他命令**干等 239 毫秒**（本课实测），一次调用就够让上游超时雪崩，而你在开发环境用小数据量怎么试都试不出来；这么做——装完先学会看状态、设 TTL、用 `SCAN` 替代遍历（代价：`SCAN` 总耗时 3.42 秒反而比 `KEYS` 的 0.37 秒长，它换来的是**不阻塞别人**，不是自己更快）。
...
![课 2 一眼全局图](../assets/lesson-02-first-run-overview.svg)

> 看图指引：左边是"装完就上线"的后果——一条取出全部数据的命令，让其他请求跟着干等；右边是"连得上、管得住、关得干净"的做法。图只回答一件事：**从"能跑起来"到"能安全上线"，中间差了哪几步**。

### 本课地图（分几步走）

| 步骤 | 这一步要解决什么 | 对应知识点 |
|------|------------------|-----------|
| 第 1 步 | 先把环境装好、连上，并知道怎么干净地关掉 | 知识点 1：安装与启动 |
| 第 2 步 | 再认识能存哪些结构，别一律当成字符串 | 知识点 2：五种基础类型与通用命令 |
| 第 3 步 | 最后学会给数据设过期、并绕开遍历的坑 | 知识点 3：key 设计与过期 |

### 知识点 1：安装与启动——先把环境握在自己手里

> 本知识点关键点：官方源 vs 系统源的版本陷阱 / 三种启动方式 / 配置文件的关键项 / 干净地关闭

> 🧭 第 1/3 步｜承接：第一幕里那条"看起来没什么问题"的 `KEYS order:*`，问题不在命令本身，而在**环境是怎么装、怎么启动的** → 本步：先把环境握在自己手里，学会启动、看状态、干净地关掉。
...
```mermaid
flowchart TB
    A["装之前先选来源"] --> B{"用系统自带源，还是官方源"}
    B -->|"系统自带源"| C["版本偏旧，可能已停止维护"]
    B -->|"官方源"| D["版本新，且与安全修复同步"]
    C --> E["不推荐用于学习之外的环境"]
    D --> F["启动方式"]
    F --> G["临时试：直接运行命令"]
    F --> H["正式用：指定配置文件启动"]
    H --> I["关闭：走正常关停流程<br/>让它先落盘再退出"]
    I --> J{"能直接强制杀掉吗"}
    J -->|"能，但有代价"| K["可能丢数据，仅作最后手段"]
```
...
> 🧭 第 2/3 步｜承接：环境起来了，下一步是"往里放东西"——但如果一律当成字符串存，就会白白丢掉它最值钱的部分 → 本步：认识能存的几种结构，学会按用途选而不是一律 `SET`。
...
```mermaid
flowchart LR
    A["要存的东西"] --> B{"是一个整体值，还是一组元素"}
    B -->|"单个值 / 计数器"| C["String 小盒子"]
    B -->|"有序、可重复"| D["List 管子<br/>两头进出"]
    B -->|"一组字段-值"| E["Hash 表格<br/>可单独改某一格"]
    B -->|"不重复、要做交并差"| F["Set 集合"]
    B -->|"不重复且要按分值排序"| G["ZSet 有序集合"]
    C --> H["选错容器：要么存不下<br/>要么取出来还得自己算"]
```
...
> 🧭 第 3/3 步｜承接：会存了，但"存进去就不管"会同时踩两个坑——内存只涨不落，以及第一幕那条让全站干等的遍历命令 → 本步：学会设过期、并弄清遍历为什么必须换一种走法。
...
```mermaid
flowchart LR
    A["要找出一批 key"] --> B{"一次取回，还是分批走"}
    B -->|"一次取回全部"| C["一次算完再返回<br/>期间别人只能干等"]
    C --> D["数据量一大就阻塞整站"]
    B -->|"分批游标遍历"| E["每次取一小批<br/>中间允许别人插队"]
    E --> F["不阻塞别人<br/>但要循环到游标归零"]
    E --> G["注意：可能重复返回<br/>需要自己去重"]
```

> 📌 **行话锚定**：本课说的"过期"，官方命令是 **EXPIRE / TTL / PERSIST**；`TTL` 返回的三种状态业界按返回值说：**-1（永不过期）/ -2（已不存在）/ 剩余秒数**。在哪遇到：`TTL <key>`；`KEYS` 命令页顶部的官方警告（生产禁用）；`SCAN` 的游标用法；`redis-cli --scan`。官方文档 [commands](https://redis.io/docs/latest/commands/)。
>
> 📚 官方文档：[KEYS（含官方警告）](https://redis.io/docs/latest/commands/keys/) ｜ [SCAN](https://redis.io/docs/latest/commands/scan/) ｜ [EXPIRE](https://redis.io/docs/latest/commands/expire/) ｜ [TTL](https://redis.io/docs/latest/commands/ttl/)

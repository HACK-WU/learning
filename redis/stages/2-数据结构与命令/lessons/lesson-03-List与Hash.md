...
> 📌 **一句话本质**：把"对象与任务在应用内存里自己拼"变成"交给 Redis 端的结构去存、去排队"，并看清这么做的边界在哪。
>
> ⚖️ **处境对照**：不这么做——对象存成一整个 JSON 字符串，改一个字段也要整存整取；慢活堵在下单接口里，用户点"提交"要等三秒。这么做——Hash 支持字段级读写与原子自增，List 两端进出 O(1) 撑起队列。但代价是真实的：Hash 比 String+JSON **只省 9.6%**（不是传说的"省 5 倍"，分片 Hash 才省 21.7%），而 500 万元素的 List 要吃掉 **138 MB** 内存、`LINDEX` 单次虽只要 4.6 毫秒——真正的问题从来不是慢，是占内存。
...
![课 3 一眼全局图](../assets/lesson-03-list-hash-overview.svg)

> 看图指引：左边是"整块存 + 慢活堵在接口里"的现状，右边是"按字段分开存、慢活排成队"的做法。图只回答一件事：**对象怎么存、慢活怎么放，才不至于又慢又丢**。

### 本课地图（分几步走）

| 步骤 | 这一步要解决什么 | 对应知识点 |
|------|------------------|-----------|
| 第 1 步 | 先学会用队列把慢活挪出主流程 | 知识点 1：List 双向操作与阻塞弹出 |
| 第 2 步 | 再弄清这个队列什么时候会辜负你 | 知识点 2：List 当消息队列的三个硬伤 |
| 第 3 步 | 最后解决"对象到底怎么存"的选型问题 | 知识点 3：Hash 存对象 vs String 存 JSON |

### 知识点 1：List 双向操作与阻塞弹出

> 本知识点关键点：两端 O(1) 的队列/栈模型 / BRPOP 阻塞与唤醒 / timeout=0 的坑 / 底层 listpack→quicklist

> 🧭 第 1/3 步｜承接：第一幕里"下单后发短信"堵在主流程，用户等三秒——那就把慢活挪出去 → 本步：学会用两端进出的结构搭出一个能跑的队列。

#### 一句话定义

List 是一组**有序、可重复**的字符串元素，支持从两端以 O(1) 复杂度插入和弹出；`BRPOP`/`BLPOP` 让消费者在队列为空时**阻塞等待**而非空轮询。

> 📌 **行话锚定**：本课说的"小数据紧凑编码、大数据分块链表"，官方叫 **listpack**（小 List，Redis 7+ 取代了早期的 ziplist）与 **quicklist**（大 List，由多个 listpack 节点串成）。在哪遇到：`OBJECT ENCODING` 返回 `listpack` 或 `quicklist`；配置项 `list-max-listpack-size` 控制切换阈值。另外 **`BRPOPLPUSH` 已废弃**，官方建议改用 **`BLMOVE`**。
>
> 📚 官方文档：[Redis data types · Lists](https://redis.io/docs/latest/develop/data-types/) ｜ [BRPOP](https://redis.io/docs/latest/commands/brpop/) ｜ [BLMOVE](https://redis.io/docs/latest/commands/blmove/)
...
> 🧭 第 2/3 步｜承接：上一步搭的队列能跑，但它"能跑"是因为没出事——第二幕问过"它到底还缺什么" → 本步：把三个硬伤摊开，看清它会在什么时候辜负你（开头那 300 多条短信就是这么丢的）。
...
flowchart TB
    A["消息被取出"] --> B["队列里随即消失"]
    B --> C{"处理成功了吗"}
    C -->|"失败"| D["没有确认机制<br/>无人重试，消息已丢"]
    C -->|"成功"| E["正常"]
    A --> F{"想让多个消费者分摊"}
    F -->|"普通队列"| G["同一条只给一个人<br/>无法让多人各处理一遍"]
    A --> H{"想回看历史消息"}
    H -->|"普通队列"| I["取走即删，无法回溯"]
    D --> J["结论：不能丢的场景改用流"]
    G --> J
...
> 📌 Stream 的详细用法见本课之后的补充课 [《Stream 与 Pub/Sub》](lesson-05-Stream与PubSub.md)（含消费者组、PEL/XACK/XCLAIM，以及与 Pub/Sub 的选型）。本课只需知道：**当你需要"消息不能丢"时，答案不是把 List 改造得更复杂，而是换用 Stream。**
...
> 🧭 第 3/3 步｜承接：队列的事告一段落，回到第二幕另一个被质疑的常识——"存对象要用 Hash，比 String 省很多内存"，实测却只省 9.6% → 本步：用实测数据给出带前提的选型判断，而不是一律选 Hash 或一律选 String。

```mermaid
flowchart TB
    A["一批对象要存"] --> B["方案一：整对象存成一个字符串"]
    A --> C["方案二：对象存成一张表"]
    A --> D["方案三：每字段各存一个字符串"]
    A --> E["方案四：拆成多张小表"]
    B --> F["改一个字段也要整存整取<br/>内存基准"]
    C --> G["可单独读写字段<br/>但省得没有传说中多"]
    D --> H["内存开销明显更高"]
    E --> I["最省，但字段数超阈值会退化"]
    G --> J["要部分读写 → 选表"]
    I --> K["要极致省内存 → 选拆表<br/>并控住字段数"]
```
...
> 📌 **行话锚定**：本课说的"对象存成一张表"，官方叫 **Hash**；"整对象存成一个字符串"就是 **String（常存 JSON）**；"拆成多张小表"业界叫 **sharded hash / hash bucketing**。在哪遇到：`OBJECT ENCODING` 返回 `listpack`（小 Hash）或 `hashtable`（大 Hash）；配置项 `hash-max-listpack-entries` 与 `hash-max-listpack-value`（超阈值会退化，内存反而变多）。官方文档说 Hash 能显著省内存，但那是对比"每字段一个 String"，**不是**对比"整对象存 JSON"。

**四种存法的对照**（本课说法 ↔ 行业叫法 ↔ 在哪遇到 ↔ 代价）：

| 本课说法（人话） | 行业标准叫法 | 典型配置 / 在哪遇到 | 代价 |
|---|---|---|---|
| 整对象存成一个字符串 | String + JSON | `SET` / `GET` | 改一个字段也要整存整取 |
| 对象存成一张表 | Hash | `HSET` / `HGET` | 可部分读写，但省内存有限（实测 9.6%） |
| 每字段各存一个字符串 | String per field | 大量独立 key | 内存开销明显更高 |
| 拆成多张小表 | sharded hash | 按 id 取模分桶 | 最省，但字段数超阈值会退化 |

> 📚 官方文档：[Redis data types · Hashes](https://redis.io/docs/latest/develop/data-types/) ｜ [HSET](https://redis.io/docs/latest/commands/hset/) ｜ [MEMORY USAGE](https://redis.io/docs/latest/commands/memory-usage/)
...
> 本课三个知识点合起来解决一个真实场景：**用户下单后把慢活丢进队列，并决定"订单对象"到底怎么存**。
>
> 完整演练见 [应用实战 03：异步任务与对象存储](../../../应用实战/03-异步任务与对象存储.md)——从"两行命令搭一个队列"走到"消息留痕 + 按访问模式选存储"，含可直接运行的代码与分步设计图。
>

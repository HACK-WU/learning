...
- **List 的双向与阻塞**：`BRPOP` 能阻塞等待，这让 List 看起来很像队列——但它**没有消费确认**，消费者崩了消息就丢了。这是理解 Stream 存在意义的前提（详见补充课《Stream 与 Pub/Sub》）。
...
| Stream 与 Pub/Sub 的选型边界 | 补充课 | 说清三者持久化语义差异，按"丢了会不会出事"选型 |
| 消费者组与 PEL / XACK / XCLAIM | 补充课 | 用消费者组实现分摊、确认与崩溃重试 |
...
> **补充课说明**：本课为 2026-09-17 对标官方 [redis.io 网页索引](../../web-index/redis-io/index.md) 审核后新增，用于兑现课 3「Stream 属阶段 2 进阶内容」的承诺，并补齐课 1 架构图中并列的 Pub/Sub。
> 编号说明：文件名序号为阶段内顺序；阶段 3 的《RDB 与 AOF 持久化》课程序号为「课 5」，二者属不同阶段，以目录与标题区分，不影响既有 9 课编号体系。
